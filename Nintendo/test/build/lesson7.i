# 1 "d:/Data/Nintendo/lesson7/source/lesson7.c"
# 1 "d:\\Data\\Nintendo\\lesson7\\build//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "d:/Data/Nintendo/lesson7/source/lesson7.c"
...
# 8 "d:/Data/Nintendo/lesson7/source/lesson7.c" 2
...
# 9 "d:/Data/Nintendo/lesson7/source/lesson7.c" 2
...
# 10 "d:/Data/Nintendo/lesson7/source/lesson7.c" 2
...
# 11 "d:/Data/Nintendo/lesson7/source/lesson7.c" 2
...
# 12 "d:/Data/Nintendo/lesson7/source/lesson7.c" 2
...
# 13 "d:/Data/Nintendo/lesson7/source/lesson7.c" 2
...
# 14 "d:/Data/Nintendo/lesson7/source/lesson7.c" 2

# 1 "d:/Data/Nintendo/lesson7/build/crate_tpl.h" 1
extern const u8 crate_tpl_end[];
extern const u8 crate_tpl[];
extern const u32 crate_tpl_size;
# 16 "d:/Data/Nintendo/lesson7/source/lesson7.c" 2
# 1 "d:/Data/Nintendo/lesson7/build/crate.h" 1
# 17 "d:/Data/Nintendo/lesson7/source/lesson7.c" 2
# 33 "d:/Data/Nintendo/lesson7/source/lesson7.c"
...
float cubedata[][5] = {
 ...
 {-1.0f,1.0f,-1.0f, 1.0f,0.0f},
 {-1.0f,-1.0f,-1.0f, 1.0f,1.0f},
# 79 "d:/Data/Nintendo/lesson7/source/lesson7.c"
 {-1.0f,-1.0f,-1.0f, 0.0f,1.0f},
 {-1.0f,1.0f,-1.0f, 0.0f,0.0f},
 {-1.0f,1.0f,1.0f, 1.0f,0.0f},
 {-1.0f,-1.0f,1.0f, 1.0f,1.0f},
...
void GL_CreateTubeTex(GXTexObj *texObj1, GXTexObj *texObj2) {
 ...
 for (y=-64; y<64; y++) {
  for (x=-64; x<64; x++) {
   ...
   vect_z = height*lengthInv;

   u8 u = ((u8) (128 + (127.0f * vect_y)));
   u8 v = ((u8) (128 + (127.0f * vect_x)));
   u8 w = ((u8) (128 + (127.0f * vect_z)));
   texbuf1[(x+64) + (y+64)*128] = (v<<8|u);
 ...
 u16 *bufptr1 = buf1;
 u16 *buf2 = (u16 *) memalign(32, 128*128);
 u16 *bufptr2 = buf2;
 ...
 GX_InitTexObj(texObj1, buf1, 128, 128, 0x3, 0, 0, 1);
 GX_InitTexObj(texObj2, buf2, 128, 128, 0x1, 0, 0, 1);
...
void GL_CreateLightBulbTexI8(GXTexObj *texLightDir) {
 ...
 GX_InitTexObj(texLightDir, (void *) &bufptr[0], 128, 128, 0x1, 2, 2, 1);
 GX_InitTexObjLOD(texLightDir,1,1, 1, 10, 0.0f, 1, 1, 0);
...
void GL_CreateDotResultTex(GXTexObj *tex) {
 ...
 for (y=0; y<res; y++) {
  for (x=0; x<res; x++) {
   float intensity = ((float) x)/((float) res) + pow(((float) y)/((float) res), 2);
   if (intensity > 255) intensity = 255;
 ...
 GX_InitTexObj(tex, (void *) &bufptr[0], res, res, 0x1, 0, 0, 1);
 GX_InitTexObjLOD(tex,1,1, 1, 10, 0.0f, 1, 1, 0);
 DCFlushRange((void *) bufptr, 128*128);
}
...
void CalcVertexAttributes(Vector *pos, Vector *lpos, int numvertices, Vector *out) {
 int i;
 for(i=0; i<numvertices; i++) {
  Vector lv, pv, half;
  Vector *v = (Vector *) cubedata[i];

  ps_guVecSub(lpos, v, &lv);
  ps_guVecNormalize(&lv);

  ps_guVecSub(pos, v, &pv);
  ps_guVecNormalize(&pv);

  ps_guVecAdd(&lv, &pv, &half);
  ps_guVecScale(&half, &half, 0.5f);
...
void SetupShader() {
 ...
 GX_SetVtxAttrFmt(0, 14, 1, 4, 0);
 GX_SetVtxAttrFmt(0, 15, 1, 4, 0);
 GX_SetVtxAttrFmt(0, 16, 1, 4, 0);
 ...
 GX_SetTexCoordGen(0x1, 1, 5, 30);
 GX_SetTexCoordGen(0x2, 1, 6, 33);
 GX_SetTexCoordGen(0x3, 1, 7, 36);

 GX_LoadTexObj(&tex_dotresult, 0);
# 362 "d:/Data/Nintendo/lesson7/source/lesson7.c"
 GX_SetTevColorIn(tevstage, 15, 15, 15, 12);
 GX_SetTevColorOp(tevstage, 0, 0, 0, 1, 0);
 GX_SetTevOrder(tevstage, 0xff, 0x100, 0xff);
 tevstage++;
...
void DrawQuads(Vector *normals, Vector *tangents, Vector *bitangents, int numverts, Vector *campos, Vector *lpos) {
 ...
 for(i=0; i<numcalls; i++) {
  ...
  GX_LoadTexMtxImm(normalMtx, 30, 0);
  GX_LoadTexMtxImm(bitangentMtx, 33, 0);
  GX_LoadTexMtxImm(tangentMtx, 36, 0);
  ...
  for(j=0; j<4; j++) {
   ...
   GX_TexCoord2f32(lv->x, hv->x);
   GX_TexCoord2f32(lv->y, hv->y);
   GX_TexCoord2f32(lv->z, hv->z);
  }
 }
...
void CalcTangSp(Vector *vertices, float texst[], Vector *tangent, Vector *bitangent) {
 ...
 ps_guVecSub(&vertices[2], &vertices[0], &v20);
 ps_guVecSub(&vertices[1], &vertices[0], &v10);

 det_inv = 1.0f/(tex10u*tex20v - tex10v*tex20u);
 ps_guVecScale(&v10, &v10tex20u, tex20u);
 ps_guVecScale(&v20, &v20tex10u, tex10u);
 ps_guVecScale(&v10, &v10tex20v, tex20v);
 ps_guVecScale(&v20, &v20tex10v, tex10v);
...
int main(int argc, char **argv) {
 ...
 for(i=0; i<numverts; i+=4) {
  ...
  ps_guVecSub(&verts[1], &verts[0], &t);
  ps_guVecSub(&verts[2], &verts[0], &b);
  ps_guVecCross(&b, &t, &pn);
  ...
  for(j=0; j<4; j++) {
   ...
   n.z = cubedata[i+j][2];
   ps_guVecNormalize(&n);
# 623 "d:/Data/Nintendo/lesson7/source/lesson7.c"
   ortht = t;
   orthb = b;

   ps_guVecNormalize(&ortht);
   ps_guVecNormalize(&orthb);
 ...
 while(1) {
  ...
  if (buttonsInv&0x1000&buttonState) { mode^=1; buttonState &= (0x1000^0xFFFF);}
  ...
  Vector lpos = setlight(view,dir, zt, 80.0f);
  ...
  GX_LoadTexObj(&tex_NxNy, 1);
  GX_LoadTexObj(&tex_Nz, 2);


  int idx = 1;
  ps_guMtxIdentity(model);
  ps_guMtxRotAxisRad(model,&cubeAxis[idx],( (rquad) * 0.01745329252f ));
  ps_guMtxTransApply(model, model, -2.0f + (idx * 2.0f), 0.0f,-4.0f - (idx * 2.0f));
  ...
  ps_guMtxInverse(model, modelinverse);
  ps_guVecMultiply(modelinverse, &lpos, &li);
  ps_guVecMultiply(modelinverse, &cam, &ci);
  ...
  float ***vertices = cubedata;
  for(i=0; i<numverts/4; i++) {

   GX_Begin(0x80, 0, 4);
   for(j=0; j<4; j++) {
    float *vertdata = (float *) vertices[i*4+j];
    float *texdata = &vertdata[3];
    GX_Position3f32(vertdata[0], vertdata[1], vertdata[2]);
    GX_TexCoord2f32(texdata[0], texdata[1]);
    GX_TexCoord2f32(0, 0);
