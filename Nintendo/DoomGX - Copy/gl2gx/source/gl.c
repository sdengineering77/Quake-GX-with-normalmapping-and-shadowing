...
// DRS  these get external ref in glint.h
...
void glTexCoord3fv( const GLfloat *stu){
	...
	// DRS TODO how about abusing tangen or bitangent for 3D coords
};
...
void glEnable(GLenum type)
{
	...
	switch(type)
	{
		case GL_ALPHA_TEST: _alphatest_enabled = true; break; // DRS
...
void glDisable(GLenum type){
	...
	switch(type)
	{
		case GL_ALPHA_TEST: _alphatest_enabled = false; break; // DRS
...
// Tenebrae GX
void glAlphaFunc(GLenum  func,  GLclampf  ref) { // DRS
	NO_CALL_IN_BEGIN;
	switch(func) {
		case GL_NEVER:
        case GL_LESS:
...
void gxgl_glGetDoublev(GLenum pname, GLdouble *params ) { // DRS
	switch(pname) {
