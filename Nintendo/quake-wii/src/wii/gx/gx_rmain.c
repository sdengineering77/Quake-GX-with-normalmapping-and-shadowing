...
Returns true if origin+radius is completely outside the frustom
=================
*/
qboolean R_CullRadius(vec3_t origin, float radius) {
	int i;
	for (i=0 ; i<5 ; i++) {
		float dist = guVecDotProduct((Vector *) &origin[0], (Vector *) &frustum[i].normal[0]) - frustum[i].dist + radius;
		if (dist<0) {
			return true;
		}
	}
...
lastposenum = posenum;
	...
	while (1) {
		...
		do {
			...
			GX_Color4u8(0xff, 0xff, 0xff, 0xff); // TODO setup without vertex color
...
void R_DrawAliasModel (entity_t *e)
{
	...
	//guMtxTranspose(modelview, temp);
	//guMtxInverse(temp, nmodelview);
	//guMtxInverse(nmodelview, nmodelview);
//	guMtxTranspose(nmodelview, nmodelview);
	guMtxCopy(modelview,nmodelview);
	for(i=0; i<3; i++) {
		nmodelview[i][3] = 0; 
	}
//	GX_LoadNrmMtxImm(nmodelview, GX_PNMTX0);
...
=============
*/
//void GL_TrivertxToF32(mtrivertx_t *vert, Vector *out) {
//	out->x = vert->v[0];
//	out->y = vert->v[1];
//	out->z = vert->v[2];
//}
...
=============
*/
/*void GL_CreateStripVolumes(dlight_t *light, Vector *mlightpos, int count, mtrivertx_t *verts, mplanenbt_t *nbts) {
	int i, j;
	float dp;
	Vector fverts[3];
	Vector normal;
...
=============
*/
/*void GL_CreateFanVolumes(dlight_t *light, Vector *mlightpos, int count, mtrivertx_t *verts, mplanenbt_t *nbts) {
	int i;
	float dp;
	Vector fverts[3];
	Vector normal;
...
/*void GL_AliasFrameVolume (dlight_t *light, aliashdr_t *paliashdr, int posenum)
{
	...
	tris 		= (maliastri_t *) ((byte *)paliashdr + paliashdr->tridata);
	tris	  += posenum * paliashdr->numtris;
...
// temp hack
/*qboolean CheckTriVisible(maliastri_t *tri, dlight_t *light, Vector *mlightpos, aliashdr_t *paliashdr, int i) {
	Vector delta[3], normal, vertices[3], verts[3], t, b;
	int k;
	float dot, dp;
...
if (showverts  && light->origin[0] == 362) {
Con_DPrintf("====== pose %i ===========\r\n", showverts);		
}
	...
	for(k=0; k<3; k++) {
		...
		verts[k].y = tri->v[k][1];
		verts[k].z = tri->v[k][2];
		guVecMultiply(model, &verts[k], &vertices[k]);
		guVecSub((Vector *)&light->origin[0], &vertices[k], &delta[k]); 
if (showverts && light->origin[0] == 362) {
Vector scaledv, ldir, up;
Mtx lmtx;
up.x = 0;
up.y = 0;
up.z = 1;
guVecSub((Vector *) &currententity->origin[0], (Vector *) &light->origin[0], &ldir);
guVecNormalize(&ldir);
guLookAt(lmtx, (Vector *) &light->origin[0], &up, &ldir);
guMtxConcat(lmtx, model, lmtx);
guVecMultiply(lmtx, &verts[k], &scaledv);
//scaledv.x = verts[k].x * paliashdr->scale[0] + paliashdr->scale_origin[0];
//scaledv.y = verts[k].y * paliashdr->scale[1] + paliashdr->scale_origin[1];
//scaledv.z = verts[k].z * paliashdr->scale[2] + paliashdr->scale_origin[2];


Con_DPrintf("idx %3i m %5.1f %5.1f %5.1f - s %5.1f %5.1f %5.1f - d %5.1f %5.1f %5.1f\r\n", 
	i, verts[k].x, verts[k].y, verts[k].z, 
	scaledv.x, scaledv.y, scaledv.z,
//	vertices[k].x, vertices[k].y, vertices[k].z,
	delta[k].x, delta[k].y, delta[k].z);		
	...
	if (fabs(normal.x) < 0.1 && fabs(normal.y) < 0.1 && fabs(normal.z) < 0.1) {
	if (showverts && i<=40) {
	Con_DPrintf("bail out -");
	}
		return false;
	}
...
if (showverts && light->origin[0] == 362) {
Con_DPrintf("l %5.1f %5.1f %5.1f - n %5.2f %5.2f %5.2f - dp %6.2f %i\r\n", light->origin[0], light->origin[1], light->origin[2], normal.x, normal.y, normal.z, dot, (dot <= 1));		
}

	GL_VecS8ToF32(&tri->normal, &normal);
	dp = guVecDotProduct(&normal, mlightpos) - tri->dist;

//if ((dp >= 0 && dot < 0) || (dp <= 0 && dot > 0)) {	
//Con_DPrintf("%s dp calc: %5.2f tri: %5.2f\r\n", currententity->model->name, dot, dp);	
//}
	
	
	float r = powf(light->radius, 2);
	for(k=0; k<3; k++) {
		float l = powf(delta[k].x, 2) + powf(delta[k].y, 2)  + powf(delta[k].z, 2) ;
//	Con_DPrintf("range1 r %5.1f l %5.1f\r\n", r, l);	
		if (l >= r) {
			return false;
		}
	}
	...
	if (dot < -1) { 
		return true; // too close to surface, no shadow
	}
...
void GL_AliasFrameVolume (dlight_t *light, aliashdr_t *paliashdr, int posenum) {
	...
	Vector 				fverts[3];
	Vector 				*normal;
	qboolean			trivisible[MAXALIASTRIS];
	float 				dp;
	maliastri_t 		*tris;
	mtriplane_t		*planes;
	int					numtris;
...
lastposenum = posenum;
	...
	verts 		+= posenum * paliashdr->numverts;
	// get triangles
	numtris 	= paliashdr->numtris;
	tris 			= (maliastri_t 	*) ((byte *)paliashdr + paliashdr->tridata);
	// get planes
	planes		= (mtriplane_t *) ((byte *)paliashdr + paliashdr->planedata);
	planes		+= posenum * numtris;
	
	// transform light to modelspace
	Mtx 		modelinv, modeltrans;
	Vector	mlightpos;
	guMtxInverse(model, modelinv);
	guVecMultiply(modelinv, (Vector *) &light->origin[0], &mlightpos);
//	guVecNormalize(&mlightpos);
...
//if (showverts && light->origin[0] == 362) {
//Con_DPrintf("====== pose %i ==== lpos %5.1f %5.1f %5.1f =======\r\n", showverts, mlightpos.x, mlightpos.y, mlightpos.z);		
//}

	
	// triangle visibility
//if (strcmp(currententity->model->name, "progs/player.mdl") != 0)
//	return;
	
	for(i=0; i<numtris; i++, planes++) {
		// check if triangle faces the model lightpos
		normal = (Vector *) &planes->normal[0];
		dp = guVecDotProduct(normal, &mlightpos) - planes->dist;
//if (showverts && light->origin[0] == 362) {
//Con_DPrintf("tri %i n %5.2f %5.2f %5.2f dist %5.1f d1 %5.1f dp %5.2f\r\n", i, normal->x, normal->y, normal->z, planes->dist, guVecDotProduct(normal, &mlightpos), dp);		
//}
		

//		if (dp > 1.0f)  {
		if (dp < 1.0f)  {
			// yes, register as being visible
			trivisible[i] = true;
		} else {
			trivisible[i] = false;
		}
...
//}
			...
			for(k=0; k<3; k++) {
//				int vertindex = tris->vertindex[k];
				int vertindex = tris->vertindex[(3-k)%3];
				v = &verts[vertindex];
				fverts[k].x = v->xyz[0];
				fverts[k].y = v->xyz[1];
				fverts[k].z = v->xyz[2];
//if (showverts && light->origin[0] == 362) {
//Con_DPrintf("%5.2f %5.2f %5.2f - ", fverts[k].x, fverts[k].y, fverts[k].z);		
//}
			}

			// neighbour check
			for(j=0; j<3; j++) {
//				int neighbour = tris->neighbour[j];
				int neighbour = tris->neighbour[2-j];
				// draw if either edge has no neighbour or its neighbouring triangle is not visible
				if (neighbour == -1 || !trivisible[neighbour]) {
					vismask |= 1<<j; // edge is visible
				}
			}
//if (showverts && light->origin[0] == 362) {
//Con_DPrintf("n %3i %3i %3i edges %i %i %i\r\n", tris->neighbour[0], tris->neighbour[1], tris->neighbour[2], vismask & 0x1, vismask & 0x2, vismask & 0x4);		
//}

//vismask = 0x7;			
			// add it (TODO: add vismask)
			UT_CreateVolumeFromVerticesTranslate(light, fverts, 3, vismask, model);
//			UT_CreateVolumeFromVerticesTranslate(light, fverts, 3, model);
...
void R_SetFrustum (void)
{
	...
	// DRS
	for(i=0; i<3; i++) {
		frustum[4].normal[i] = vpn[i];
	}
	frustum[4].type = PLANE_ANYZ;
	frustum[4].dist = DotProduct (r_origin, frustum[4].normal);
...
// current viewleaf
	...
	V_SetContentsColor (CONTENTS_EMPTY); // use colored fog instead
