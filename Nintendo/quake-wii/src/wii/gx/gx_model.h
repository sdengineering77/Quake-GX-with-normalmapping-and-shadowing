...
#define SURF_UNDERSLIME			0x100
#define SURF_UNDERLAVA			0x200
...
#define SHADOWVOL_CASTERTYPE_SURFACE		0x01
#define SHADOWVOL_CASTERTYPE_TRIANGLE		0x02
typedef struct shadowvol_s {
	shadowvolfrag_t			*nextfrag;
	struct shadowvol_s	*nextvol;
	volumeneighbour_t		*nextneighbour;
	void								*caster; // holds the geometry responsible for this volume
	int								castertype;
...
// passage vertex
typedef struct mpassagevert_s {
	struct 		mpassagevert_s *nextvert;
	vec3_t 	v;
} mpassagevert_t;

// passages
#define PASSAGE_PLANEBACK		0x01
typedef struct mpassage_s {
	mpassagevert_t 		*nextvert;
	struct mpassage_s *nextpassage;
	mplane_t					*plane;
	int							flags; // planeback
	struct mleaf_s			*leafs[2]; // 0 = front, 1 = back
} mpassage_t;
...
#define MAX_SURFLIGHTS		6
#define MAX_SURFATTENFACT	0.15f
...
typedef struct mvertxref_s {
	signed short vertindex;
	vecs8_t	tangent;
	vecs8_t	bitangent;	
} mvertxref_t;
...
typedef struct {
	...
	int							numposes;
	int							poseverts;
	int							vertxrefdata;// mvertxref_t
	int							vertxdata;	// mtrivertx_t
	int							commands;	// gl command list with embedded s/t
	int							tridata; // maliastri_t
	int							planedata; // mtriplane_t;
