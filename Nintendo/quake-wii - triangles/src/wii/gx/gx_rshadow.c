...
void UT_DefragShadowTriMem(void) {
	int i, j;//, k;
	int lastused = r_numshadowtris - 1;
	int firstfree = r_shadowtriidx; // as a side effect this contains the first free tri after defrag
	Con_DPrintf("Defrag");
	for(i=firstfree; i<r_numshadowtris; i++) {
		if (!r_shadowtri_t[i].prevtri) {
			// found first free tri
			firstfree = i;
			// search for a used
			for(j=lastused; j>i; j--) {
				if (r_shadowtri_t[j].prevtri) {
					lastused = j;
					r_numshadowtris = lastused;
					
					// triangle 'j' has ptr to prev, trace it, and set correct new
					r_shadowtri_t[j].prevtri->nexttri = &r_shadowtri_t[i];
					
					r_shadowtri_t[i].prevtri = r_shadowtri_t[j].prevtri;
					r_shadowtri_t[i].nexttri = r_shadowtri_t[j].nexttri;
					r_shadowtri_t[i].plane = r_shadowtri_t[j].plane;
					r_shadowtri_t[i].p1 = r_shadowtri_t[j].p1;
					r_shadowtri_t[i].p2 = r_shadowtri_t[j].p2;
					r_shadowtri_t[i].p3 = r_shadowtri_t[j].p3;

					r_shadowtri_t[j].prevtri = NULL;
...
shadowtri_t *UT_AllocShadowTri(void) {
	...
	if (r_numshadowtris < MAX_SHADOWTRIS) {
		...
		int i;//,j;
//		for (j=0; j<2; j++) {
			for(i=r_shadowtriidx; i<MAX_SHADOWTRIS; i++) {
				if (!r_shadowtri_t[i].prevtri) {
					returntri = &r_shadowtri_t[i];
					r_shadowtriidx = i + 1; // to find next one faster
					break;
				}
			}
//			if (returntri) {
...
//		returntri->used = true;
		returntri->prevtri = (shadowtri_t *) 0x01;
	} else {
...
//Con_DPrintf("Delete curr %p next %p delete %p\r\n", curr, curr->nexttri, deleteit);
		if (curr == deleteit) {
			...
			if (deleteit->nexttri) {
				deleteit->nexttri->prevtri = prev;
			}
//			deleteit->used = false;
			deleteit->prevtri = NULL;
			success = 1;
			int idx = (deleteit - &r_shadowtri_t[0]) / sizeof(shadowtri_t);
			if (idx < r_shadowtriidx) {
				r_shadowtriidx = idx;
			}
			break;
		}
...
int UT_CountTrianglesInVolume(shadowvolt_t *vol) {
	int count = 0;
	shadowtri_t *t = vol->nexttri;
	while(t) {
		count++;
		t = t->nexttri;
	}
...
*/	
	...
	// 6) Only one edge intersecting, plane cuts at opposite vertex
	if (c1 && !c2 && !c3) {
		// * p1 - p2 intersects plane at pi1, plane intersection at c3
		// calculate dp with p1 to determine front and back triangles
		float dp;
		Vector p1_plane;
		*num_front = 1;
		*num_back = 1;
		guVecSub(&triangle->p1, plane_xyz, &p1_plane);
		dp = guVecDotProduct(&p1_plane, plane_normal);
		if (dp >= 0) {
			// front p1 - pi1 - p3 back pi1 - p2 - p3
			R_FillTriangle(&triangles_front[0], &triangle->p1,	&pi1,				&triangle->p3);
			R_FillTriangle(&triangles_back[0], &pi1, 				&triangle->p2,	&triangle->p3);
		} else {
			// back p1 - pi1 - p3 front pi1 - p2 - p3
			R_FillTriangle(&triangles_back[0], &triangle->p1,	&pi1,				&triangle->p3);
			R_FillTriangle(&triangles_front[0], &pi1, 				&triangle->p2,	&triangle->p3);
		}	
	} else
	// 7) Only one edge intersecting, plane cuts at opposite vertex
	if (c2 && !c1 && !c3) {
		// * p2 - p3 intersects plane at pi2, plane intersection at c1
		// calculate dp with p2 to determine front and back triangles
		float dp;
		Vector p2_plane;
		*num_front = 1;
		*num_back = 1;
		guVecSub(&triangle->p2, plane_xyz, &p2_plane);
		dp = guVecDotProduct(&p2_plane, plane_normal);
		if (dp >= 0) {
			// front p2 - pi2 - p1 back pi2 - p3 - p1
			R_FillTriangle(&triangles_front[0], &triangle->p2,	&pi2,				&triangle->p1);
			R_FillTriangle(&triangles_back[0], &pi2, 				&triangle->p3,	&triangle->p1);
		} else {
			// back p2 - pi2 - p1 front pi2 - p3 - p1
			R_FillTriangle(&triangles_back[0], &triangle->p2,	&pi2,				&triangle->p1);
			R_FillTriangle(&triangles_front[0], &pi2, 				&triangle->p3,	&triangle->p1);
		}	
	} else
	// 8) Only one edge intersecting, plane cuts at opposite vertex
	if (c3 && !c2 && !c1) {
		// * p3 - p1 intersects plane at pi3, plane intersection at c2
		// calculate dp with p1 to determine front and back triangles
		float dp;
		Vector p3_plane;
		*num_front = 1;
		*num_back = 1;
		guVecSub(&triangle->p3, plane_xyz, &p3_plane);
		dp = guVecDotProduct(&p3_plane, plane_normal);
		if (dp >= 0) {
			// front p3 - pi3 - p2 back pi3 - p1 - p2
			R_FillTriangle(&triangles_front[0], &triangle->p3,	&pi3,				&triangle->p2);
			R_FillTriangle(&triangles_back[0], &pi3, 				&triangle->p1,	&triangle->p2);
		} else {
			// back p3 - pi3 - p2 front pi3 - p1 - p2
			R_FillTriangle(&triangles_back[0], &triangle->p3,	&pi3,				&triangle->p2);
			R_FillTriangle(&triangles_front[0], &pi3, 				&triangle->p1,	&triangle->p2);
		}	
	}
	
	// error
	else {
		Con_Printf("Some stuff goes wrong here!!!! %p %p %p", c1, c2, c3);
	}
...
//		float scale_i = rad2/dist; // TODO calculate scales in separate loop
//		float scale_j = rad2/dist;
...
qboolean R_IsTriangleInVolumeBounds(shadowvolt_t *vol, shadowtri_t *triangle) {
	...
	qboolean showDebug = false;
	
	int n_y = (triangle->plane->plane_normal.y * 10.0f);
	int n_z = (triangle->plane->plane_normal.z * 10.0f);
	int p1x = triangle->p1.x;
	int p2x = triangle->p2.x;
	int p3x = triangle->p3.x;
	int p1y = triangle->p1.y;
	int p2y = triangle->p2.y;
	int p3y = triangle->p3.y;
...
//Con_DPrintf("tri in volume: %d %d %d %d %d \r\n", n_y, n_z, p1x, p2x, p3x); 
		
if (showDebug) {
Con_DPrintf("tri in volume: p1 %5.1f %5.1f %5.1f  p2 %5.1f %5.1f %5.1f p3 %5.1f %5.1f %5.1f plane %5.1f %5.1f %5.1f xyz %5.1f %5.1f %5.1f\r\n", 
...
Con_DPrintf("vol has no planes???\r\n"); 
	...
	for(i=0; i<numplanes; i++) {
		...
		for(p=0; p<3; p++) {
			if (p == 0) {
				guVecSub(&triangle->p1, curr->plane_xyz, &ptri_xyz);
			} else
			if (p == 1) {
				guVecSub(&triangle->p2, curr->plane_xyz, &ptri_xyz);
			} else
			if (p == 2) {
				guVecSub(&triangle->p3, curr->plane_xyz, &ptri_xyz);
			}
			dp = guVecDotProduct(&curr->plane_normal, &ptri_xyz);

//Con_DPrintf("tri in volume: pl %i pt %i delta %5.1f %5.1f %5.1f  normal %5.1f %5.1f %5.1f xyz %5.1f %5.1f %5.1f dp %5.2f \r\n", i, p, 
//	ptri_xyz.x, 	ptri_xyz.y, 	ptri_xyz.z,
//	curr->plane_normal.x, curr->plane_normal.y, curr->plane_normal.z, 
//	curr->plane_xyz->x, curr->plane_xyz->y, curr->plane_xyz->z, 
//	dp);
...
//	return true;
	if (showDebug) {
//	if (ret || showDebug) {
		if (ret)  Con_DPrintf("tri IS in volume\r\n");
		if (showDebug)  Con_DPrintf("======== showDebug ========== \r\n");
		for(i=0; i<numplanes; i++) {
			Vector ptri_xyz;
			shadowplane_t *curr = &vol->planes[i];
			// tri points
...
Con_DPrintf("- 2 - numTrianglesOnPlane %i -", numTrianglesOnPlane);
	for(i=0; i<numTrianglesOnPlane; i++) {
//Con_DPrintf("- 3 - it %i - ", i);
		R_CutTriangle(
			/* in */		&triangles[i], 
...
//Con_DPrintf("- 4 - it %i - ", i);
		// any cuts?
		if (num_front+num_back > 1) {
//Con_DPrintf("Triangle cuts back!!!\r\n");
			return true;
		}
	}
//Con_DPrintf("Triangle doesn't cut back!!!\r\n");
	return false;
}
...
qboolean R_IsRealCut(
	/* in */		shadowtri_t 		*triangle, 
					shadowvolt_t		*cuttingplane_vol,
					shadowplane_t	*cuttingplane) {
	Vector *c[3];
	Vector pi[3];
	Vector *plane_normal = &cuttingplane->plane_normal; 
	Vector *plane_xyz = cuttingplane->plane_xyz;
	c[0] = c[1] = c[2] = NULL;
	// ok, we check if the triangle intersects with plane. This results in an intersection point for each edge
	c[0] = R_CalculateIntersection(&triangle->p1, &triangle->p2, plane_normal, plane_xyz, &pi[0]) ;
	c[1] = R_CalculateIntersection(&triangle->p2, &triangle->p3, plane_normal, plane_xyz, &pi[1]) ;
	c[2] = R_CalculateIntersection(&triangle->p3, &triangle->p1, plane_normal, plane_xyz, &pi[2]) ;
	
	// for each intersection we check if it is located on a triangle on the cutting plane
	// so we'll iterate though each triangle, see if it is on the cutting plane and then check intersectionpoints
//	qboolean intersectedCuttingPlane = false;
	shadowtri_t *currtri = cuttingplane_vol->nexttri;
	while(currtri) {
		if (currtri->plane == cuttingplane) {
			int i;
			// this triangle is on plane
		    // check intersections on tri
			for (i=0; i<3; i++) {
				// is this a intersecting point?
				if (c[i]) {
					// yes, check if intersecting point on triangle
					if (R_CheckSide(&pi[i], &currtri->p1, &currtri->p2, &currtri->p3) && 
						R_CheckSide(&pi[i], &currtri->p2, &currtri->p3, &currtri->p1) && 
						R_CheckSide(&pi[i], &currtri->p3, &currtri->p1, &currtri->p2)) {
						return true;
					}
				}
			}
	...
	return false;//intersectedCuttingPlane;
}

qboolean debugCheckPoints(shadowtri_t *tri) {
	int i, j;
	Vector *p = &tri->p1;
	for (i=0; i<3; i++) {
		for(j=i+1; j<3; j++) {
			...
			if (x == -608 && y == 828 && z == 129) {
				Con_DPrintf("x == -608 && y == 828 && z == 129");
				return true;
			}
		}
	}
...
void R_CutVolumes(dlight_t *light) {
	...
	int i, j, count_new_add = 0, count_del = 0, count_cut = 0, count_cutback = 0;
	int num_volumes;
	shadowvolt_t *vol_i;
	shadowvolt_t *vol_j;
...
//Con_DPrintf("iteration %i - ", iteration);
				num_planes = vol2->num_planes;
				planes = vol2->planes;
//				Con_DPrintf("1 vol1 %p vol1->next %p planes %i \r\n", vol1, vol1->nexttri, num_planes);
...
//Con_DPrintf("plane %i \r\n", c1);
					...
					while(currtri) {
/*						Con_DPrintf("\r\n tri %p plane nr %i p1 %5.1f %5.1f %5.1f p2 %5.1f %5.1f %5.1f\r\n", currtri, c1, 
...
//Con_DPrintf("- 3 - ");
							...
							if (R_IsRealCut(currtri, vol2, &planes[c1])) {
//							if (R_CutsTrianglesOnPlane(vol2, &planes[c1], currtri)) {
								...
								count_del++;
//Con_DPrintf("- 4 - ");
//Con_DPrintf("before %i ", UT_CountTrianglesInVolume(vol1));

/*Con_DPrintf("=================================\r\n");
Con_DPrintf("tri %p plane nr %i p1 %5.1f %5.1f %5.1f p2 %5.1f %5.1f %5.1f\r\n", currtri, c1, 
	planes[c1].plane_normal.x,
	planes[c1].plane_normal.y,
	planes[c1].plane_normal.z,
	planes[c1].plane_xyz->x,
...
Con_DPrintf("orig  p1 %5.1f %5.1f %5.1f p2 %5.1f %5.1f %5.1f p3 %5.1f %5.1f %5.1f \r\n",
		...
		currtri->p2.x, currtri->p2.y, currtri->p2.z, 
		currtri->p3.x, currtri->p3.y, currtri->p3.z);
Con_DPrintf("orig  plane normal %5.1f %5.1f %5.1f plane xyz %5.1f %5.1f %5.1f \r\n",
		currtri->plane->plane_normal.x, currtri->plane->plane_normal.y, currtri->plane->plane_normal.z,
		currtri->plane->plane_xyz->x, currtri->plane->plane_xyz->y, currtri->plane->plane_xyz->z);
*/		
...
//									showDebug = debugCheckPoints(newtri);
...
//Con_DPrintf("- 7 - ");
//Con_DPrintf("after %i \r\n", UT_CountTrianglesInVolume(vol1));
								
//								if (showDebug) {
//									Con_DPrintf("R_CutTriangle f %i b %i \r\n", num_front, num_back);
//									Con_DPrintf("orig  p1 %5.1f %5.1f %5.1f p2 %5.1f %5.1f %5.1f p3 %5.1f %5.1f %5.1f\r\n",
//											currtri->p1.x, currtri->p1.y, currtri->p1.z, 
//											currtri->p2.x, currtri->p2.y, currtri->p2.z, 
//											currtri->p3.x, currtri->p3.y, currtri->p3.z);
...
Con_DPrintf("num add %i num del %i num cut %i num cutback %i\r\n", count_new_add, count_del, count_cut, count_cutback);
}
...
//if (!isTestMap) num_volumes = 0;
	...
	for (i=0; i<num_volumes; i++) {
		vol_i = &light->staticshadowvolumes[i];
		Con_DPrintf("\r\nR_OptimizeVolumes Nextvol %p\r\n", vol_i);
		for (j=i+1; j<num_volumes; j++) {
			vol_j = &light->staticshadowvolumes[j];

			shadowtri_t 		*currtri = NULL;
//Con_DPrintf("R_OptimizeVolumes NextSub %p\r\n", vol_j);
			...
			for(iteration=0; iteration<2; iteration++) {
				...
				// check each of vol1's tri in vol2's volume
				while(currtri) {
				
				
					if (R_IsTriangleInVolumeBounds(vol2, currtri)) {
						// delete it
						count_del++;
						UT_DeleteTriangleFromVolume(vol1, currtri);
//Con_DPrintf("deleted p1 %5.1f %5.1f %5.1f p2 %5.1f %5.1f %5.1f p3 %5.1f %5.1f %5.1f\r\n",
//currtri->p1.x, currtri->p1.y, currtri->p1.z, 
//currtri->p2.x, currtri->p2.y, currtri->p2.z, 
//currtri->p3.x, currtri->p3.y, currtri->p3.z);
					}
...
Con_DPrintf("R_OptimizeVolumes num del %i\r\n", count_del);
	
}



void R_OptimizeFast(shadowvolt_t *vol_i, dlight_t *light) {
	int j, iteration, count_del = 0;
	int num_volumes;
	shadowvolt_t *vol_j;
...
void R_CutFast(shadowvolt_t *vol_i, dlight_t *light) {
	int j, count_new_add = 0, count_del = 0, count_cut = 0, count_cutback = 0;
	int num_volumes;
	shadowvolt_t *vol_j;
...
void R_CalculateVolumesForLight(dlight_t *light, msurface_t *firstsurface, int num_surfaces) {
	...
	for (i=0; i<num_surfaces; i++) {
		...
		if (vol || (vol = UT_AllocShadowVolume())) {
			if(R_CreateVolumeFromSurface(light, surf, vol)) {
//				if (isTestMap) R_CutFast(vol, light);
//				if (isTestMap) R_OptimizeFast(vol, light);
				vol = NULL;
