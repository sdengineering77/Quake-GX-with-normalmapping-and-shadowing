...
public:
	...
	modelDataWrapper(const modelDataWrapper &other);
	modelDataWrapper(modelDataWrapper &other);
...
//	modelDataWrapper &	operator++();
//	modelDataWrapper 	operator++(int);
//	modelDataWrapper &	operator--();
//	modelDataWrapper 	operator--(int);
//	modelDataWrapper &	operator+=(const int plus);
//	modelDataWrapper &	operator-=(const int minus);
//	modelDataWrapper 	operator+(const int plus);
//	modelDataWrapper 	operator-(const int minus);
//	type * 				operator+(type * plus);
//	type * 				operator-(type * minus);
//	modelDataWrapper &	operator=(const type * ptr);
//	modelDataWrapper &	operator=(type * ptr);
//	modelDataWrapper &	operator=(const int val);
...
typedef struct cm_model_s {
	...
	modelDataWrapper<cm_vertex_t> 			vertices;			// array with all vertices used by the model
//	cm_vertex_t *			vertices;			// array with all vertices used by the model
	int						maxEdges;			// size of edge array
	int						numEdges;			// number of edges
	modelDataWrapper<cm_edge_t>				edges;				// array with all edges used by the model
	modelDataWrapper<cm_node_t>				node;				// first node of spatial subdivision
	// blocks with allocated memory
	modelDataWrapper<cm_nodeBlock_t>		nodeBlocks;			// list with blocks of nodes
	modelDataWrapper<cm_polygonRefBlock_t>	polygonRefBlocks;	// list with blocks of polygon references
	modelDataWrapper<cm_brushRefBlock_t>	brushRefBlocks;		// list with blocks of brush references
	modelDataWrapper<cm_polygonBlock_t>		polygonBlock;		// memory block with all polygons
	modelDataWrapper<cm_brushBlock_t>		brushBlock;			// memory block with all brushes
...
//	cm_brushBlock_t *		brushBlock;			// memory block with all brushes
	...
	int						numMergedPolys;
	int						usedMemory;
	bool					purged; // DRS added
	bool					isPurgable; // DRS added
	int						purgeCnt;
} cm_model_t;
...
public:
	...
	void			ensureModelLoaded(cmHandle_t handle); // DRS added
	void			ensureModelLoaded(cm_model_t *model);

	void			virtualizeAll( void );
	// get clip handle for model
	cmHandle_t		LoadModel( const char *modelName, const bool precache );
	cmHandle_t		LoadModel( const char *modelName, const bool precache, cm_model_t * target );
...
private:			// CollisionMap_load.cpp
	...
	cm_model_t *	LoadRenderModel( const char *fileName, cm_model_t *target );					// ASE/LWO models
...
private:			// CollisionMap_files.cpp
	...
	bool			ParseCollisionModel( idLexer *src, cm_model_t * target ); // DRS added target
	bool			LoadCollisionModelFile( const char *name, unsigned int mapFileCRC, cm_model_t *target = NULL); // DRS: added target
