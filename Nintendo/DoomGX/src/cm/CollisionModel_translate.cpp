...
void idCollisionModelManagerLocal::TranslateTrmEdgeThroughPolygon( cm_traceWork_t *tw, cm_polygon_t *poly, cm_trmEdge_t *trmEdge ) {
	...
	ensureModelLoaded(tw->model);

	// check edges for a collision
	for ( i = 0; i < poly->numEdges; i++) {
		edgeNum = poly->edges[i];
		edge = tw->model->edges.data + abs(edgeNum);
		...
		v1 = tw->model->vertices.data + edge->vertexNum[INTSIGNBITSET(edgeNum)];
		CM_SetVertexSidedness( v1, tw->polygonVertexPlueckerCache[i], trmEdge->pl, trmEdge->bitNum );
		v2 = tw->model->vertices.data + edge->vertexNum[INTSIGNBITNOTSET(edgeNum)];
		...
		epsPl.FromLine( tw->model->vertices.data[edge->vertexNum[0]].p + edge->normal * CM_CLIP_EPSILON,
						tw->model->vertices.data[edge->vertexNum[1]].p + edge->normal * CM_CLIP_EPSILON );
		...
		if ( f2 < tw->trace.fraction ) {
			...
			start = tw->model->vertices.data[edge->vertexNum[0]].p;
			end = tw->model->vertices.data[edge->vertexNum[1]].p;
	...
	if ( f < tw->trace.fraction ) {
		...
		for ( i = 0; i < poly->numEdges; i++ ) {
			...
			if ( edge->checkcount != idCollisionModelManagerLocal::checkCount ) {
				...
				pl.FromLine(tw->model->vertices.data[edge->vertexNum[0]].p, tw->model->vertices.data[edge->vertexNum[1]].p);
...
void idCollisionModelManagerLocal::TranslateVertexThroughTrmPolygon( cm_traceWork_t *tw, cm_trmPolygon_t *trmpoly, cm_polygon_t *poly, cm_vertex_t *v, idVec3 &endp, idPluecker &pl ) {
	...
	if ( f < tw->trace.fraction ) {
		...
		tw->trace.c.modelFeature = v - tw->model->vertices.data;
...
bool idCollisionModelManagerLocal::TranslateTrmThroughPolygon( cm_traceWork_t *tw, cm_polygon_t *p ) {
	...
	if ( tw->pointTrace ) {
		...
		for ( i = 0; i < p->numEdges; i++ ) {
			edgeNum = p->edges[i];
			e = tw->model->edges.data + abs(edgeNum);
			...
			tw->polygonEdgePlueckerCache[i].FromLine( tw->model->vertices.data[e->vertexNum[0]].p,
														tw->model->vertices.data[e->vertexNum[1]].p );

			v = &tw->model->vertices.data[e->vertexNum[INTSIGNBITSET(edgeNum)]];
		...
		for ( i = 0; i < p->numEdges; i++ ) {
			...
			for ( k = 0; k < 2; k++ ) {
				...
				v = tw->model->vertices.data + e->vertexNum[k ^ INTSIGNBITSET(edgeNum)];
