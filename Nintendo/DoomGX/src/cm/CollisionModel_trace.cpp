...
void idCollisionModelManagerLocal::TraceThroughModel( cm_traceWork_t *tw ) {
	...
	if ( !tw->rotation ) {
		...
		idCollisionModelManagerLocal::TraceThroughAxialBSPTree_r( tw, tw->model->node.data, 0, 1, tw->start, tw->end );
	}
	else {
		...
		if ( d > CIRCLE_APPROXIMATION_LENGTH ) {
			...
			for ( i = 0; i < numSteps; i++ ) {
				...
				// trace through spatial subdivision and then through leafs
				idCollisionModelManagerLocal::TraceThroughAxialBSPTree_r( tw, tw->model->node.data, 0, 1, start, end );
		...
		idCollisionModelManagerLocal::TraceThroughAxialBSPTree_r( tw, tw->model->node.data, 0, 1, start, tw->end );
	}
}
