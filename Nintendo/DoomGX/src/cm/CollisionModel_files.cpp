...
void idCollisionModelManagerLocal::WriteCollisionModel( idFile *fp, cm_model_t *model ) {
	...
	if (model && model->name.Find("bioscanbeam", false, 0, model->name.Length()) > -1) {
		Sys_Printf(">>> WriteCollisionModel model %s\r\n", model->name.c_str());
		Sys_Printf(">>> WriteCollisionModel model %s num vertices %d\r\n", model->name.c_str(), model->numVertices);
		Sys_Printf(">>> WriteCollisionModel model %s model->vertices.data %p\r\n", model->name.c_str(), model->vertices.data);
		Sys_Printf(">>> WriteCollisionModel model %s num edges %d\r\n", model->name.c_str(), model->numEdges);
		Sys_Printf(">>> WriteCollisionModel model %s model->edges.data %p\r\n", model->name.c_str(), model->edges.data);
	}
	...
	for ( i = 0; i < model->numVertices; i++ ) {
		fp->WriteFloatString( "\t/* %d */ ( %f %f %f )\n", i, model->vertices.data[i].p[0], model->vertices.data[i].p[1], model->vertices.data[i].p[2] );
	}
	fp->WriteFloatString( "\t}\n" );
	// edges
	fp->WriteFloatString( "\tedges { /* numEdges = */ %d\n", model->numEdges );
	for ( i = 0; i < model->numEdges; i++ ) {
		fp->WriteFloatString( "\t/* %d */ ( %d %d ) %d %d\n", i, model->edges.data[i].vertexNum[0], model->edges.data[i].vertexNum[1], model->edges.data[i].internal, model->edges.data[i].numUsers );
	}
	fp->WriteFloatString( "\t}\n" );
	// nodes
	fp->WriteFloatString( "\tnodes {\n" );
	WriteNodes( fp, model->node.data );
	fp->WriteFloatString( "\t}\n" );
	// polygons
	checkCount++;
	polygonMemory = CountPolygonMemory( model->node.data );
	fp->WriteFloatString( "\tpolygons /* polygonMemory = */ %d {\n", polygonMemory );
	checkCount++;
	WritePolygons( fp, model->node.data );
	fp->WriteFloatString( "\t}\n" );
	// brushes
	checkCount++;
	brushMemory = CountBrushMemory( model->node.data );
	fp->WriteFloatString( "\tbrushes /* brushMemory = */ %d {\n", brushMemory );
	checkCount++;
	WriteBrushes( fp, model->node.data );
	fp->WriteFloatString( "\t}\n" );
	// closing brace
	fp->WriteFloatString( "}\n" );
}
...
void idCollisionModelManagerLocal::ParseVertices( idLexer *src, cm_model_t *model ) {
	...
	model->vertices.data = (cm_vertex_t *) Mem_Alloc( model->maxVertices * sizeof( cm_vertex_t ) );
	for ( i = 0; i < model->numVertices; i++ ) {
		src->Parse1DMatrix( 3, model->vertices.data[i].p.ToFloatPtr() );
		model->vertices.data[i].side = 0;
		model->vertices.data[i].sideSet = 0;
		model->vertices.data[i].checkcount = 0;
	}
	src->ExpectTokenString( "}" );
}
...
void idCollisionModelManagerLocal::ParseEdges( idLexer *src, cm_model_t *model ) {
	...
	model->edges.data = (cm_edge_t *) Mem_Alloc( model->maxEdges * sizeof( cm_edge_t ) );
	for ( i = 0; i < model->numEdges; i++ ) {
		src->ExpectTokenString( "(" );
		model->edges.data[i].vertexNum[0] = src->ParseInt();
		model->edges.data[i].vertexNum[1] = src->ParseInt();
		src->ExpectTokenString( ")" );
		model->edges.data[i].side = 0;
		model->edges.data[i].sideSet = 0;
		model->edges.data[i].internal = src->ParseInt();
		model->edges.data[i].numUsers = src->ParseInt();
		model->edges.data[i].normal = vec3_origin;
		model->edges.data[i].checkcount = 0;
		model->numInternalEdges += model->edges.data[i].internal;
	}
	src->ExpectTokenString( "}" );
}
...
void idCollisionModelManagerLocal::ParsePolygons( idLexer *src, cm_model_t *model ) {
	...
	if ( src->CheckTokenType( TT_NUMBER, 0, &token ) ) {
		model->polygonBlock.data = (cm_polygonBlock_t *) Mem_Alloc( sizeof( cm_polygonBlock_t ) + token.GetIntValue() );
		model->polygonBlock.data->bytesRemaining = token.GetIntValue();
		model->polygonBlock.data->next = ( (byte *) model->polygonBlock.data ) + sizeof( cm_polygonBlock_t );
	}
...
void idCollisionModelManagerLocal::ParseBrushes( idLexer *src, cm_model_t *model ) {
	...
	if ( src->CheckTokenType( TT_NUMBER, 0, &token ) ) {
		model->brushBlock.data = (cm_brushBlock_t *) Mem_Alloc( sizeof( cm_brushBlock_t ) + token.GetIntValue() );
		model->brushBlock.data->bytesRemaining = token.GetIntValue();
		model->brushBlock.data->next = ( (byte *) model->brushBlock.data ) + sizeof( cm_brushBlock_t );
	}
	...
	while ( !src->CheckTokenString( "}" ) ) {
		...
		R_FilterBrushIntoTree( model, model->node.data, NULL, b );
	}
}
...
================
*/
bool idCollisionModelManagerLocal::ParseCollisionModel( idLexer *src, cm_model_t * target ) {
	cm_model_t *model;
	idToken token;
...
Sys_Printf(">>> TRACE CMLOAD %s %p vertices %p\r\n", model->name.c_str(), model, model->vertices.data);
			continue;
		}

		if ( token == "edges" ) {
			ParseEdges( src, model );
Sys_Printf(">>> TRACE CMLOAD %s %p edges %p\r\n", model->name.c_str(), model, model->edges.data);
			continue;
		}

		if ( token == "nodes" ) {
			src->ExpectTokenString( "{" );
			model->node.data = ParseNodes( src, model, NULL );
			src->ExpectTokenString( "}" );
Sys_Printf(">>> TRACE CMLOAD %s %p nodes %p\r\n", model->name.c_str(), model, model->node.data);
			continue;
		}

		if ( token == "polygons" ) {
			ParsePolygons( src, model );
Sys_Printf(">>> TRACE CMLOAD %s %p polygons %p %p\r\n", model->name.c_str(), model, model->polygonRefBlocks.data, model->polygonBlock.data);
			continue;
		}

		if ( token == "brushes" ) {
			ParseBrushes( src, model );
Sys_Printf(">>> TRACE CMLOAD %s %p brushes %p %p\r\n", model->name.c_str(), model, model->brushRefBlocks.data, model->brushBlock.data);
			continue;
		}
...
Sys_Printf(">>> TRACE CMLOAD %s %p used memory %d\r\n", model->name.c_str(), model, model->usedMemory);
	return true;
}
...
================
*/
bool idCollisionModelManagerLocal::LoadCollisionModelFile( const char *name, unsigned int mapFileCRC, cm_model_t *target ) {
	idStr fileName;
	idToken token;
	idLexer *src;
	unsigned int crc;
Sys_Printf(">>> LoadCollisionModelFile %s\r\n", name);
Sys_PrintBacktrace();
	// load it
	fileName = name;
	if (!target) { // DRS TODO: this doesn't make the code any cleaner...
	...
	while ( 1 ) {
		...
		if ( token == "collisionModel" ) {
			if ( !ParseCollisionModel( src, target ) ) {
				delete src;
				return false;
			}
			continue;
