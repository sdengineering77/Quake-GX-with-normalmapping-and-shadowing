...
bool idCollisionModelManagerLocal::TestTrmInPolygon( cm_traceWork_t *tw, cm_polygon_t *p ) {
	...
	ensureModelLoaded(tw->model); // DRS
	...
	if ( tw->isConvex ) {
		...
		for ( i = 0; i < p->numEdges; i++ ) {
			...
			for ( j = 0; j < 2; j++ ) {
				v = &tw->model->vertices.data[edge->vertexNum[j]];
	...
	for ( i = 0; i < p->numEdges; i++ ) {
		...
		tw->polygonEdgePlueckerCache[i].FromLine( tw->model->vertices.data[edge->vertexNum[0]].p,
													tw->model->vertices.data[edge->vertexNum[1]].p );
		v = &tw->model->vertices.data[edge->vertexNum[INTSIGNBITSET(edgeNum)]];
	...
	for ( i = 0; i < p->numEdges; i++ ) {
		...
		for ( j = 0; j < tw->numPolys; j++ ) {
#if 1
			v1 = tw->model->vertices.data + edge->vertexNum[0];
			CM_SetTrmPolygonSidedness( v1, tw->polys[j].plane, j );
			v2 = tw->model->vertices.data + edge->vertexNum[1];
			...
			if ( k >= tw->polys[j].numEdges ) {
				...
				tw->trace.c.contents = p->contents;
				tw->trace.c.material = p->material;
				tw->trace.c.point = tw->model->vertices.data[edge->vertexNum[ !flip ]].p;
