...
void idCollisionModelManagerLocal::FreePolygon( cm_model_t *model, cm_polygon_t *poly ) {
	...
	if ( model->polygonBlock.data == NULL && poly ) {
		Mem_Free( poly );
	}
}
...
void idCollisionModelManagerLocal::FreeBrush( cm_model_t *model, cm_brush_t *brush ) {
	...
	if ( model->brushBlock.data == NULL && brush ) {
		Mem_Free( brush );
	}
}
...
void idCollisionModelManagerLocal::FreeModel( cm_model_t *model ) {
	...
	if ( model->node.data ) {
		FreeTree_r( model, model->node.data, model->node.data );
	}
	// free blocks with polygon references
	for ( polygonRefBlock = model->polygonRefBlocks.data; polygonRefBlock; polygonRefBlock = nextPolygonRefBlock ) {
		nextPolygonRefBlock = polygonRefBlock->next;
		Mem_Free( polygonRefBlock );
	}
	// free blocks with brush references
	for ( brushRefBlock = model->brushRefBlocks.data; brushRefBlock; brushRefBlock = nextBrushRefBlock ) {
		nextBrushRefBlock = brushRefBlock->next;
		Mem_Free( brushRefBlock );
	}
	// free blocks with nodes
	for ( nodeBlock = model->nodeBlocks.data; nodeBlock; nodeBlock = nextNodeBlock ) {
		nextNodeBlock = nodeBlock->next;
		Mem_Free( nodeBlock );
	}
	// free block allocated polygons
	Mem_Free( model->polygonBlock.data );
	// free block allocated brushes
	Mem_Free( model->brushBlock.data );
	// free edges
	Mem_Free( model->edges.data );
	// free vertices
	Mem_Free( model->vertices.data );
	// free the model
	delete model;
}
...
void idCollisionModelManagerLocal::FreeTrmModelStructure( void ) {
	...
	models[MAX_SUBMODELS]->node.data->polygons = NULL;
	models[MAX_SUBMODELS]->node.data->brushes = NULL;
	...
	while( 1 ) {
		for ( pref = node->polygons; pref; pref = pref->next ) {
			...
			for ( i = 0; i < p->numEdges; i++ ) {
				edgeNum = p->edges[i];
				edge = model->edges.data + abs( edgeNum );
				if ( edge->normal[0] == 0.0f && edge->normal[1] == 0.0f && edge->normal[2] == 0.0f ) {
					// if the edge is only used by this polygon
					if ( edge->numUsers == 1 ) {
						dir = model->vertices.data[ edge->vertexNum[edgeNum < 0]].p - model->vertices.data[ edge->vertexNum[edgeNum > 0]].p;
...
cm_model_t *idCollisionModelManagerLocal::AllocModel( void ) {
	...
	model->maxVertices = 0;
	model->numVertices = 0;
	model->vertices.data = NULL;
	model->maxEdges = 0;
	model->numEdges = 0;
	model->edges.data = NULL;
	model->node.data = NULL;
	model->nodeBlocks.data = NULL;
	model->polygonRefBlocks.data = NULL;
	model->brushRefBlocks.data = NULL;
	model->polygonBlock.data = NULL;
	model->brushBlock.data = NULL;
	...
	model->purged = false;
	model->isPurgable = false; // DRS TODO: true when loaded through LoadModel
	model->purgeCnt = 0;
...
cm_node_t *idCollisionModelManagerLocal::AllocNode( cm_model_t *model, int blockSize ) {
	...
	if ( !model->nodeBlocks.data || !model->nodeBlocks.data->nextNode ) {
		...
		nodeBlock->next = model->nodeBlocks.data;
		model->nodeBlocks.data = nodeBlock;
	...
	node = model->nodeBlocks.data->nextNode;
	model->nodeBlocks.data->nextNode = node->parent;
	node->parent = NULL;
...
cm_polygonRef_t *idCollisionModelManagerLocal::AllocPolygonReference( cm_model_t *model, int blockSize ) {
	...
	if ( !model->polygonRefBlocks.data || !model->polygonRefBlocks.data->nextRef ) {
		...
		prefBlock->next = model->polygonRefBlocks.data;
		model->polygonRefBlocks.data = prefBlock;
	...
	pref = model->polygonRefBlocks.data->nextRef;
	model->polygonRefBlocks.data->nextRef = pref->next;
...
cm_brushRef_t *idCollisionModelManagerLocal::AllocBrushReference( cm_model_t *model, int blockSize ) {
	...
	if ( !model->brushRefBlocks.data || !model->brushRefBlocks.data->nextRef ) {
		...
		brefBlock->next = model->brushRefBlocks.data;
		model->brushRefBlocks.data = brefBlock;
	...
	bref = model->brushRefBlocks.data->nextRef;
	model->brushRefBlocks.data->nextRef = bref->next;
...
cm_polygon_t *idCollisionModelManagerLocal::AllocPolygon( cm_model_t *model, int numEdges ) {
	...
	if ( model->polygonBlock.data && model->polygonBlock.data->bytesRemaining >= size ) {
		poly = (cm_polygon_t *) model->polygonBlock.data->next;
		model->polygonBlock.data->next += size;
		model->polygonBlock.data->bytesRemaining -= size;
	} else {
...
cm_brush_t *idCollisionModelManagerLocal::AllocBrush( cm_model_t *model, int numPlanes ) {
	...
	if ( model->brushBlock.data && model->brushBlock.data->bytesRemaining >= size ) {
		brush = (cm_brush_t *) model->brushBlock.data->next;
		model->brushBlock.data->next += size;
		model->brushBlock.data->bytesRemaining -= size;
	} else {
...
void idCollisionModelManagerLocal::SetupTrmModelStructure( void ) {
	...
	node->planeType = -1;
	model->node.data = node;
	// allocate vertex and edge arrays
	model->numVertices = 0;
	model->maxVertices = MAX_TRACEMODEL_VERTS;
	model->vertices.data = (cm_vertex_t *) Mem_ClearedAlloc( model->maxVertices * sizeof(cm_vertex_t) );
	model->numEdges = 0;
	model->maxEdges = MAX_TRACEMODEL_EDGES+1;
	model->edges.data = (cm_edge_t *) Mem_ClearedAlloc( model->maxEdges * sizeof(cm_edge_t) );
...
cmHandle_t idCollisionModelManagerLocal::SetupTrmModel( const idTraceModel &trm, const idMaterial *material ) {
	...
	model->node.data->brushes = NULL;
	model->node.data->polygons = NULL;
	...
	for ( i = 0; i < trm.numPolys; i++, trmPoly++ ) {
		...
		trmPolygons[i]->next = model->node.data->polygons;
		model->node.data->polygons = trmPolygons[i];
	}
	...
	if ( trm.isConvex ) {
		...
		// link brush at node
		trmBrushes[0]->next = model->node.data->brushes;
		model->node.data->brushes = trmBrushes[0];
	}
	// model bounds
...
cm_polygon_t *idCollisionModelManagerLocal::TryMergePolygons( cm_model_t *model, cm_polygon_t *p1, cm_polygon_t *p2 ) {
	...
	edge = model->edges.data + abs(edgeNum);
	delta = model->vertices.data[edge->vertexNum[INTSIGNBITNOTSET(edgeNum)]].p -
					model->vertices.data[edge->vertexNum[INTSIGNBITSET(edgeNum)]].p;
	...
	if ( !keep1 ) {
		...
		GetEdge( model, model->vertices.data[model->edges.data[abs(edgeNum1)].vertexNum[INTSIGNBITSET(edgeNum1)]].p,
					model->vertices.data[model->edges.data[abs(edgeNum2)].vertexNum[INTSIGNBITNOTSET(edgeNum2)]].p,
						&newEdgeNum1, -1 );
		if ( newEdgeNum1 == 0 ) {
			keep1 = true;
		}
	...
	for ( i = 0; i < newp->numEdges; i++ ) {
		...
		model->edges.data[abs(newp->edges[i])].numUsers++;
	}
	...
	while( 1 ) {
		for ( pref = node->polygons; pref; pref = pref->next ) {
			...
			if ( newp ) {
				...
				ReplacePolygons( model, model->node.data, polygon, p, newp );
				// decrease usage count for edges of both merged polygons
				for ( i = 0; i < polygon->numEdges; i++ ) {
					model->edges.data[abs(polygon->edges[i])].numUsers--;
				}
				for ( i = 0; i < p->numEdges; i++ ) {
					model->edges.data[abs(p->edges[i])].numUsers--;
				}
	...
	while( 1 ) {
		do {
			...
			for ( pref = node->polygons; pref; pref = pref->next ) {
				...
				if ( MergePolygonWithTreePolygons( model, model->node.data, p ) ) {
					merge = true;
					break;
				}
			}
...
bool idCollisionModelManagerLocal::PointInsidePolygon( cm_model_t *model, cm_polygon_t *p, idVec3 &v ) {
	...
	for ( i = 0; i < p->numEdges; i++ ) {
		...
		edge = model->edges.data + abs(edgeNum);
		//
		v1 = &model->vertices.data[edge->vertexNum[INTSIGNBITSET(edgeNum)]].p;
		v2 = &model->vertices.data[edge->vertexNum[INTSIGNBITNOTSET(edgeNum)]].p;
		dir1 = (*v2) - (*v1);
		vec = v - (*v1);
...
void idCollisionModelManagerLocal::FindContainedEdges( cm_model_t *model, cm_polygon_t *p ) {
	...
	for ( i = 0; i < p->numEdges; i++ ) {
		...
		w.Clear();
		w += model->vertices.data[edge->vertexNum[INTSIGNBITSET(edgeNum)]].p;
		w += model->vertices.data[edge->vertexNum[INTSIGNBITNOTSET(edgeNum)]].p;
...
void idCollisionModelManagerLocal::FindInternalEdges( cm_model_t *model, cm_node_t *node ) {
	...
	while( 1 ) {
		for ( pref = node->polygons; pref; pref = pref->next ) {
			...
			FindInternalPolygonEdges( model, model->node.data, p );
...
int idCollisionModelManagerLocal::GetVertex( cm_model_t *model, const idVec3 &v, int *vertexNum ) {
	...
	if ( model->numVertices >= model->maxVertices ) {
		...
		oldVertices = model->vertices.data;
		model->vertices.data = (cm_vertex_t *) Mem_ClearedAlloc( model->maxVertices * sizeof(cm_vertex_t) );
		memcpy( model->vertices.data, oldVertices, model->numVertices * sizeof(cm_vertex_t) );
		Mem_Free( oldVertices );

		cm_vertexHash->ResizeIndex( model->maxVertices );
	}
	model->vertices.data[model->numVertices].p = vert;
	model->vertices.data[model->numVertices].checkcount = 0;
...
int idCollisionModelManagerLocal::GetEdge( cm_model_t *model, const idVec3 &v1, const idVec3 &v2, int *edgeNum, int v1num ) {
	...
	if (found) {
		for (e = cm_edgeHash->First( hashKey ); e >= 0; e = cm_edgeHash->Next( e ) )
		{
			...
			if ( model->edges.data[e].numUsers != 1 ) {
				continue;
			}

			vertexNum = model->edges.data[e].vertexNum;
	...
	if ( model->numEdges >= model->maxEdges ) {
		...
		oldEdges = model->edges.data;
		model->edges.data = (cm_edge_t *) Mem_ClearedAlloc( model->maxEdges * sizeof(cm_edge_t) );
		memcpy( model->edges.data, oldEdges, model->numEdges * sizeof(cm_edge_t) );
		Mem_Free( oldEdges );

		cm_edgeHash->ResizeIndex( model->maxEdges );
	}
	// setup edge
	model->edges.data[model->numEdges].vertexNum[0] = v1num;
	model->edges.data[model->numEdges].vertexNum[1] = v2num;
	model->edges.data[model->numEdges].internal = false;
	model->edges.data[model->numEdges].checkcount = 0;
	model->edges.data[model->numEdges].numUsers = 1; // used by one polygon atm
	model->edges.data[model->numEdges].normal.Zero();
	//
...
void idCollisionModelManagerLocal::CreatePolygon( cm_model_t *model, idFixedWinding *w, const idPlane &plane, const idMaterial *material, int primitiveNum ) {
	...
	for ( i = 0, j = 1; i < w->GetNumPoints(); i++, j++ ) {
		...
		if ( polyEdges[numPolyEdges] ) {
			...
			v1num = model->edges.data[ abs(polyEdges[numPolyEdges]) ].vertexNum[ INTSIGNBITNOTSET(polyEdges[numPolyEdges]) ];
	...
	R_FilterPolygonIntoTree( model, model->node.data, NULL, p );
}
...
void idCollisionModelManagerLocal::PolygonFromWinding( cm_model_t *model, idFixedWinding *w, const idPlane &plane, const idMaterial *material, int primitiveNum ) {
	...
	w = WindingOutsideBrushes( w, plane, contents, primitiveNum, model->node.data );
...
void idCollisionModelManagerLocal::ConvertBrush( cm_model_t *model, const idMapBrush *mapBrush, int primitiveNum ) {
	...
	AddBrushToNode( model, model->node.data, brush );
}
...
void idCollisionModelManagerLocal::OptimizeArrays( cm_model_t *model ) {
	...
	for ( i = 0; i < model->numEdges; i++ ) {
		remap[ model->edges.data[i].vertexNum[0] ] = true;
		remap[ model->edges.data[i].vertexNum[1] ] = true;
	}
	...
	for ( i = 0; i < model->numVertices; i++ ) {
		if ( remap[ i ] ) {
			...
			model->vertices.data[ newNumVertices ] = model->vertices.data[ i ];
			newNumVertices++;
		}
	}
	...
	for ( i = 1; i < model->numEdges; i++ ) {
		v = model->edges.data[i].vertexNum;
		v[0] = remap[ v[0] ];
		v[1] = remap[ v[1] ];
	}

	// create remap index and move edges
	//newNumEdges = 1; // DRS TODO: I hope this was newNumEdges = 1 only because edge[0] is always preserved
	newNumEdges = 0; // DRS set this to zero so that write edges doesn't fail in some case

	for ( i = 1; i < model->numEdges; i++ ) {
		if (!newNumEdges) { // DRS added
			newNumEdges = 1;
		}
		// if the edge is used
		if ( model->edges.data[ i ].numUsers ) {
			remap[ i ] = newNumEdges;
			model->edges.data[ newNumEdges ] = model->edges.data[ i ];
			newNumEdges++;
		}
	}
	// change polygon edge indexes
	checkCount++;
	RemapEdges( model->node.data, remap );
	...
	oldVertices = model->vertices.data;
	if ( oldVertices ) {
		model->vertices.data = (cm_vertex_t *) Mem_ClearedAlloc( model->numVertices * sizeof(cm_vertex_t) );
		memcpy( model->vertices.data, oldVertices, model->numVertices * sizeof(cm_vertex_t) );
		Mem_Free( oldVertices );
	}
	...
	oldEdges = model->edges.data;
	if ( oldEdges ) {
		model->edges.data = (cm_edge_t *) Mem_ClearedAlloc( model->numEdges * sizeof(cm_edge_t) );
		memcpy( model->edges.data, oldEdges, model->numEdges * sizeof(cm_edge_t) );
		Mem_Free( oldEdges );
	}
}
...
void idCollisionModelManagerLocal::FinishModel( cm_model_t *model ) {
	...
	checkCount++;
	MergeTreePolygons( model, model->node.data );
	// find internal edges (no mesh can ever collide with internal edges)
	checkCount++;
	FindInternalEdges( model, model->node.data );
	// calculate edge normals
	checkCount++;
	CalculateEdgeNormals( model, model->node.data );
	...
	CM_GetNodeBounds( &model->bounds, model->node.data );
	// get model contents
	model->contents = CM_GetNodeContents( model->node.data );
...
================
*/
cm_model_t *idCollisionModelManagerLocal::LoadRenderModel( const char *fileName, cm_model_t *target ) {
	int i, j;
	...
	model->node.data = CreateAxialBSPTree( model, model->node.data );
	...
	// DRS TODO: remove
	if (model && model->name.Find("bioscanbeam", false, 0, model->name.Length()) > -1) {
		Sys_Printf(">>> load render model %s\r\n", model->name.c_str());
		Sys_Printf(">>> load render model %s num vertices %d\r\n", model->name.c_str(), model->numVertices);
		Sys_Printf(">>> load render model %s model->vertices.data %p\r\n", model->name.c_str(), model->vertices.data);
		Sys_Printf(">>> load render model %s num edges %d\r\n", model->name.c_str(), model->numEdges);
		Sys_Printf(">>> load render model %s model->edges.data %p\r\n", model->name.c_str(), model->edges.data);
	}
...
cm_model_t *idCollisionModelManagerLocal::CollisionModelForMapEntity( const idMapEntity *mapEnt ) {
	...
	model = AllocModel();
	model->node.data = AllocNode( model, NODE_BLOCK_SIZE_SMALL );
	...
	brushCount = CM_CountNodeBrushes( model->node.data );
	if ( brushCount > 4 ) {
		model->node.data = CreateAxialBSPTree( model, model->node.data );
	} else {
		model->node.data->planeType = -1;
	}

	// get bounds for hash
	if ( brushCount ) {
		CM_GetNodeBounds( &bounds, model->node.data );
	} else {
...
void idCollisionModelManagerLocal::PrintModelInfo( const cm_model_t *model ) {
	ensureModelLoaded((cm_model_t *) model);
...
Sys_Printf(">>> idCollisionModelManagerLocal::LoadMap\r\n");
	if ( mapFile == NULL ) {
...
bool idCollisionModelManagerLocal::GetModelVertex( cmHandle_t model, int vertexNum, idVec3 &vertex ) const {
	...
	((idCollisionModelManagerLocal *) this)->ensureModelLoaded(model);
	vertex = models[model]->vertices.data[vertexNum].p;
...
bool idCollisionModelManagerLocal::GetModelEdge( cmHandle_t model, int edgeNum, idVec3 &start, idVec3 &end ) const {
	...
	((idCollisionModelManagerLocal *) this)->ensureModelLoaded(model);
	start = models[model]->vertices.data[models[model]->edges.data[edgeNum].vertexNum[0]].p;
	end = models[model]->vertices.data[models[model]->edges.data[edgeNum].vertexNum[1]].p;
...
bool idCollisionModelManagerLocal::GetModelPolygon( cmHandle_t model, int polygonNum, idFixedWinding &winding ) const {
	...
	for ( i = 0; i < poly->numEdges; i++ ) {
		...
		winding += models[model]->vertices.data[ models[model]->edges.data[abs(edgeNum)].vertexNum[INTSIGNBITSET(edgeNum)] ].p;
	}

	return true;
}

void idCollisionModelManagerLocal::ensureModelLoaded(cmHandle_t handle) {
	if (handle >= 0 && handle < numModels) {
		ensureModelLoaded(models[handle]);
	}
}

void idCollisionModelManagerLocal::ensureModelLoaded(cm_model_t * model) {
	char filename[1024];
	idLexer *src = NULL;
	idToken token;
//if (model->name.Find("pda_world", false, 0, model->name.Length())) {
//	Sys_Printf(">>> TRACE pda_world %s %p nodes %p\r\n", model->name.c_str(), model, model->node.data);
//	Sys_PrintBacktrace();
//}
//Sys_Printf(">>> idCollisionModelManagerLocal::ensureModelLoaded %s %p purged %d\r\n", model->name.c_str(), model, (model && model->purged));
if (model && model->name.Find("bioscanbeam", false, 0, model->name.Length()) > -1) {
	Sys_Printf(">>> ensureModelLoaded model %s\r\n", model->name.c_str());
	Sys_Printf(">>> ensureModelLoaded model %s num vertices %d\r\n", model->name.c_str(), model->numVertices);
	Sys_Printf(">>> ensureModelLoaded model %s model->vertices.data %p\r\n", model->name.c_str(), model->vertices.data);
	Sys_Printf(">>> ensureModelLoaded model %s num edges %d\r\n", model->name.c_str(), model->numEdges);
	Sys_Printf(">>> ensureModelLoaded model %s model->edges.data %p\r\n", model->name.c_str(), model->edges.data);
	Sys_PrintBacktrace();
}

	if (model && model->purged) {
		// reload
		model->purged = false;
		sprintf(filename, "%s.cmtext", model->name.c_str());

		src = new idLexer( filename );
		src->SetFlags( LEXFL_NOSTRINGCONCAT | LEXFL_NODOLLARPRECOMPILE );
		if ( !src->IsLoaded() ) {
			delete src;
			Sys_Error("idCollisionModelManagerLocal::ensureModelLoaded error loading %s\r\n", filename);
		}
...
Sys_Printf(">>> idCollisionModelManagerLocal::ensureModelLoaded parse %s\r\n", model->name.c_str());
				if ( !ParseCollisionModel( src, model ) ) {
					delete src;
					Sys_Error("idCollisionModelManagerLocal::ensureModelLoaded error parsing collision model %s\r\n", filename);
				}
				continue;
			}
...
void		idCollisionModelManagerLocal::virtualizeAll( void ) {
	char filename[1024];
	idFile *file = NULL;
	...
	for (int i=0; i<numModels; i++) {
		...
		cm_brushRefBlock_t *brushRefBlock, *nextBrushRefBlock;
		cm_nodeBlock_t *nodeBlock, *nextNodeBlock;
		cm_model_t *model = models[i];

		if (!model || model->purged || !model->isPurgable) continue;
		...
		if (!model->purgeCnt) {
			...
			if (!file) {
				Sys_Error("Error opening file for write: %s\r\n", filename);
			}
			WriteCollisionModel(file, model);
...
cmHandle_t idCollisionModelManagerLocal::LoadModel( const char *modelName, const bool precache ) {
	return LoadModel(modelName, precache, NULL);
}
...
==================
*/
cmHandle_t idCollisionModelManagerLocal::LoadModel( const char *modelName, const bool precache, cm_model_t * target ) {
	int handle;
	...
	if ( LoadCollisionModelFile( modelName, 0, target ) ) {
		handle = FindModel( modelName );
		if ( handle >= 0 ) {
			models[handle]->isPurgable = true;
			return handle;
		} else {
	...
	if (!target) {
		models[numModels] = LoadRenderModel( modelName, NULL );
		if ( models[numModels] != NULL ) {
			models[numModels]->isPurgable = true;
			numModels++;
			return ( numModels - 1 );
		}
	} else {
		LoadRenderModel( modelName, target );
...
bool idCollisionModelManagerLocal::TrmFromModel_r( idTraceModel &trm, cm_node_t *node ) {
	...
	while ( 1 ) {
Sys_Printf(">>> idCollisionModelManagerLocal::TrmFromModel_r node %p\r\n", node);
if (node) {
	Sys_Printf(">>> idCollisionModelManagerLocal::TrmFromModel_r node->polygons %p\r\n", node->polygons);
}
		for ( pref = node->polygons; pref; pref = pref->next ) {
			p = pref->p;
...
bool idCollisionModelManagerLocal::TrmFromModel( const cm_model_t *model, idTraceModel &trm ) {
	...
	// copy polygons
	checkCount++;
Sys_Printf(">>> TrmFromModel_r %s %p\r\n", model->name.c_str(), model);
Sys_PrintBacktrace();

	if ( !TrmFromModel_r( trm, model->node.data ) ) {
	...
	// copy vertices
	for ( i = 0; i < model->numVertices; i++ ) {
		trm.verts[ i ] = model->vertices.data[ i ].p;
	...
	// copy edges
	for ( i = 0; i < model->numEdges; i++ ) {
		trm.edges[ i ].v[0] = model->edges.data[ i ].vertexNum[0];
		trm.edges[ i ].v[1] = model->edges.data[ i ].vertexNum[1];
	}
 ...
 */
template <class type>
modelDataWrapper<type>::modelDataWrapper() {
	data = NULL;
}
...
template <class type>
modelDataWrapper<type>::modelDataWrapper(const modelDataWrapper<type> &other) {
	*this = other;
}

template <class type>
modelDataWrapper<type>::modelDataWrapper(modelDataWrapper<type> &other) {
	*this = other;
}


//template <class type>
//modelDataWrapper<type>::operator const type * ( void ) const {
//	//printf(">>> operator const modelDataWrapper * () const %p\r\n", data);
//	return data;
//}
//
//template <class type>
//modelDataWrapper<type>::operator const type * ( void ) {
//	//printf(">>> operator const modelDataWrapper<type> * () %p\r\n", data);
//	return data;
//}
//
//template <class type>
//modelDataWrapper<type>::operator type * ( void ) const {
//	//printf(">>> operator const modelDataWrapper * () const %p\r\n", data);
//	return data;
//}
//
//template <class type>
//modelDataWrapper<type>::operator type * ( void ) {
//	//printf(">>> operator const modelDataWrapper<type> * () %p\r\n", data);
//	return data;
//}
//
//template <class type>
//modelDataWrapper<type>::operator byte * ( void ) {
//	//printf(">>> operator const modelDataWrapper<type> * () %p\r\n", data);
//	return data;
//}
//
//template <class type>
//modelDataWrapper<type>::operator void * ( void ) {
//	//printf(">>> operator const modelDataWrapper<type> * () %p\r\n", data);
//	return data;
//}
//
////template <class type>
////modelDataWrapper<type>::operator bool ( void ) {
////	return data != NULL;
////}
////
////template <class type>
////modelDataWrapper<type>::operator int ( void ) {
////	return (int) data;
////}
//
//template <class type>
//type *			modelDataWrapper<type>::operator->() {
////printf(">>> modelDataWrapper<type>::operator->() %p\r\n", data);
//	return data;
//}
//
//template <class type>
...
//
//template <class type>
//type &			modelDataWrapper<type>::operator[](int index) {
//	return data[index];
//}
//
//template <class type>
//type &			modelDataWrapper<type>::operator[](int index) const {
//	return data[index];
//}
//
//
//template <class type>
//modelDataWrapper<type> &	modelDataWrapper<type>::operator++() {
//	data++;
////printf(">>> modelDataWrapper<type>::operator++ ptr = %p\r\n", data);
//	return *this;
//}
//
//template <class type>
//modelDataWrapper<type> 	modelDataWrapper<type>::operator++(int) {
//	modelDataWrapper<type> cpy = *this; // shallow copy is ok
//	data++;
////printf(">>> modelDataWrapper<type>::operator++(int) ptr = %p\r\n", data);
//	return cpy;
//}
//
//template <class type>
...
//
//template <class type>
//modelDataWrapper<type> &	modelDataWrapper<type>::operator+=(const int plus) {
////printf(">>> modelDataWrapper<type>::operator+= %d\r\n", plus);
//	data += plus;
//	return *this;
//}
//
//template <class type>
//modelDataWrapper<type> &	modelDataWrapper<type>::operator-=(const int minus) {
////printf(">>> modelDataWrapper<type>::operator-= %d\r\n", minus);
//	data -= minus;
//	return *this;
//}
...
//
//template <class type>
//type *		modelDataWrapper<type>::operator+(type * plus) {
//	modelDataWrapper<type> cpy = *this; // shallow copy is ok
//	cpy.data += plus;
//	return cpy.data;
//}
//
//template <class type>
//type *		modelDataWrapper<type>::operator-(type * minus) {
//	modelDataWrapper<type> cpy = *this; // shallow copy is ok
//	cpy.data -= minus;
//	return cpy.data;
//
...
//
//template <class type>
//modelDataWrapper<type> &	modelDataWrapper<type>::operator=(const type * ptr) {
////printf(">>> modelDataWrapper<type>::operator= %p\r\n", ptr);
//	this->data = (type *) ptr;
//	return *this;
//}
//
//template <class type>
//modelDataWrapper<type> &	modelDataWrapper<type>::operator=(type * ptr) {
////printf(">>> modelDataWrapper<type>::operator= %p\r\n", ptr);
//	this->data = ptr;
//	return *this;
//}
//
//template <class type>
//modelDataWrapper<type> &	modelDataWrapper<type>::operator=(const int ptr) {
//	this->data = (type *) ptr;
//	return *this;
//}
//
//template <class type>
//bool			modelDataWrapper<type>::operator==(const type * ptr) const {
//	return (this->data == ptr);
//}
//
//template <class type>
//bool			modelDataWrapper<type>::operator==(const int val) const {
