...
void idCollisionModelManagerLocal::DrawEdge( cm_model_t *model, int edgeNum, const idVec3 &origin, const idMat3 &axis ) {
	...
	start = model->vertices.data[edge->vertexNum[side]].p;
	end = model->vertices.data[edge->vertexNum[!side]].p;
	if ( isRotated ) {
		start *= axis;
		end *= axis;
	}
...
void idCollisionModelManagerLocal::DrawPolygon( cm_model_t *model, cm_polygon_t *p, const idVec3 &origin, const idMat3 &axis, const idVec3 &viewOrigin ) {
	...
	if ( cm_backFaceCull.GetBool() ) {
		edgeNum = p->edges[0];
		edge = model->edges.data + abs(edgeNum);
		dir = model->vertices.data[edge->vertexNum[0]].p - viewOrigin;
	...
	if ( cm_drawNormals.GetBool() ) {
		...
		for ( i = 0; i < p->numEdges; i++ ) {
			edgeNum = p->edges[i];
			edge = model->edges.data + abs(edgeNum);
			center += model->vertices.data[edge->vertexNum[edgeNum < 0]].p;
		}
	...
	if ( cm_drawFilled.GetBool() ) {
		...
		for ( i = p->numEdges - 1; i >= 0; i-- ) {
			edgeNum = p->edges[i];
			edge = model->edges.data + abs(edgeNum);
			winding += origin + model->vertices.data[edge->vertexNum[INTSIGNBITSET(edgeNum)]].p * axis;
		}
...
void idCollisionModelManagerLocal::DrawModel( cmHandle_t handle, const idVec3 &modelOrigin, const idMat3 &modelAxis,
	...
	checkCount++;
	DrawNodePolygons( model, model->node.data, modelOrigin, modelAxis, viewPos, radius );
}
