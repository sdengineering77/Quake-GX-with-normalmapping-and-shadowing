...
int idDeviceContext::FindFont( const char *name ) {
	...
	if ( renderSystem->RegisterFont( fileName, fonts[index] ) ){
		...
		// DRS bug??? font struct not revoked from list...
		fonts.RemoveIndex(index);
...
void idDeviceContext::Init() {
	...
	whiteImage = declManager->FindMaterial("guis/assets/white.tga", true, false);
	...
	cursorImages[CURSOR_ARROW] = declManager->FindMaterial("ui/assets/guicursor_arrow.tga", true, false); // DRS keep, hard reference
	cursorImages[CURSOR_HAND] = declManager->FindMaterial("ui/assets/guicursor_hand.tga", true, false);
	scrollBarImages[SCROLLBAR_HBACK] = declManager->FindMaterial("ui/assets/scrollbarh.tga", true, false);
	scrollBarImages[SCROLLBAR_VBACK] = declManager->FindMaterial("ui/assets/scrollbarv.tga", true, false);
	scrollBarImages[SCROLLBAR_THUMB] = declManager->FindMaterial("ui/assets/scrollbar_thumb.tga", true, false);
	scrollBarImages[SCROLLBAR_RIGHT] = declManager->FindMaterial("ui/assets/scrollbar_right.tga", true, false);
	scrollBarImages[SCROLLBAR_LEFT] = declManager->FindMaterial("ui/assets/scrollbar_left.tga", true, false);
	scrollBarImages[SCROLLBAR_UP] = declManager->FindMaterial("ui/assets/scrollbar_up.tga", true, false);
	scrollBarImages[SCROLLBAR_DOWN] = declManager->FindMaterial("ui/assets/scrollbar_down.tga", true, false);
...
idDeviceContext::idDeviceContext() {
    ...
    Sys_DebugPrintf("idDeviceContext::idDeviceContext() size %d\r\n", sizeof(*this));
#endif
...
//Sys_Printf(">>> idDeviceContext::DrawCursor mouse pointer %f %f %f\r\n", *x, *y, size);
	...
	if ( text && color.w != 0.0f ) {
		...
		while (s && *s && count < len) {
			...
			if ( idStr::IsColor((const char*)s) ) {
				...
				s += 2;
				count += 2;
				// DRS TODO: shouldn't this paint something?
				continue;
			} else {
...
void idDeviceContext::DrawEditCursor( float x, float y, float scale ) {
	...
	SetFontByScale(scale);
	float useScale = scale * useFont->glyphScale;
	const glyphInfo_t *glyph2 = &useFont->glyphs[(int) ((overStrikeMode) ? '_' : '|')];
