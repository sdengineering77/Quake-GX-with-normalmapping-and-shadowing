...
idWindow::idWindow(idUserInterfaceLocal *ui) {
    ...
    Sys_DebugPrintf("idWindow::idWindow(idUserInterfaceLocal *ui) size %d\r\n", sizeof(*this));
#endif
...
#ifdef TRACE_CONSTRUCTS
    Sys_DebugPrintf("idWindow::idWindow(idDeviceContext *d, idUserInterfaceLocal *ui) size %d @%p\r\n", sizeof(*this), this);
#endif
...
void * idWindow::operator new(std::size_t size) throw(std::bad_alloc) {
...
void idWindow::operator delete(void *pMem) throw() {
...
void idWindow::Redraw(float x, float y) {
	idStr str;
//Sys_Printf(">>> idWindow::Redraw %f %f %s\r\n", x, y, this->name.c_str());
	...
	for ( int i = 0; i < c; i++ ) {
		if ( drawWindows[i].win ) {
			//Sys_Printf(">>> idWindow::Redraw drawWindows[i].win->Redraw %s\r\n", drawWindows[i].win->GetName());
			drawWindows[i].win->Redraw( clientRect.x + xOffset, clientRect.y + yOffset );
		} else {
			//Sys_Printf(">>> idWindow::Redraw drawWindows[i].simp->Redraw %s\r\n", drawWindows[i].simp->name.c_str());
...
bool idWindow::ParseScript(idParser *src, idGuiScriptList &list, int *timeParm, bool elseBlock ) {
	...
	idToken token;
//Sys_Printf(">>> idWindow::ParseScript\r\n");
...
//Sys_Printf(">>> END idWindow::ParseScript\r\n");
...
//Sys_DebugPrintf(">>> idWindow::ParseInternalVar %s\r\n", _name);
...
//Sys_Printf(">>> ParseInternalVar not handled\r\n");
	return false;
}
...
bool idWindow::ParseRegEntry(const char *name, idParser *src) {
	...
	idWinStr *vars;
	if (src->ReadToken(&tok)) {
		//Sys_Printf(">>> idWindow::ParseRegEntry token: %s\r\n", tok.c_str());
		if (var) {
			var->Set(tok);
			return true;
		}
