...
void idUserInterfaceManagerLocal::BeginLevelLoad() {
	int c = guis.Num();
Sys_Printf(">>> idUserInterfaceManagerLocal::BeginLevelLoad num guis %d\r\n", c);

	for ( int i = 0; i < c; i++ ) {
// DRS		if ( (guis[ i ]->GetDesktop()->GetFlags() & WIN_MENUGUI) == 0 ) {
			guis[ i ]->ClearRefs();
...
Sys_Printf( ">>> purging %s.\r\n", guis[i]->GetSourceFile() );
			// use this to make sure no materials still reference this gui
			bool remove = true;
			// DRS TODO hmmm this actually allocates selfs and increases mem usage....
			// However, since all selfs were deleted when purged, there cannot be any references
			// left so this code should be obsolete
...
void idUserInterfaceManagerLocal::VirtualizeAll( void ) {
	int c = guis.Num();
...
void idUserInterfaceLocal::MakeVirtual() { // DRS added
	isVirtual = true;
	delete desktop;
	desktop = NULL;
}

void idUserInterfaceLocal::EnsureLoaded() { // DRS: added
	if (isVirtual) {
		this->InitFromFile(this->GetSourceFile());
	}
}
...
const char *idUserInterfaceLocal::Comment() const {
	((idUserInterfaceLocal * const) this)->EnsureLoaded();
	if ( desktop ) {
...
#ifdef DOOMGX
	Sys_Printf(">>> idUserInterfaceLocal::InitFromFile %s\r\n", qpath);
	if (fileSystem->FindFile(qpath, false) == FIND_NO) {
		Sys_Printf(">>> Not found, return false\r\n");
		return false;
	}
	Sys_Printf(">>> Found!\r\n");
#endif
...
Sys_Printf(">>> idUserInterfaceLocal::HandleEvent\r\n");
	time = _time;
	((idUserInterfaceLocal * const) this)->EnsureLoaded();
