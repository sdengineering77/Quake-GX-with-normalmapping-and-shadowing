...
BSEntity::BSEntity(idGameBearShootWindow* _game) {
    ...
    Sys_DebugPrintf("BSEntity::BSEntity(idGameBearShootWindow* _game) size %d\r\n", sizeof(*this));
#endif
...
// DRS TODO:
void * BSEntity::operator new(std::size_t size) throw(std::bad_alloc) {
...
void BSEntity::operator delete(void *pMem) throw() {
...
#ifdef TRACE_CONSTRUCTS
    Sys_DebugPrintf("idGameBearShootWindow::idGameBearShootWindow size %d\r\n", sizeof(*this));
#endif
	dc = d;
	gui = g;
	CommonInit();
...
void idGameBearShootWindow::UpdateBear() {
	...
	if ( bear->position.y > 380 ) {
		...
		if ( bear->velocity.Length() < 25 ) {
			...
			if ( bearScale ) {
// DRS TODO				session->sw->PlayShaderDirectly( "arcade_balloonpop" );
			}
		}
	}
