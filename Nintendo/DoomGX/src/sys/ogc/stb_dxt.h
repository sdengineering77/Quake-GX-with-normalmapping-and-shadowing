// stb_dxt.h - v1.05 - CMPR compressor - public domain
// original by fabian "ryg" giesen - ported to C by stb, CMPR by DRS
...
//
// version history:
//   v1.05  - (DRS) modified to do CMPR instead of DXT
...
static void stb__CompressColorBlock(unsigned char *dest, unsigned char *block, int mode)
{
  ...
  for (int i=0; i<32; i+=2) {
	  tmpmask |= ((mask>>i) & 0x03)<<(30-i);
  }
  mask = tmpmask;
  dest[0] = (unsigned char) (max16 >> 8);
  dest[1] = (unsigned char) (max16);
  dest[2] = (unsigned char) (min16 >> 8);
  dest[3] = (unsigned char) (min16);
  dest[4] = (unsigned char) (mask >> 24);
  dest[5] = (unsigned char) (mask >> 16);
  dest[6] = (unsigned char) (mask >> 8);
  dest[7] = (unsigned char) (mask);
}
