...
idRenderModelStatic::idRenderModelStatic() {
	...
	modelname = "<undefined>";
	filename.Clear(); // DRS added
	fileoffset = 0; // DRS added
	bounds.Clear();
	lastModifiedFrame = 0;
	lastArchivedFrame = 0;
	overlaysAdded = 0;
...
void idRenderModelStatic::Print() const {
	if (purged) { // DRS: added
		((idRenderModelStatic * const) this)->LoadModel();
	}
...
int idRenderModelStatic::Memory() const {
	...
	totalBytes += modelname.DynamicMemoryUsed();
...
void idRenderModelStatic::InitFromFile( const char *fileName ) {
	...
	if ( extension.Icmp( "ase" ) == 0 ) {
		loaded		= LoadASE( modelname );
		reloadable	= true;
	} else if ( extension.Icmp( "lwo" ) == 0 ) {
		loaded		= LoadLWO( modelname );
		reloadable	= true;
	} else if ( extension.Icmp( "flt" ) == 0 ) {
		loaded		= LoadFLT( modelname );
		reloadable	= true;
	} else if ( extension.Icmp( "ma" ) == 0 ) {
		loaded		= LoadMA( modelname );
		reloadable	= true;
	} else {
		common->Warning( "idRenderModelStatic::InitFromFile: unknown type for model: \'%s\'", modelname.c_str() );
		loaded		= false;
	}

	if ( !loaded ) {
		common->Warning( "Couldn't load model: '%s'", modelname.c_str() );
		MakeDefaultModel();
		return;
	}
...
void idRenderModelStatic::LoadModel() {
	PurgeModel();
	if (this->filename.IsEmpty()) { // DRS TODO: using filename to detect how to load is a bit cheap
		InitFromFile( modelname );
	} else {
		session->rw->ReloadModelFromMapFile(this); // ... and perhaps (re)loading them via renderworld is too
	}
}
...
================
*/
void idRenderModelStatic::InitEmpty( const char *modelname, const char *filename, int fileoffset ) {
	...
	// shadowed normally
	if ( !idStr::Cmpn( modelname, "_area", 5 ) ) {
	...
	if (filename) { // DRS added
		reloadable = true;
		this->filename = filename;
	} else {
		reloadable = false;	// if it didn't come from a file, we (DRS probably) can't reload it
...
/*
================
idRenderModelStatic::GetFileName
================
*/
const char *idRenderModelStatic::GetFileName() const {
	return filename;
}

int			idRenderModelStatic::GetFileOffset() const {
	return fileoffset;
}
/*
================
...
idRenderModel *idRenderModelStatic::InstantiateDynamicModel( const struct renderEntity_s *ent, const struct viewDef_s *view, idRenderModel *cachedModel ) {
	...
	common->Error( "InstantiateDynamicModel called on static model '%s'", modelname.c_str() );
	return NULL;
}
...
void idRenderModelStatic::FinishSurfaces() {
	...
	for ( i = 0 ; i < numOriginalSurfaces ; i++ ) {
		...
		if ( surf->geometry == NULL || surf->shader == NULL ) {
			MakeDefaultModel();
			common->Error( "Model %s, surface %i had NULL geometry", modelname.c_str(), i );
		}
		if ( surf->shader == NULL ) {
			MakeDefaultModel();
			common->Error( "Model %s, surface %i had NULL shader", modelname.c_str(), i );
		}
	}
...
bool idRenderModelStatic::ConvertASEToModelSurfaces( const struct aseModel_s *ase ) {
	...
	for ( objectNum = 0 ; objectNum < ase->objects.Num() ; objectNum++ ) {
		...
		for ( j = 0; j < mesh->numFaces; j++ ) {
			for ( k = 0; k < 3; k++ ) {
				...
				if ( v < 0 || v >= mesh->numVertexes ) {
					common->Error( "ConvertASEToModelSurfaces: bad vertex index in ASE file %s", modelname.c_str() );
				}
				...
				if ( mesh->numTVFaces == mesh->numFaces && mesh->numTVertexes != 0 ) {
					...
					if ( tv < 0 || tv >= mesh->numTVertexes ) {
						common->Error( "ConvertASEToModelSurfaces: bad tex coord index in ASE file %s", modelname.c_str() );
					}
		...
		if ( tri->numIndexes > mesh->numFaces * 3 ) {
			common->FatalError( "ConvertASEToModelSurfaces: index miscount in ASE file %s", modelname.c_str() );
		}
		if ( tri->numVerts > mesh->numFaces * 3 ) {
			common->FatalError( "ConvertASEToModelSurfaces: vertex miscount in ASE file %s", modelname.c_str() );
		}
...
bool idRenderModelStatic::ConvertLWOToModelSurfaces( const struct st_lwObject *lwo ) {
	...
	if ( layer->point.count <= 0 ) {
		common->Warning( "ConvertLWOToModelSurfaces: model \'%s\' has bad or missing vertex data", modelname.c_str() );
		return false;
	}
	...
	if ( numTVertexes ) {
		...
		common->Warning( "ConvertLWOToModelSurfaces: model \'%s\' has bad or missing uv data", modelname.c_str() );
	  	numTVertexes = 1;
	...
	for ( lwoSurf = lwo->surf, i = 0; lwoSurf; lwoSurf = lwoSurf->next, i++ ) {
		...
		for ( j = 0; j < layer->polygon.count; j++ ) {
			...
			if ( poly->nverts != 3 ) {
				common->Warning( "ConvertLWOToModelSurfaces: model %s has too many verts for a poly! Make sure you triplet it down", modelname.c_str() );
				continue;
			}
		...
		if ( tri->numIndexes > layer->polygon.count * 3 ) {
			common->FatalError( "ConvertLWOToModelSurfaces: index miscount in LWO file %s", modelname.c_str() );
		}
		if ( tri->numVerts > layer->polygon.count * 3 ) {
			common->FatalError( "ConvertLWOToModelSurfaces: vertex miscount in LWO file %s", modelname.c_str() );
		}
	...
	for ( objectNum = 0 ; objectNum < ma->objects.Num() ; objectNum++ ) {
		...
		for ( j = 0; j < mesh->numFaces; j++ ) {
			for ( k = 0; k < 3; k++ ) {
				...
				if ( v < 0 || v >= mesh->numVertexes ) {
					common->Error( "ConvertMAToModelSurfaces: bad vertex index in MA file %s", modelname.c_str() );
				}
				...
				if ( mesh->numTVertexes != 0 ) {
					tv = mesh->faces[j].tVertexNum[k];
					if ( tv < 0 || tv >= mesh->numTVertexes ) {
						common->Error( "ConvertMAToModelSurfaces: bad tex coord index in MA file %s", modelname.c_str() );
					}
					// collapse the tex coord if it was slightly offset
					tv = tvRemap[tv];
				}
		...
		// allocate space for the indexes and copy them
		if ( tri->numIndexes > mesh->numFaces * 3 ) {
			common->FatalError( "ConvertMAToModelSurfaces: index miscount in MA file %s", modelname.c_str() );
		}
		if ( tri->numVerts > mesh->numFaces * 3 ) {
			common->FatalError( "ConvertMAToModelSurfaces: vertex miscount in MA file %s", modelname.c_str() );
		}
...
void idRenderModelStatic::PurgeModel() {
	...
	for ( i = 0 ; i < surfaces.Num() ; i++ ) {
		...
		if ( surf->geometry ) {
			R_ReallyFreeStaticTriSurf( surf->geometry ); // DRS do not wait removing it DRS TODO: negative effects?
