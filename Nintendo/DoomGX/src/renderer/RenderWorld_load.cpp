...
void idRenderWorldLocal::FreeWorld() {
    Sys_Printf(">>> void idRenderWorldLocal::FreeWorld()\r\n");
	int i;
...
void idRenderWorldLocal::TouchWorldModels( void ) {
    Sys_Printf(">>> void idRenderWorldLocal::TouchWorldModels( void )\r\n");
	int i;
...
================
*/
idRenderModel *idRenderWorldLocal::ParseModel( idLexer *src, const char *filename, int fileoffset, idRenderModel *target ) {
    Sys_Printf(">>> idRenderModel *idRenderWorldLocal::ParseModel( idLexer *src )\r\n");
	idRenderModel	*model;
	idToken			token;
	int				i, j;
	srfTriangles_t	*tri;
	...
	if (!target) {
		...
		model->InitEmpty( token, filename, fileoffset );
	} else {
		model = target;
	}
...
DRS fileoffset and target are hacked in to make static map models reloadable
================
*/
idRenderModel *idRenderWorldLocal::ParseShadowModel( idLexer *src, const char *filename, int fileoffset, idRenderModel *target ) {
    Sys_Printf(">>> idRenderModel *idRenderWorldLocal::ParseShadowModel( idLexer *src )\r\n");
	idRenderModel	*model;
	idToken			token;
	int				j;
	srfTriangles_t	*tri;
...
void idRenderWorldLocal::SetupAreaRefs() {
    Sys_Printf(">>> void idRenderWorldLocal::SetupAreaRefs()\r\n");
	int		i;
...
void idRenderWorldLocal::ParseInterAreaPortals( idLexer *src ) {
    Sys_Printf(">>> void idRenderWorldLocal::ParseInterAreaPortals( idLexer *src ) TODO: allocates windings and vec5_t in arena 1\r\n");
	int i, j;
...
void idRenderWorldLocal::ParseNodes( idLexer *src ) {
    Sys_Printf(">>> void idRenderWorldLocal::ParseNodes( idLexer *src )\r\n");
	int			i;
...
int idRenderWorldLocal::CommonChildrenArea_r( areaNode_t *node ) {
    Sys_Printf(">>> int idRenderWorldLocal::CommonChildrenArea_r( areaNode_t *node )\r\n");
	int	nums[2];
...
void idRenderWorldLocal::ClearWorld() {
    Sys_Printf(">>> void idRenderWorldLocal::ClearWorld()\r\n");
	numPortalAreas = 1;
...
void idRenderWorldLocal::FreeDefs() {
    Sys_Printf(">>> void idRenderWorldLocal::FreeDefs()\r\n");
	int		i;
...
bool idRenderWorldLocal::ReloadModelFromMapFile( idRenderModel * model ) {
    Sys_Printf(">>> bool idRenderWorldLocal::ReloadModelFromMapFile( %p )\r\n", model);
	...
	if ( !src->IsLoaded() ) {
		delete src;
		Sys_Error( "idRenderWorldLocal::InitFromMap: %s not found\r\n", filename.c_str() ); // DRS TODO keep it like this?
		return false;
	}

	// search the appropiate location in the file
	src->SetFileOffset(model->GetFileOffset());

	// parse the file
	if ( !src->ReadToken( &token ) ) {
		delete src;
		Sys_Error( "no token found\r\n" ); // DRS TODO keep it like this?
		return false;
	}

	if ( token == "model" ) {
		ParseModel( src, filename, model->GetFileOffset(), model );
	} else if ( token == "shadowModel" ) {
		ParseShadowModel( src, filename, model->GetFileOffset(), model );
	}
...
bool idRenderWorldLocal::InitFromMap( const char *name ) {
    Sys_Printf(">>> bool idRenderWorldLocal::InitFromMap( %s )\r\n", name);
	idLexer *		src;
	idToken			token;
	idStr			filename;
	idRenderModel *	lastModel;
...
#ifndef DOOMGX
	if ( session->writeDemo ) { // DRS TODO: perhaps... has other 'non annotated by DRS TODO' occurences, but all #ifndef DOOMGX'd
		WriteLoadMap();
	}
#endif
	...
	// parse the file
	while ( 1 ) {
		int position = src->GetFileOffset();
		...
		if ( token == "model" ) {
			// DRS TODO: this isn't really needed when models are loaded dynamically
			// so try to skip the data and just append an empty model with correct
			// name and filename set. Applies to shadowmodel as well
			lastModel = ParseModel( src, filename, position ); // DRS: added position
		...
		if ( token == "shadowModel" ) {
			lastModel = ParseShadowModel( src, filename, position ); // DRS: added position
...
void idRenderWorldLocal::ClearPortalStates() {
    Sys_Printf(">>> void idRenderWorldLocal::ClearPortalStates()\r\n");
	int		i, j;
...
void idRenderWorldLocal::AddWorldModelEntities() {
    Sys_Printf(">>> void idRenderWorldLocal::AddWorldModelEntities()\r\n");
	int		i;
...
bool idRenderWorldLocal::CheckAreaForPortalSky( int areaNum ) {
    Sys_Printf(">>> bool idRenderWorldLocal::CheckAreaForPortalSky( %d )\r\n", areaNum);
	areaReference_t	*ref;
