...
idRenderModelManagerLocal::idRenderModelManagerLocal() {
    ...
    Sys_DebugPrintf("idRenderModelManagerLocal::idRenderModelManagerLocal() size %d\r\n", sizeof(*this));
#endif
...
/*
=================
idRenderModelManagerLocal::VirtualizeAll
=================
*/
void idRenderModelManagerLocal::VirtualizeAll( void ) { // DRS added
	for ( int i = 1 ; i < models.Num() ; i++ ) {
		idRenderModel	*model = models[i];

		if ( !model->IsReloadable() || !model->IsLoaded()) {
			continue;
		}
Sys_Printf(">>> purge model: %s (%s) size %d\r\n", model->Name(), model->GetFileName(), model->Memory());
		model->PurgeModel();
	}
}
