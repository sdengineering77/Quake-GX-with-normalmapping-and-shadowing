...
void idMaterial::CommonInit() {
	...
	decalInfo.end[2] = 0;
	decalInfo.end[3] = 0;
if (this->base != NULL && strcmp(this->GetName(), "guis/assets/white") == 0) {
	Sys_Printf(">>> %p idMaterial::CommonInit()\r\n", this);
	Sys_PrintBacktrace();
}
...
//    Sys_DebugPrintf("idMaterial::idMaterial() size %d\r\n", sizeof(*this));
#endif
...
void idMaterial::FreeData() {
	int i;
if (this->base != NULL && strcmp(this->GetName(), "guis/assets/white") == 0) {
	Sys_Printf(">>> %p idMaterial::FreeData()\r\n", this);
	Sys_PrintBacktrace();
}
	if ( stages ) {
...
if (this->base != NULL && strcmp(this->GetName(), "guis/assets/white") == 0) {
	Sys_Printf(">>> %p idMaterial::FreeData() END expressionRegisters %p\r\n", this, expressionRegisters);
}
...
idImage *idMaterial::GetEditorImage( void ) const {
	((idMaterial * const) this)->EnsureNotPurged(); // DRS added
...
bool idMaterial::CheckSurfaceParm( idToken *token ) {
	...
	this->EnsureNotPurged(); // DRS added
...
//DRS TODO:		if ( !token.Icmp( "megaTexture" ) ) {
//			if ( src.ReadTokenOnLine( &token ) ) {
...
void idMaterial::ParseMaterial( idLexer &src ) {
	...
	while ( 1 ) {
if (this->base != NULL && strcmp(this->GetName(), "guis/assets/white") == 0) {
	Sys_Printf(">>> %p numRegisters %d\r\n", this, numRegisters);
}
		if ( TestMaterialFlag( MF_DEFAULTED ) ) {	// we have a parse error
if (this->base != NULL && strcmp(this->GetName(), "guis/assets/white") == 0) {
	Sys_Printf(">>> %p RETURN numRegisters %d TestMaterialFlag( MF_DEFAULTED )\r\n", this, numRegisters);
}
			return;
		}
		if ( !src.ExpectAnyToken( &token ) ) {
			SetMaterialFlag( MF_DEFAULTED );
if (this->base != NULL && strcmp(this->GetName(), "guis/assets/white") == 0) {
	Sys_Printf(">>> %p RETURN numRegisters %d !src.ExpectAnyToken( &token )\r\n", this, numRegisters);
}
			return;
		}
		...
		// DECAL_MACRO for backwards compatibility with the preprocessor macros
			...
			SetMaterialFlag( MF_DEFAULTED );
if (this->base != NULL && strcmp(this->GetName(), "guis/assets/white") == 0) {
	Sys_Printf(">>> %p RETURN numRegisters %d unknown general material parameter '%s' in '%s' \r\n", this, numRegisters, token.c_str(), GetName() );
}
			return;
		}
	}
	...
	AddImplicitStages();
if (this->base != NULL && strcmp(this->GetName(), "guis/assets/white") == 0) {
	Sys_Printf(">>> %p numRegisters %d AddImplicitStages DONE\r\n", this, numRegisters);
}

	// order the diffuse / bump / specular stages properly
	SortInteractionStages();
if (this->base != NULL && strcmp(this->GetName(), "guis/assets/white") == 0) {
	Sys_Printf(">>> %p numRegisters %d SortInteractionStages() DONE\r\n", this, numRegisters);
}
...
if (this->base != NULL && strcmp(this->GetName(), "guis/assets/white") == 0) {
	Sys_Printf(">>> %p numRegisters %d culltype DONE\r\n", this, numRegisters);
}
...
if (this->base != NULL && strcmp(this->GetName(), "guis/assets/white") == 0) {
	Sys_Printf(">>> %p numRegisters %d END\r\n", this, numRegisters);
}
}
...
if (this->base != NULL && strcmp(this->GetName(), "guis/assets/white") == 0) {
Sys_Printf(">>> %p idMaterial::Parse debug\r\n", this);
}
...
if (this->base != NULL && strcmp(this->GetName(), "guis/assets/white") == 0) {
	Sys_Printf(">>> %p idMaterial::Parse numRegisters %d expressionRegisters %p \r\n", this, numRegisters, expressionRegisters);
}
...
if (this->base != NULL && strcmp(this->GetName(), "guis/assets/white") == 0) {
	Sys_Printf(">>> %p idMaterial::Parse numRegisters %d MakeDefault\r\n", this, numRegisters);
}
		MakeDefault();
		return false;
	}
if (this->base != NULL && strcmp(this->GetName(), "guis/assets/white") == 0) {
	Sys_Printf(">>> %p idMaterial::Parse numRegisters %d End\r\n", this, numRegisters);
}
	return true;
}
...
if (numRegisters > 0 && expressionRegisters == NULL) { // DRS TODO remove
	Sys_Printf(">>> %p idMaterial::EvaluateRegisters numRegisters > 0 && expressionRegisters == NULL %s\r\n", this, this->GetName());
	Sys_PrintBacktrace();
}
