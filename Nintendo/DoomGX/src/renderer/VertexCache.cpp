...
#define R_MemAlloc(s) Mem_Alloc(s);
#define R_MemFree(p)  Mem_Free(p);

#else

#define R_MemAlloc(s) memalign(32, s);
#define R_MemFree(p)  free(p);
...
void *idVertexCache::Position( vertCache_t *buffer ) {
	...
	if ( buffer->vbo ) {
		...
		if ( buffer->indexBuffer ) {
			Sys_Printf("DRS TODO: qglBindBufferARB( GL_ELEMENT_ARRAY_BUFFER_ARB, buffer->vbo );\r\n");
		} else {
			Sys_Printf("DRS TODO: qglBindBufferARB( GL_ARRAY_BUFFER_ARB, buffer->vbo );");
		}
...
void idVertexCache::Init() {
	...
	byte	*junk;
	if (!virtualMemory) { // DRS, this junk block makes SRAM map look less pretty
		junk = (byte *)R_MemAlloc( frameBytes );
	} else {
		junk = NULL;
	}
...
void idVertexCache::Alloc( void *data, int size, vertCache_t **buffer, bool indexBuffer ) {
	vertCache_t	*block;
//Sys_Printf(">>> idVertexCache::Alloc\r\n");
...
// DRS TODO	qglBufferDataARB( GL_ARRAY_BUFFER_ARB, (GLsizeiptrARB)size, data, GL_STATIC_DRAW_ARB );
		...
		block->virtMem = R_MemAlloc( size );
		if (data) {
			SIMDProcessor->Memcpy( block->virtMem, data, size );
		}
//Sys_Printf(">>> idVertexCache::Alloc return %p\r\n", block->virtMem);
	}
}
...
vertCache_t	*idVertexCache::AllocFrameTemp( void *data, int size ) {
	vertCache_t	*block;
//Sys_Printf(">>> idVertexCache::AllocFrameTemp data %p size %d\r\n", data, size);
	if ( size <= 0 ) {
...
//Sys_Printf(">>> idVertexCache::AllocFrameTemp virtMem %p vbo %p\r\n", block->virtMem, block->vbo);
...
//Sys_Printf(">>> idVertexCache::Alloc return %p vm+offset %p\r\n", block->virtMem, (byte *)block->virtMem + block->offset);
	}
...
Sys_Printf(">>> idVertexCache::EndFrame\r\n");
	...
	if( !virtualMemory ) {
		...
		Sys_Printf("DRS TODO: idVertexCache::EndFrame qglBindBufferARB( GL_ARRAY_BUFFER_ARB, 0 );\r\n");
		Sys_Printf("DRS TODO: idVertexCache::EndFrame qglBindBufferARB( GL_ELEMENT_ARRAY_BUFFER_ARB, 0 );\r\n");
	}
...
#ifdef DOOMGX
	// DRS: clear GX vertex cache DRS TODO remove if no effect (corrupted texture coordinates)
	GX_InvVtxCache();
#endif
}
