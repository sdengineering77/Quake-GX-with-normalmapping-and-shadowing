...
#else
idCVar idImageManager::image_preload( "image_preload", "0", CVAR_RENDERER | CVAR_BOOL | CVAR_ARCHIVE, "if 0, dynamically load all images" ); // DRS
#endif
...
#else
// DRS TODO: should this be seen as texture mem? If so, make the data point to texture memory
idCVar idImageManager::image_cacheMegs( "image_cacheMegs", "1", CVAR_RENDERER | CVAR_ARCHIVE, "maximum MB set aside for temporary loading of full-sized precompressed images" );
#endif
...
static void makeNormalizeVectorCubeMap( idImage *image ) {
    Sys_Printf("void makeNormalizeVectorCubeMap( idImage *image ) NOT REQUIRED\r\n");
}
...
#else
		qgxSetGLTexParam(-1, -1, globalImages->textureMinFilter, globalImages->textureMaxFilter, globalImages->textureAnisotropy);
#endif
	}
}
