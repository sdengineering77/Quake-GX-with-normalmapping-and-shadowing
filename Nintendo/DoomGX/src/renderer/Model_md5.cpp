...
idMD5Mesh::idMD5Mesh() {
    ...
    Sys_DebugPrintf("idMD5Mesh::idMD5Mesh() size %d\r\n", sizeof(*this));
#endif
...
void idRenderModelMD5::LoadModel() {
	...
	fileSystem->ReadFile( modelname, NULL, &timeStamp );
}
...
idBounds idRenderModelMD5::Bounds( const renderEntity_t *ent ) const {
	...
	if ( !ent ) {
		...
		if (purged) { // DRS: added
			((idRenderModelMD5 * const) this)->LoadModel();
		}
		return bounds;
	}
