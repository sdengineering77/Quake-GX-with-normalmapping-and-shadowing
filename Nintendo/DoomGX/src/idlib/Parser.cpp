...
int idParser::ReadSourceToken( idToken *token ) {
	...
	while( !idParser::tokens ) {
		...
		if ( idParser::scriptstack->ReadToken( token ) ) {
			...
			// DRS TODO: mmm the marker stuff seems to be TOOLS only... remove it?
			// DRS if ( !marker_p ) {
			if ( marker_p < 0) {
...
void idParser::GetStringFromMarker( idStr& out, bool clean ) {
	Sys_Printf("TODO: idParser::GetStringFromMarker\r\n");
/*	DRS This function seems to be used by TOOLS only
 	char*	p;
	char	save;
