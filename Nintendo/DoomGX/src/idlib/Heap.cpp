...
int Sys_Backtrace (void **buffer, int size); // DRS TODO: cleanup
bool _enable_memtrace = false; // DRS TODO: remove
...
void *Sys_Alloc(size_t size) {
	...
	if (_enable_memtrace) {
		...
		char dump[1024];
		dump[0] = '\0';
		int depth = Sys_Backtrace(stack, 12);
		for (int i = 0; i<depth; i++) {
			sprintf(&dump[strlen(dump)], "[%02d:%p] ", i, stack[i]);
		}
		Sys_Printf(">>> MEMTRACE Allocated %d free %d %p %s\r\n", size, info.free_size, p, dump);
	}
...
void Sys_Free(void *p) {
	...
	if (_enable_memtrace) {
		...
		Sys_Printf("Freed %p tot %d free %d\r\n", p, info2.free_size - info1.free_size, info2.free_size);
	}
}
...
void * operator new(std::size_t size) throw(std::bad_alloc) {
	void *alloc;
	if (mem_heap) {
		alloc = Mem_Alloc16(size);
	} else {
		u32 p = (u32) SYS_GetArena1Lo(); // DRS Permanent, malloc wont work pre main()?
		p = (p + 3) &0xFFFFFFFC;
		SYS_SetArena1Lo((void *) (p + size));
		alloc = (void *)p;
	}
	if (!alloc) {
		free(0);
...
void *operator new[](std::size_t size) throw(std::bad_alloc) {
	...
	if (mem_heap) {
		...
		u32 p = (u32) SYS_GetArena1Lo(); // DRS Permanent
		p = (p + 3) &0xFFFFFFFC;
		SYS_SetArena1Lo((void *) (p + size));
		alloc = (void *) p;
	}
	if (!alloc) {
		free(0); // DRS so we can trace
		throw std::bad_alloc();
	}
	return alloc;
}
