...
int sp = idLexer::script_p;
Sys_Printf(">>> idLexer::script_p %d\r\n", sp);
...
================
*/
ID_INLINE int idLexer::CheckString( const char *str ) { // DRS const {
	int i;
...
int idLexer::GetLastWhiteSpace( idStr &whiteSpace ) const {
	whiteSpace.Clear();
	fileDataPtr p = script_p;
//	DRS for ( const char *p = whiteSpaceStart_p; p < whiteSpaceEnd_p; p++ ) {
	for ( p = whiteSpaceStart_p; p < whiteSpaceEnd_p; p++ ) {
		whiteSpace.Append( *p );
	}
...
int idLexer::GetLastWhiteSpaceStart( void ) const {
	return whiteSpaceStart_p; // DRS - buffer;
}
...
int idLexer::GetLastWhiteSpaceEnd( void ) const {
	return whiteSpaceEnd_p; // DRS  - buffer;
}
...
void idLexer::Reset( void ) {
	...
	idLexer::script_p = 0;// DRS idLexer::buffer;
	// pointer in script buffer before reading token
	idLexer::lastScript_p = 0; // DRS idLexer::buffer;
	// begin of white space
	idLexer::whiteSpaceStart_p = 0; // DRS NULL;
	// end of white space
	idLexer::whiteSpaceEnd_p = 0; // DRS NULL;
...
int idLexer::LoadFile( const char *filename, bool OSPath ) {
	...
	length = fp->Length();
	// DRS buf = (char *) Mem_Alloc( length + 1 );
	// DRS buf[length] = '\0';
	// DRS fp->Read( buf, length );
	idLexer::fileTime = fp->Timestamp();
	idLexer::filename = fp->GetFullPath();
	idLib::fileSystem->CloseFile( fp );

	// DRS idLexer::buffer = buf;
	idLexer::length = length;
	// pointer in script buffer
	idLexer::script_p = 0; // DRS idLexer::buffer;
	idLexer::script_p.setFile(filename);//setBuffer(buf, length); // DRS added
	// pointer in script buffer before reading token
	idLexer::lastScript_p = 0; // DRS idLexer::buffer;
	// pointer to end of script buffer
	idLexer::end_p = length; // DRS &(idLexer::buffer[length]);

	idLexer::tokenavailable = 0;
	idLexer::line = 1;
	idLexer::lastline = 1;
	idLexer::allocated = false; // DRS true;
	idLexer::loaded = true;
...
int idLexer::LoadMemory( const char *ptr, int length, const char *name, int startLine ) {
	...
	// DRS idLexer::buffer = ptr;
	idLexer::fileTime = 0;
	idLexer::length = length;
	// pointer in script buffer
	idLexer::script_p = 0; // DRS idLexer::buffer;
	idLexer::script_p.setBuffer((char *) ptr, length);
	// pointer in script buffer before reading token
	idLexer::lastScript_p = 0; // DRS idLexer::buffer;
	// pointer to end of script buffer
	idLexer::end_p = length; // DRS &(idLexer::buffer[length]);
...
void idLexer::FreeSource( void ) {
	...
	if ( idLexer::allocated ) {
		Mem_Free( (void *) script_p.buf);// DRS idLexer::buffer );
		// DRS idLexer::buffer = NULL;
		idLexer::allocated = false;
	}
	idLexer::tokenavailable = 0;
	idLexer::token = "";
...
//    Sys_DebugPrintf("idLexer::idLexer( void ) size %d\r\n", sizeof(*this));
#endif
...
#ifdef TRACE_CONSTRUCTS
//    Sys_DebugPrintf("idLexer::idLexer( int flags ) size %d\r\n", sizeof(*this));
#endif
...
#ifdef TRACE_CONSTRUCTS
//    Sys_DebugPrintf("idLexer::idLexer( const char *filename, int flags, bool OSPath ) size %d\r\n", sizeof(*this));
#endif
...
#ifdef TRACE_CONSTRUCTS
//    Sys_DebugPrintf("idLexer::idLexer( const char *ptr, int length, const char *name, int flags ) size %d\r\n", sizeof(*this));
#endif
...
fileDataPtr::~fileDataPtr() {
	if (master) {
		if (buf && allocated) {
Sys_Printf(">>> ~fileDataPtr Free %p\r\n", buf);
			Mem_Free(buf);
		}
		if (file) {
Sys_Printf(">>> ~fileDataPtr CloseFile %p\r\n", buf);
			fileSystem->CloseFile(file);
		}
	} else {
//Sys_Printf(">>> ~fileDataPtr COPY IS DELETED\r\n");
	}
//Sys_Printf(">>> ~fileDataPtr END\r\n");
}
...
// DRS TODO: what would be of better design; this or putting this in idlexer::loadfile
// DRS TODO: delete allocated if applicable...
void fileDataPtr::setFile(const char *filename) {
	Sys_Printf(">>> fileDataPtr::setFile Open %s\r\n", filename);

	if (file) {
		Sys_Error("fileDataPtr::setFile is being reused!");
	}
...
/*	if (file->GetFileType() == FILETYPE_INZIP) {
		...
		if (length > FILE_DATA_PTR_BUFSIZE) {
			...
			Mem_Free(tempbuf);
			// from now on it is on disk and is 'streamed' instead of read fully into mem
			...
			// create new buffer
			buf = (char *) Mem_Alloc(FILE_DATA_PTR_BUFSIZE);
			if (!buf) {
		...
		Sys_Printf(">>> Finished backup %d\r\n", length);
	} else if (file->Length() <= FILE_DATA_PTR_BUFSIZE) {
		// read file and set as buffer
		void *tempbuf = NULL;
Sys_Printf(">>> READ %s\r\n", filename);

		fileSystem->CloseFile(file);
		size = fileSystem->ReadFile(filename, &tempbuf, NULL);
		setBuffer((char *) tempbuf, size);
	} else {*/
		// stream it from disk
		buf = (char *) Mem_Alloc(FILE_DATA_PTR_BUFSIZE);
		if (!buf) {
...
//	}
	allocated = true;
Sys_Printf(">>> fileDataPtr::setFile allocated  %p\r\n", buf);

}

// DRS TODO: delete file if applicable...
void fileDataPtr::setBuffer(char *buffer, int length) {
	if (!buffer) {
		Sys_Error("fileDataPtr::setBuffer !buffer\r\n");
	}
...
char fileDataPtr::readBuf(int position) {
	if (position < size) {
		if (position < posStart || position >= posEnd) {
			int numread = 0;
			Sys_Printf(">>> fileDataPtr::readBuf update %d %d %d length:%d\r\n", position, posStart, posEnd, this->file->Length());
			// load FILE_DATA_PTR_BUFSIZE bytes from file into buf, starting at ptr
			if (file->Seek(position, FS_SEEK_SET) < 0) {
				Sys_Error("fileDataPtr::readBuf: Seek failed %d %d %d length:%d\r\n", position, posStart, posEnd, this->file->Length());
			}
			numread = file->Read(buf, FILE_DATA_PTR_BUFSIZE);
Sys_Printf(">>> fileDataPtr::readBuf numread: %d\r\n", numread);
...
*/
			...
			posStart = position;
			posEnd = position + numread;
		}
		return buf[position - posStart];
	}
...
//printf(">>> fileDataPtr::operator*() %d %d\r\n", ptr, posStart);
	return readBuf(this->ptr);
}

char 			fileDataPtr::operator[](int & index) {
	return readBuf(this->ptr + index);
}
