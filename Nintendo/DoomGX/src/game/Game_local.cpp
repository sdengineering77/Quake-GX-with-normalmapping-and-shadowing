...
void idGameLocal::Init( void ) {
	...
	declManager->SetInsideLevelLoad();
...
Sys_Printf(">>> TRACE idGameLocal::LoadMap %s\r\n", mapName);
	// clear the sound system
	// DRS TODO: gameSoundWorld->ClearAllSoundEmitters();
Sys_PrintMainHeapInfo();

	InitAsyncNetwork();

Sys_Printf(">>> TRACE idGameLocal::LoadMap mapfile\r\n");
Sys_PrintMainHeapInfo();
...
Sys_Printf(">>> TRACE idGameLocal::LoadMap clear\r\n");
Sys_PrintMainHeapInfo();
	spawnedEntities.Clear();
	activeEntities.Clear();
	numEntitiesToDeactivate = 0;
...
Sys_Printf(">>> TRACE idGameLocal::LoadMap AAS\r\n");
Sys_PrintMainHeapInfo();
...
Sys_Printf(">>> TRACE idGameLocal::LoadMap smoke init\r\n");
Sys_PrintMainHeapInfo();
	smokeParticles->Init();

Sys_Printf(">>> TRACE idGameLocal::LoadMap find entitydef\r\n");
Sys_PrintMainHeapInfo();
...
Sys_Printf(">>> TRACE idGameLocal::LoadMap END\r\n");
Sys_PrintMainHeapInfo();
}
...
Sys_PrintMainHeapInfo();
_enable_memtrace = true; // DRS TODO: remove
...
Sys_PrintMainHeapInfo();
Sys_Printf(">>> TRACE idGameLocal::InitFromNewMap LoadMap\r\n");
	gamestate = GAMESTATE_STARTUP;
...
Sys_PrintMainHeapInfo();
Sys_Printf(">>> TRACE idGameLocal::InitFromNewMap InitScriptForMap\r\n");
	InitScriptForMap();

Sys_PrintMainHeapInfo();
Sys_Printf(">>> TRACE idGameLocal::InitFromNewMap MapPopulate\r\n");
	MapPopulate();
...
Sys_Printf(">>> idGameLocal::RunFrame\r\n");
#ifdef _DEBUG
	if ( isMultiplayer ) {
		assert( !isClient );
	}
...
bool idGameLocal::SpawnEntityDef( const idDict &args, idEntity **ent, bool setDefaults ) {
	...
	// DRS TODO: revert this method to 2 parm version
	idDeclEntityDef *def = (idDeclEntityDef *) FindEntityDef( classname, false, true );
...
================
*/
const idDeclEntityDef *idGameLocal::FindEntityDef( const char *name, bool makeDefault, bool parse ) const {
	const idDecl *decl = NULL;
	if ( isMultiplayer ) {
		if (parse) {
			decl = declManager->FindType( DECL_ENTITYDEF, va( "%s_mp", name ), false );
		} else {
			decl = declManager->FindDeclWithoutParsing( DECL_ENTITYDEF, va( "%s_mp", name ), makeDefault );
		}
	...
	if ( !decl ) {
		if (parse) {
			...
			decl = declManager->FindDeclWithoutParsing( DECL_ENTITYDEF, name, makeDefault );
		}
	}
...
//Sys_Printf(">>> idGameLocal::SpawnMapEntities CacheDictionaryMedia\r\n");
//			CacheDictionaryMedia( &args );

Sys_Printf(">>> idGameLocal::SpawnMapEntities SpawnEntityDef\r\n");
			SpawnEntityDef( args );
Sys_Printf(">>> idGameLocal::SpawnMapEntities Virtualize animations\r\n");
Sys_PrintMainHeapInfo();
			animationLib.VirtualizeAll(); // DRS
			renderModelManager->VirtualizeAll(); // DRS
			uiManager->VirtualizeAll();
			declManager->VirtualizeAll(); //DRS TODO: remove code
			collisionModelManager->virtualizeAll();
			num++;
		} else {
			inhibit++;
		}
