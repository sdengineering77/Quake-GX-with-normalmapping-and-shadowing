...
void idWeapon::GetWeaponDef( const char *objectname, int ammoinclip ) {
	...
	if ( shader && *shader ) {
// DRS TODO		sndHum = declManager->FindSound( shader );
// DRS TODO		StartSoundShader( sndHum, SND_CHANNEL_BODY, 0, false, NULL );
	}
...
void idWeapon::Event_Melee( void ) {
	...
	if ( !gameLocal.isClient ) {
		...
		if ( *hitSound != '\0' ) {
// DRS TODO			const idSoundShader *snd = declManager->FindSound( hitSound );
// DRS TODO			StartSoundShader( snd, SND_CHANNEL_BODY2, 0, true, NULL );
		}
