...
void idGameEdit::ParseSpawnArgsToRefSound( const idDict *args, refSound_t *refSound ) {
	...
	if ( temp[0] != '\0' ) {
// DRS TODO:		refSound->shader = declManager->FindSound( temp );
	}
}
...
// DRS TODO:	shader = declManager->FindSound( sound );
return true;// DRS TODO:	return StartSoundShader( shader, channel, soundShaderFlags, broadcast, length );
}
...
void idEntity::AddDamageEffect( const trace_t &collision, const idVec3 &velocity, const char *damageDefName ) {
	...
	if ( *sound != '\0' ) {
// DRS TODO:		StartSoundShader( declManager->FindSound( sound ), SND_CHANNEL_BODY, 0, false, NULL );
	}
	...
	if ( entityGui && cmds && *cmds ) {
		...
		while( 1 ) {
			...
			if ( token.Icmp("play") == 0 ) {
				if ( src.ReadToken( &token2 ) ) {
// DRS TODO:					const idSoundShader *shader = declManager->FindSound(token2);
// DRS TODO:					entityGui->StartSoundShader( shader, SND_CHANNEL_ANY, 0, false, NULL );
				}
				continue;
			}
...
// DRS TODO:	declManager->FindSound( soundName );
}
...
// DRS TODO:	StartSoundShader( declManager->FindSound( soundName ), (s_channelType)channel, 0, false, &length );
// DRS TODO:	idThread::ReturnFloat( MS2SEC( length ) );
}
...
void idEntity::Event_FadeSound( int channel, float to, float over ) {
	if ( refSound.referenceSound ) {
// DRS TODO:		refSound.referenceSound->FadeSound( channel, to, over );
	}
}
...
// DRS TODO:				shader = declManager->SoundByIndex( index, false );
				channel = (s_channelType)msg.ReadByte();
// DRS TODO:				StartSoundShader( shader, channel, 0, false, NULL );
			}
			return true;
		}
