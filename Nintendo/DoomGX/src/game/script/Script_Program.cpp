...
function_t::function_t() {
    ...
    Sys_DebugPrintf("function_t::function_t() size %d\r\n", sizeof(*this));
#endif
...
#ifdef TRACE_CONSTRUCTS
//    Sys_DebugPrintf("idTypeDef::idTypeDef( etype_t etype, idVarDef *edef, const char *ename, int esize, idTypeDef *aux ) size %d\r\n", sizeof(*this));
#endif
...
#ifdef TRACE_CONSTRUCTS
//    Sys_DebugPrintf("idTypeDef::idTypeDef( const idTypeDef &other ) size %d\r\n", sizeof(*this));
#endif
...
#ifdef DOOMGX
void * idTypeDef::operator new(std::size_t size) throw(std::bad_alloc) {
...
void idTypeDef::operator delete(void *pMem) throw() {
...
#ifdef TRACE_CONSTRUCTS
//    Sys_DebugPrintf("idVarDef::idVarDef( idTypeDef *typeptr ) size %d\r\n", sizeof(*this));
#endif
...
#ifdef DOOMGX
void * idVarDef::operator new(std::size_t size) throw(std::bad_alloc) {
	void *alloc = Mem_Alloc16(size);
	if (!alloc) {
		throw std::bad_alloc();
	}
...
void idVarDef::operator delete(void *pMem) throw() {
	if ((unsigned int) pMem >= 0x90000000) {
		Mem_Free16(pMem);
	} else {
		free(pMem);
...
idScriptObject::idScriptObject() {
#ifdef TRACE_CONSTRUCTS
    Sys_DebugPrintf("idScriptObject::idScriptObject() size %d\r\n", sizeof(*this));
#endif
...
Sys_Printf(">>> idProgram::CompileFile\r\n\r\n");
...
idProgram::idProgram() {
#ifdef TRACE_CONSTRUCTS
    Sys_DebugPrintf("idProgram::idProgram() size %d\r\n", sizeof(*this));
#endif
