...
idMD5Anim::idMD5Anim() {
    ...
    Sys_DebugPrintf("idMD5Anim::idMD5Anim() size %d\r\n", sizeof(*this));
#endif
    isVirtual   = true;
	ref_count	= 0;
	numFrames	= 0;
...
/*
====================
idMD5Anim::MakeVirtual
====================
*/
void idMD5Anim::MakeVirtual( void ) { // DRS
	this->isVirtual = true;
...
void idMD5Anim::EnsureLoaded( void ) {
	if (this->isVirtual) {
		this->Reload();
...
void idMD5Anim::Free( void ) {
    isVirtual   = true; // DRS: needs load when referenced, but its value is a 'don't care' at this point
	numFrames	= 0;
	numJoints	= 0;
	frameRate	= 24;
	animLength	= 0;
...
size_t idMD5Anim::Allocated( void ) const {
	((idMD5Anim * const) this)->EnsureLoaded();
...
bool idMD5Anim::LoadAnim( const char *filename ) {
	...
	Free();
	isVirtual = false; // DRS: gets loaded, so will be in mem
...
void idMD5Anim::GetOrigin( idVec3 &offset, int time, int cyclecount ) const {
	...
	((idMD5Anim * const) this)->EnsureLoaded(); // DRS absolute dirt
...
idAnimManager::idAnimManager() {
#ifdef TRACE_CONSTRUCTS
    Sys_DebugPrintf("idAnimManager::idAnimManager() size %d\r\n", sizeof(*this));
#endif
...
void idAnimManager::VirtualizeAll( void ) {
	idMD5Anim **animptr;
