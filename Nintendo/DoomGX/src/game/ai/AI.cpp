...
idMoveState::idMoveState() {
    ...
    Sys_DebugPrintf("idMoveState::idMoveState() size %d\r\n", sizeof(*this));
#endif
...
#ifdef TRACE_CONSTRUCTS
    Sys_DebugPrintf("idAASFindCover::idAASFindCover( const idVec3 &hideFromPos ) size %d\r\n", sizeof(*this));
#endif
...
#ifdef TRACE_CONSTRUCTS
    Sys_DebugPrintf("idAASFindAreaOutOfRange::idAASFindAreaOutOfRange( const idVec3 &targetPos, float maxDist ) size %d\r\n", sizeof(*this));
#endif
...
#ifdef TRACE_CONSTRUCTS
    Sys_DebugPrintf("idAASFindAttackPosition::idAASFindAttackPosition( const idAI *self, const idMat3 &gravityAxis, idEntity *target, const idVec3 &targetPos, const idVec3 &fireOffset ) size %d\r\n", sizeof(*this));
#endif
...
idAI::idAI() {
#ifdef TRACE_CONSTRUCTS
    Sys_DebugPrintf("idAI::idAI() size %d\r\n", sizeof(*this));
#endif
...
void idAI::Spawn( void ) {
	...
	idVec3				local_dir;
	bool				talks;
Sys_Printf(">>> TRACE_CLIPMODEL idAI::Spawn %p %s\r\n", this, this->GetName()); // DRS TODO: remove
Sys_PrintBacktrace();
...
Sys_Printf(">>> TRACE_CLIPMODEL idAI::Spawn set clipmodel %p %s\r\n", this, this->GetName());
...
// DRS TODO		const idSoundShader *shader = declManager->FindSound(meleeDef->GetString( "snd_miss" ));
// DRS TODO		StartSoundShader( shader, SND_CHANNEL_DAMAGE, 0, false, NULL );
		return;
	}
	...
	if ( p && *p ) {
// DRS TODO		shader = declManager->FindSound( p );
// DRS TODO		StartSoundShader( shader, SND_CHANNEL_DAMAGE, 0, false, NULL );
	}
...
*/
void idAI::Hide( void ) {
Sys_Printf(">>> EV_HIDE %s \r\n", this->GetName()); // DRS TODO: remove
	idActor::Hide();
	fl.takedamage = false;
Sys_Printf(">>> SetContents\r\n");
Sys_Printf(">>> physicsObj class %s\r\n", physicsObj.GetClassname());
Sys_Printf(">>> physicsObj getClipModel %p\r\n", physicsObj.GetClipModel(0));
	physicsObj.SetContents( 0 );
Sys_Printf(">>> Unlink\r\n");
	physicsObj.GetClipModel()->Unlink();
	StopSound( SND_CHANNEL_AMBIENT, false );
...
void idAI::SetChatSound( void ) {
	...
	if ( snd && *snd ) {
// DRS TODO		chat_snd = declManager->( snd );
...
#ifdef TRACE_CONSTRUCTS
    Sys_DebugPrintf("idCombatNode::idCombatNode( void ) size %d\r\n", sizeof(*this));
#endif
