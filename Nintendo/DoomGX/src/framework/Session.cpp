...
Sys_Printf(">>> TRACE Session_Map_f() calls StartNewGame\r\n");
...
Sys_Printf(">>> TRACE Session_DevMap_f() calls StartNewGame\r\n");
	sessLocal.StartNewGame( map, true );
}
...
void idSessionLocal::StartWipe( const char *_wipeMaterial, bool hold ) {
	console->Close();
Sys_Printf(">>> TRACE idSessionLocal::StartWipe %s %d\r\n", _wipeMaterial, hold);
...
================
*/
#include<ogc/video.h> // DRS TODO: temp hack, remove
void idSessionLocal::CompleteWipe() {
Sys_Printf(">>> TRACE idSessionLocal::CompleteWipe\r\n");
	if ( com_ticNumber == 0 ) {
...
#endif
		UpdateScreen( true );
		// DRS TODO: temp hack, remove
		com_ticNumber++;
		VIDEO_WaitVSync();
	}
Sys_Printf(">>> TRACE idSessionLocal::CompleteWipe END\r\n");
}
...
static void Session_Disconnect_f( const idCmdArgs &args ) {
	sessLocal.Stop();
	sessLocal.StartMenu();
// DRS TODO:	if ( soundSystem ) {
//		soundSystem->SetMute( false );
//	}
}

#if defined ID_DEMO_BUILD || defined DOOMGX
/*
================
Session_EndOfDemo_f
================
...
static void Session_EndOfDemo_f( const idCmdArgs &args ) {
	...
	sessLocal.StartMenu();
	if ( soundSystem ) {
// DRS TODO:		soundSystem->SetMute( false );
	}
...
//DRS TODO	sw->StartWritingDemo( writeDemo );
...
//DRS TODO	sw->StopAllSounds();
// DRS TODO:	soundSystem->SetPlayingSoundWorld( menuSoundWorld );
...
// DRS TODO:			soundSystem->SetMute( true );
...
void idSessionLocal::BeginAVICapture( const char *demoName ) {
	...
	aviDemoFrameCount = 0;
	aviTicStart = 0;
//DRS TODO	sw->AVIOpen( va( "demos/%s/", aviDemoShortName.c_str() ), aviDemoShortName.c_str() );
}
...
Sys_Printf(">>> TRACE idSessionLocal::StartNewGame %s %d\r\n", mapName, devmap);
...
Sys_Printf(">>> TRACE END idSessionLocal::StartNewGame\r\n");
...
Sys_Printf(">>> TRACE MoveToNewMap %s\r\n", mapName);
	ExecuteMapChange();
...
Sys_Printf(">>> TRACE idSessionLocal::UnloadMap\n");
...
Sys_Printf(">>> TRACE idSessionLocal::ExecuteMapChange\r\n");
...
Sys_Printf(">>> TRACE BeginLevelLoad\r\n");
Sys_PrintMainHeapInfo();
	if ( !reloadingSameMap ) {
		declManager->BeginLevelLoad();
		renderSystem->BeginLevelLoad();
// DRS TODO:		soundSystem->BeginLevelLoad();
	}

Sys_Printf(">>> TRACE uiManager->BeginLevelLoad\r\n");
Sys_PrintMainHeapInfo();
	uiManager->BeginLevelLoad();
Sys_Printf(">>> TRACE uiManager->Reload\r\n");
Sys_PrintMainHeapInfo();
//	uiManager->Reload( true ); // DRS TODO: will this work?

	// set the loading gui that we will wipe to
Sys_Printf(">>> TRACE LoadLoadingGui\r\n");
Sys_PrintMainHeapInfo();
...
Sys_Printf(">>> TRACE ShowLoadingGui\r\n");
Sys_PrintMainHeapInfo();
	ShowLoadingGui();
...
Sys_Printf(">>> TRACE !reloadingSameMap\r\n");
Sys_PrintMainHeapInfo();
	if ( !reloadingSameMap ) {
		renderSystem->EndLevelLoad();
// DRS TODO:		soundSystem->EndLevelLoad( mapString.c_str() );
		declManager->EndLevelLoad();
		SetBytesNeededForMapLoad( mapString.c_str(), fileSystem->GetReadCount() );
	}
// DRS moved up	uiManager->EndLevelLoad();
...
Sys_Printf(">>> TRACE guiLoading && bytesNeededForMapLoad\r\n");
Sys_PrintMainHeapInfo();
...
// DRS TODO:	soundSystem->SetPlayingSoundWorld( sw );
...
// DRS TODO:	soundSystem->SetMute( false );
	...
	mapSpawned = true;
	Sys_ClearEvents();
Sys_Printf(">>> TRACE END ExecuteMapChange\r\n");
Sys_PrintMainHeapInfo();
}
...
// DRS TODO:	idSoundWorld *sw = soundSystem->GetPlayingSoundWorld();
//	if ( sw ) {
...
// DRS TODO:	if ( Sys_GetDriveFreeSpace( cvarSystem->GetCVarString( "fs_savepath" ) ) < 25 ) {
...
// DRS TODO:	idSoundWorld *pauseWorld = soundSystem->GetPlayingSoundWorld();
//	if ( pauseWorld ) {
//		pauseWorld->Pause();
...
// DRS TODO:		if ( pauseWorld ) {
// DRS TODO:			soundSystem->SetPlayingSoundWorld( pauseWorld );
//			pauseWorld->UnPause();
//		}
		return false;
	}
...
void idSessionLocal::AdvanceRenderDemo( bool singleFrameOnly ) {
	...
	while( skipFrames > -1 ) {
		...
		if ( ds == DS_SOUND ) {
//DRS TODO			sw->ProcessDemoCommand( readDemo );
			continue;
		}
...
// DRS TODO menuSoundWorld = soundSystem->AllocSoundWorld( rw );
	...
	if (!guiMainMenu) { // DRS don't let the ID_DEMO_BUILD define decide what to load
		...
		sessLocal.isDemoVersion = true;
	} else {
		sessLocal.isDemoVersion = false;
	}
