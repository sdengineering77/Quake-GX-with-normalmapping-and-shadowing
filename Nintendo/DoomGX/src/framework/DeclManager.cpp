...
*/
extern bool _enable_memtrace; // DRS TODO: remove
...
public:
	...
	virtual void				VirtualizeAll( void );
	virtual void				BeginLevelLoad();
	virtual void 				SetInsideLevelLoad();
	...
	virtual const idDecl *		FindType( declType_t type, const char *name, bool makeDefault = true, bool keep = true );
	...
	// DRS .h + .cpp
	virtual const idMaterial *		FindMaterial( const char *name, bool makeDefault = true, bool keep = true);
...
//    Sys_DebugPrintf("idDeclFile::idDeclFile( const char *fileName, declType_t defaultType ) size %d\r\n", sizeof(*this));
#endif
...
idDeclFile::idDeclFile() {
#ifdef TRACE_CONSTRUCTS
//    Sys_DebugPrintf("idDeclFile::idDeclFile() size %d\r\n", sizeof(*this));
#endif
...
void * idDeclFile::operator new(std::size_t size) throw(std::bad_alloc) {
...
void idDeclFile::operator delete(void *pMem) throw() {
...
#ifdef DOOMGX
// DRS TODO: remove when everthing's in (or can it act as feature?)
			if (!newDecl) { // I commented sound, so not all decls can be processed...
				Mem_Free( buffer );
				return 0;
			}
#endif
...
// DRS TODO		if ( newDecl->textSource ) {
//			Mem_Free( newDecl->textSource );
//			newDecl->textSource = NULL;
//		}

		// DRS TODO newDecl->SetTextLocal( buffer + startMarker, size );
		newDecl->textSourceOffset = startMarker;
		newDecl->textSourceSize = size;
		newDecl->textLength = size;
		//
...
void idDeclManagerLocal::Init( void ) {
	...
	ClearHuffmanFrequencies();
#endif
	insideLevelLoad = false; // DRS added
...
/* DRS TODO RegisterDeclType( "sound",				DECL_SOUND,			idDeclAllocator<idSoundShader> );*/
...
// DRS TODO			if ( decl->textSource ) {
//				Mem_Free( decl->textSource );
//				decl->textSource = NULL;
//			}
			decl->textSourceSize = 0; // not necessary
			decl->textSourceOffset = 0;
...
void Sys_PrintMainHeapInfo(void); // DRS TODO: remove
void idDeclManagerLocal::VirtualizeAll( void ) {
	// DRS TODO: all decl types (selfs) must get a restore func
	int	num = linearLists[DECL_MATERIAL].Num();
	for ( int j = 0 ; j < num ; j++ ) {
		idDeclLocal *decl = linearLists[DECL_MATERIAL][j];
		if (decl->self) {
			decl->self->FreeData();
			decl->Invalidate();
		}
...
void idDeclManagerLocal::BeginLevelLoad() {
	// insideLevelLoad = true; DRS removed
Sys_Printf(">>> idDeclManagerLocal::BeginLevelLoad\r\n");
Sys_PrintMainHeapInfo();
...
Sys_PrintMainHeapInfo();
Sys_Printf(">>> END idDeclManagerLocal::BeginLevelLoad\r\n");

}

void idDeclManagerLocal::SetInsideLevelLoad() {
...
Sys_Printf("TODO: check if fileslist is really cleaned up, including names from Sys_ListFiles\r\n");
}
...
=================
*/
void Sys_PrintBacktrace(void); // DRS TODO: remove
const idDecl *idDeclManagerLocal::FindType( declType_t type, const char *name, bool makeDefault, bool keep ) {
	idDeclLocal *decl;
//Sys_Printf(">>> const idDecl *idDeclManagerLocal::FindType( declType_t type, const char *name, bool makeDefault )\r\n");
//if (name && strcmp("models/mapobjects/shutter/shutter_small.lwo", name) == 0) {
//	Sys_PrintBacktrace();
//	_enable_memtrace = true;
//}
	if ( !name || !name[0] ) {
		name = "_emptyName";
		Sys_Printf( "idDeclManager::FindType: empty %s name", GetDeclType( (int)type )->typeName.c_str() );
	}
...
//Sys_Printf(">>> END const idDecl *idDeclManagerLocal::FindType( declType_t type, const char *name, bool makeDefault )\r\n");
	return decl->self;
}
...
const idDecl* idDeclManagerLocal::FindDeclWithoutParsing( declType_t type, const char *name, bool makeDefault) {
	idDeclLocal* decl;
//Sys_Printf(">>> const idDecl* idDeclManagerLocal::FindDeclWithoutParsing( declType_t type, const char *name, bool makeDefault)\r\n");
	decl = FindTypeWithoutParsing(type, name, makeDefault);
	if(decl) {
		return decl->self;
	}
...
void idDeclManagerLocal::PrintType( const idCmdArgs &args, declType_t type ) {
	...
	common->Printf( "----------\n" );
	if ( decl->textLength > 0) {//decl->textSource != NULL ) {
...
idDecl *idDeclManagerLocal::CreateNewDecl( declType_t type, const char *name, const char *_fileName ) {
	...
	for ( i = hashTables[typeIndex].First( hash ); i >= 0; i = hashTables[typeIndex].Next( i ) ) {
		if ( linearLists[typeIndex][i]->name.Icmp( canonicalName ) == 0 ) {
			...
			// DRS make sure its loaded
			if (linearLists[typeIndex][i]->declState == DS_UNPARSED) {
Sys_Printf(">>> idDeclManager::CreateNewDecl reparse decl\r\n");
// DRS TODO: remove				linearLists[typeIndex][i]->ParseLocal();
			}
...
const idMaterial *idDeclManagerLocal::FindMaterial( const char *name, bool makeDefault, bool keep ) {
	return static_cast<const idMaterial *>( FindType( DECL_MATERIAL, name, makeDefault, keep ) );
}
...
idDeclLocal *idDeclManagerLocal::FindTypeWithoutParsing( declType_t type, const char *name, bool makeDefault ) {
	...
	Sys_Printf( ">>> idDeclManagerLocal::FindTypeWithoutParsing(%d, %s, %d)\r\n", typeIndex, name, makeDefault);

	if ( typeIndex < 0 || typeIndex >= declTypes.Num() || declTypes[typeIndex] == NULL ) {
		// DRS TODO common->FatalError( "idDeclManager::FindTypeWithoutParsing: bad type: %i", typeIndex );
		Sys_Printf( ">>> WARNING TODO: idDeclManager::FindTypeWithoutParsing: bad type: %i %i %s %i\r\n", typeIndex, declTypes.Num(), name, makeDefault);
		return NULL;
	}
	...
	decl->type = type;
	decl->declState = DS_UNPARSED;
// DRS TODO	decl->textSource = NULL;
	decl->textSourceSize = 0;
	decl->textSourceOffset = 0;
...
#ifdef TRACE_CONSTRUCTS
//    Sys_DebugPrintf("idDeclLocal::idDeclLocal( void ) size %d\r\n", sizeof(*this));
#endif

	name = "unnamed";
// DRS TODO	textSource = NULL;
	textSourceSize = 0;
	textSourceOffset = 0;
...
void * idDeclLocal::operator new(std::size_t size) throw(std::bad_alloc) {
	void *alloc = Mem_Alloc16(size);
	if (!alloc) {
		throw std::bad_alloc();
	}
...
void idDeclLocal::operator delete(void *pMem) throw() {
	if ((unsigned int) pMem >= 0x90000000) {
		Mem_Free16(pMem);
	} else {
		free(pMem);
...
void idDeclLocal::GetText( char *text ) const {
	...
	if (textSourceSize == 0) {
		byte *textSource; // DRS for data buffer

		sprintf(filename, "%s.textSource", this->name.c_str());
		if (fileSystem->ReadFile(filename, (void **) &textSource, NULL) < 0) {
			Sys_Error("idDeclLocal::GetText: Couldn't open file: %s\r\n", filename);
		}
		...
		text[textLength] = '\0';
#endif
		// DRS Free temp buffer
		if (textSource) {
			Mem_Free(textSource);
		}
	} else {
		byte *declfile;
		if (fileSystem->ReadFile(this->sourceFile->fileName, (void **) &declfile, NULL) < 0) {
			Sys_Error("idDeclLocal::GetText: Couldn't open file: %s\r\n", filename);
		}
		memcpy( text, declfile + textSourceOffset, textSourceSize );
		text[textLength] = '\0';
		// DRS Free temp buffer
		if (declfile) {
			Mem_Free(declfile);
...
// DRS TODO	Mem_Free( textSource );
	// DRS We'll store them on disk to save memory
	char filename[1024];
	sprintf(filename, "%s.textSource", this->name.c_str());
...
// DRS TODO	textSource = (char *)Mem_Alloc( compressedLength );
//	memcpy( textSource, compressed, compressedLength );
	fileSystem->WriteFile(filename, compressed, compressedLength);

#else
	compressedLength = length;
// DRS TODO	textSource = (char *) Mem_Alloc( length + 1 );
// 	memcpy( textSource, text, length );
//	textSource[length] = '\0';
	// DRS TODO: test
	fileSystem->WriteFile(filename, text, length);

#endif
	textLength = length;

// DRS TODO
	textSourceOffset = 0; // DRS must be 0 since it is on disk now
	textSourceSize = 0;
}
...
bool idDeclLocal::ReplaceSourceFileText( void ) {
	...
	if ( sourceFile->fileSize ) {
		...
		if ( MD5_BlockChecksum( buffer, oldFileLength ) != sourceFile->checksum ) { // DRS TODO: warning sourceFile->checksum always unsigned?
			Mem_Free( buffer );
			common->Warning( "The file %s has been modified outside of the engine.", GetFileName() );
			return false;
		}
...
Sys_Printf("parsing %s %s\n", declManagerLocal.declTypes[type]->typeName.c_str(), name.c_str());
	// if no text source try to generate default text
	if ( textLength == 0) {//textSource == NULL ) {
		// DRS this eventually calls setText, which is then stored on disk
		// therefor no need to free the data below
	...
	// free generated text
	if ( generatedDefaultText ) {//|| parsedOutsideLevelLoad) { // DRS TODO: does this work?
		//if (parsedOutsideLevelLoad) {
		//	Sys_Printf("DRS TODO: delete textSource %p\r\n", textSource);
		//}
// DRS TODO		Mem_Free( textSource );
//		textSource = 0;
		textLength = 0;
...
//Sys_Printf(">>> idDeclLocal::Purge DRS TODO: shouldn't this free the data?\r\n");
	if ( parsedOutsideLevelLoad ) {
		return;
	}
