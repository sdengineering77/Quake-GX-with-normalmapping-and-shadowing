...
idGuiModel::idGuiModel() {
    ...
    Sys_DebugPrintf("idGuiModel::idGuiModel() size %d\r\n", sizeof(*this));
#endif
...
void idGuiModel::EmitSurface( guiModelSurface_t *surf, float modelMatrix[16], float modelViewMatrix[16], bool depthHack ) {
	srfTriangles_t	*tri;
//Sys_Printf(">>> idGuiModel::EmitSurface vtx %d ind %d\r\n", surf->numVerts, surf->numIndexes);
	...
	// DRS all allocs must be 32B aligned
...
//Sys_Printf(">>> tri->indexes %p\r\n", tri->indexes);
//for(int i=0; i<tri->numIndexes; i++) {
//	Sys_Printf(">>> tri->indexes %d\r\n", tri->indexes[i]);
//}
...
//Sys_Printf(">>> tri->verts %p\r\n", tri->verts);
//for(int i=0; i<tri->numVerts; i++) {
//	Sys_Printf(">>> tri->verts x %f y %f z %f x %f y %f\r\n", tri->verts[i].xyz.x, tri->verts[i].xyz.y, tri->verts[i].xyz.z, tri->verts[i].st.x, tri->verts[i].st.y);
//}
...
void idGuiModel::EmitFullScreen( void ) {
	viewDef_t	*viewDef;
Sys_Printf(">>> idGuiModel::EmitFullScreen\r\n");
...
//Sys_Printf(">>> idGuiModel::DrawStretchPic( const idDrawVert *dverts, const glIndex_t *dindexes, int vertCount, int indexCount, const idMaterial *hShader, bool clip, float min_x, float min_y, float max_x, float max_y )\r\n");
	if ( !glConfig.isInitialized ) {
		return;
	}
...
//Sys_Printf(">>> clip vertcount %d, indexcnt %d\r\n", vertCount, indexCount);
...
//Sys_Printf(">>> clip vtx x %f y %f z %f s %f t %f\r\n", dv->xyz.x, dv->xyz.y, dv->xyz.z, dv->st.x, dv->st.y);
			}
			...
			for ( j = 2; j < w.GetNumPoints(); j++ ) {
				...
				surf->numIndexes += 3;
//Sys_Printf(">>> clip ind %d\r\n", numVerts - surf->firstVert);
//Sys_Printf(">>> clip ind %d\r\n", numVerts + j - 1 - surf->firstVert);
//Sys_Printf(">>> clip ind %d\r\n", numVerts + j - surf->firstVert);
			}
		}
...
//			idDrawVert *dv = (idDrawVert *) &dverts[i];
//			Sys_Printf(">>> !clip vtx x %f y %f z %f s %f t %f\r\n", dv->xyz.x, dv->xyz.y, dv->xyz.z, dv->st.x, dv->st.y);
//		}
//		for (int i=0; i<indexCount; i++) {
//			Sys_Printf(">>> !clip ind %d\r\n", dindexes[i]);
