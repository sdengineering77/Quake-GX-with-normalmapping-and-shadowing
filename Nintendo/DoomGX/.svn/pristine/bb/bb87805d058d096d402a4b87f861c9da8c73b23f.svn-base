...
idGuiScript::idGuiScript() {
    ...
    Sys_DebugPrintf("idGuiScript::idGuiScript() size %d\r\n", sizeof(*this));
#endif
...
void * idGuiScript::operator new(std::size_t size) throw(std::bad_alloc) {
...
void idGuiScript::operator delete(void *pMem) throw() {
...
void * idGSWinVar::operator new(std::size_t size) throw(std::bad_alloc) {
	void *alloc = Mem_Alloc16(size);
	if (!alloc) {
		throw std::bad_alloc();
	}
...
void idGSWinVar::operator delete(void *pMem) throw() {
	if ((unsigned int) pMem >= 0x90000000) {
		Mem_Free16(pMem);
	} else {
		free(pMem);
...
// DRS TODO:				declManager->FindSound( token.c_str() );
						}
					}
				}
			}
