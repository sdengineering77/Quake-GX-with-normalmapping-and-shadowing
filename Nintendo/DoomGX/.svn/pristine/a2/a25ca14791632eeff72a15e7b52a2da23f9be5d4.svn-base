...
idPlayer::idPlayer() {
    ...
    Sys_DebugPrintf("idPlayer::idPlayer() size %d\r\n", sizeof(*this));
#endif
...
// DRS TODO	declManager->FindSound( "player_sounds_hitArmor" );
...
Sys_Printf("TODO Player.cpp 2933 item->GetAttributes\r\n");
// DRS TODO	item->GetAttributes( attr );
...
// DRS TODO						StartSoundShader( declManager->FindSound( sound ), SND_CHANNEL_DEMONIC, 0, false, NULL );
				}
				if ( baseSkinName.Length() ) {
					powerUpSkin = declManager->FindSkin( baseSkinName + "_berserk" );
				}
...
// DRS TODO						StartSoundShader( declManager->FindSound( sound ), SND_CHANNEL_ANY, 0, false, NULL );
				}
				break;
			}
			case ADRENALINE: {
...
// DRS TODO						const idSoundShader *shader = declManager->FindSound( pdaVideoWave );
// DRS TODO						StartSoundShader( shader, SND_CHANNEL_PDA, 0, false, NULL );
				}
			}
		}
	}
...
// DRS TODO				const idSoundShader *shader = declManager->FindSound( pdaAudio );
			int ms;
// DRS TODO				StartSoundShader( shader, SND_CHANNEL_PDA, 0, false, &ms );
			StartAudioLog();
	...
	if ( token.Icmp( "stoppdaaudio" ) == 0 ) {
		if ( objectiveSystem && objectiveSystemOpen && pdaAudio.Length() > 0 ) {
			// ID commented (???) idSoundShader *shader = declManager->FindSound( pdaAudio );
			StopAudioLog();
			StopSound( SND_CHANNEL_PDA, false );
		}
		return true;
