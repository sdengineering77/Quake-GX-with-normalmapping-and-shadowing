...
#else
#define	MAX_SHADOW_INDEXES		0xB000
#define	MAX_SHADOW_VERTS		0xB000
#endif
...
void Mem_InitShadowArrays(void) {
	shadowIndexes = (glIndex_t *) Mem_Alloc32(sizeof(glIndex_t) * MAX_SHADOW_INDEXES);
	shadowVerts = (idVec4 *) Mem_Alloc32(sizeof(idVec4) * MAX_SHADOW_VERTS);
}
...
#else
		// DRS TODO: perhaps we *could* calc these in HW T&L: vertex passed as normal, subtract from light,
		// normalize and multiply by radius. Would use about 10 GPU cycles per position excluding texture
		// export. Doubt that it ends up faster than doing it on CPU instead
		return R_CreateTurboShadowVolume( ent, tri, light, cullInfo );
#endif
