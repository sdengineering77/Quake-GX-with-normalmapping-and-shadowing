...
// DRS TODO				menuSoundWorld->PlayShaderDirectly(snd);
			}
			continue;
		}
	}
...
void idSessionLocal::HandleMainMenuCommands( const char *menuCommand ) {
	...
	for( icmd = 0; icmd < args.Argc(); ) {
		const char *cmd = args.Argv( icmd++ );
Sys_Printf(">>> TRACE idSessionLocal::HandleMainMenuCommands() cmd %s \r\n", cmd);
		...
		if ( !idStr::Icmp( cmd, "startGame" ) ) {
			...
			if ( icmd < args.Argc() ) {
Sys_Printf(">>> TRACE idSessionLocal::HandleMainMenuCommands() calls StartNewGame icmd < args.Argc() path\r\n");
				StartNewGame( args.Argv( icmd++ ) );
			} else {
Sys_Printf(">>> TRACE idSessionLocal::HandleMainMenuCommands() calls StartNewGame icmd >= args.Argc() path\r\n");
				if (!sessLocal.isDemoVersion) {
...
// DRS TODO				menuSoundWorld->PlayShaderDirectly( snd, channel );
		...
		if ( !idStr::Icmp( cmd, "music" ) ) {
			if ( args.Argc() - icmd >= 1 ) {
				idStr snd = args.Argv( icmd++ );
// DRS TODO				menuSoundWorld->PlayShaderDirectly( snd, 2 );
			}
			continue;
		}
