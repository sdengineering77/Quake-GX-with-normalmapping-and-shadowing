...
Sys_Printf("DRS TODO RB_PrepareStageTexturing generate texcoords\r\n");
// DRS TODO: this should generate texcoords by vertex position
// 		GX_LoadTexMtxImm(texmtx, GX_TEXMTX0, GX_MTX3x4);
//		GX_SetTexCoordGen(GX_TG_TEXCOORD0, GX_TG_MTX3x4, GX_TG_POS, GX_TEXMTX0);
...
//Sys_Printf(">>> void RB_FinishStageTexturing( const shaderStage_t *pStage, const drawSurf_t *surf, idDrawVert *ac )\r\n");
	...
	if ( pStage->texture.hasMatrix ) {
		...
		qglMatrixMode( GL_MODELVIEW );
#else
		qgxLoadTexMatrixIdentity(GX_TEXMTX0); // DRS TODO: this only works if it is meant for the stage that takes this MTX
#endif
	}
...
//Sys_Printf(">>> RB_T_FillDepthBuffer TODO set cheap shader, no textures\r\n");
...
#ifndef DOOMGX
	// DRS TODO: not having this can cause several issues
	// DRS TODO: disable it again below
	...
	qgxSetVertexPointer(GX_VA_TEX0, reinterpret_cast<void *>(&ac->st), sizeof( idDrawVert ));
#endif
...
#else
			qgxSetKColor0( color, 4 );
			qgxAlphaFunc( GL_GREATER, regs[ pStage->alphaTestRegister ] );
#endif
			// bind the texture
	...
	if ( drawSolid ) {
		...
		qgxSetKColor0( color, 4 );
#endif
		globalImages->whiteImage->Bind(); // DRS TODO: just disable texture stage if it isfaster
...
//Sys_Printf(">>> RB_STD_FillDepthBuffer( drawSurf_t **drawSurfs, int numDrawSurfs )\r\n");
...
//Sys_Printf(">>> void RB_STD_T_RenderShaderPasses( const drawSurf_t *surf )\r\n");
	...
	// set polygon offset if necessary
	// DRS TODO: remove trace
	if ( shader->TestMaterialFlag(MF_POLYGONOFFSET) ) {
		Sys_Printf(">>> !!!! RB_STD_T_RenderShaderPasses POLYGON OFFSET %f %f!!!!\r\n", r_offsetFactor.GetFloat(), r_offsetUnits.GetFloat() * shader->GetPolygonOffset());
	}
#ifndef DOOMGX
	if ( shader->TestMaterialFlag(MF_POLYGONOFFSET) ) {
		qglEnable( GL_POLYGON_OFFSET_FILL );
...
//Sys_Printf(">>> idDrawVert %p\r\n", ac);
	...
	for ( stage = 0; stage < shader->GetNumStages() ; stage++ ) {
		...
		if ( newStage ) {
Sys_Printf(">>> !!!! RB_STD_T_RenderShaderPasses USES VERTEX COLORS newstyle!!!!\r\n");
			//--------------------------
			//
			// new style stages
			//
		...
		if ( pStage->vertexColor == SVC_IGNORE ) {
			...
			#ifdef DOOMGX
			qgxSetKColor0( color, 4 );
// DRS TODO: qglColorPointer( 4, GL_UNSIGNED_BYTE, sizeof( idDrawVert ), (void *)&ac->color );
//			if ( color[0] != 1 || color[1] != 1 || color[2] != 1 || color[3] != 1 ) {
// DRS TODO: this should set a different shader so that vertex color is multiplied
//			it also requires the color array
//			}
			Sys_Printf("TODO draw_common.cpp VERTEX COLORS\r\n");
			#else
			qglColorPointer( 4, GL_UNSIGNED_BYTE, sizeof( idDrawVert ), (void *)&ac->color );
			qglEnableClientState( GL_COLOR_ARRAY );
...
void RB_STD_FogAllLights( void ) {
    // DRS TODO: Sys_Printf("void RB_STD_FogAllLights( void )\r\n");
}
...
void RB_STD_LightScale( void ) {
    // DRS TODO: Sys_Printf("void RB_STD_LightScale( void )\r\n");
}
...
//Sys_Printf("DRS TODO: RB_STD_DrawShaderPasses\r\n");
