...
#else
			qgxLoadMatrixf(drawSurf->space->modelViewMatrix);
#endif
		}
...
#else
	qgxLoadTexMatrixf( GX_TEXMTX0, matrix );
#endif
...
void RB_BeginDrawingView (void) {
	...
	qgxLoadProjMatrixf(backEnd.viewDef->projectionMatrix, backEnd.viewDef->isPerspective);
	qgxMatrixMode(GL_MODELVIEW);

	qgxViewport( tr.viewportOffset[0] + backEnd.viewDef->viewport.x1,
		tr.viewportOffset[1] + backEnd.viewDef->viewport.y1,
		backEnd.viewDef->viewport.x2 + 1 - backEnd.viewDef->viewport.x1,
		backEnd.viewDef->viewport.y2 + 1 - backEnd.viewDef->viewport.y1 );

	qgxScissor( tr.viewportOffset[0] + backEnd.viewDef->viewport.x1 + backEnd.viewDef->scissor.x1,
...
//Sys_Printf(">>> RB_SubmittInteraction\r\n");
	if ( !din->bumpImage ) {
		return;
	}
...
//Sys_Printf(">>> void RB_CreateSingleDrawInteractions( const drawSurf_t *surf, void (*DrawInteraction)(const drawInteraction_t *) )\r\n");
