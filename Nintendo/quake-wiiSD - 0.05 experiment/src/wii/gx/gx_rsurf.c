...
//		GX_LoadTexMtxImm(modelViewInv, GX_TEXMTX4, GX_MTX3x4);
		...
		GX_SetTevColorOp(tevstage, GX_TEV_ADD, GX_TB_ZERO, GX_CS_SCALE_1, GX_TRUE, GX_TEVPREV);
		GX_SetTevOrder(tevstage, GX_TEXCOORD3, GX_TEXMAP4, GX_COLOR0A0);
		tevstage++;
	
		// shadow stencil tex
		if (currLight->stenciltexnum>-1) {
			GXColor background = {0x80, 0x00, 0x00, 0xff}; // DRS "globalize"=)
			GX_SetTevKColor(GX_TEV_KCSEL_K0, background);
			GX_SetTevKColorSel(GX_TEVSTAGE0, GX_TEV_KCSEL_K0);	

			GX_SetTevSwapMode(tevstage, GX_TEV_SWAP0, GX_TEV_SWAP0);
			GX_SetTevColorIn(tevstage, GX_CC_TEXC, GX_CC_KONST, GX_CC_CPREV, GX_CC_ZERO);
			GX_SetTevColorOp(tevstage, GX_TEV_COMP_R8_EQ, GX_TB_ZERO, GX_CS_SCALE_1, GX_FALSE, GX_TEVPREV);
...
//	GX_InitLightAttnA(&cl_hwlight_obj[0], 0.05f * currLight->radius, 0.0f, 0.0f);
	...
	GX_InitLightAttnA(&cl_hwlight_obj[0], 0.02f * currLight->radius, 0.0f, 0.0f);
