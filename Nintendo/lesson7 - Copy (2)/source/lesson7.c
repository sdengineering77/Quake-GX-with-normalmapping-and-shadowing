...
Vector setlight(Mtx view,f32 theta,f32 phi,f32 dist, GXColor litcol, GXColor ambcol,GXColor matcol)
{
	...
	//GX_InitLightSpot(&lobj,90.0f,GX_SP_COS2);
	//GX_InitLightAttn(&lobj,1,1,1,1.0f,1.0f,1.0f);	
	//GX_InitLightDistAttn(&lobj,150.0f,10.1f,GX_DA_GENTLE);
...
//		dir+= 0.05f;
		...
		//GX_SetChanCtrl(GX_COLORNULL,GX_DISABLE,GX_SRC_REG,GX_SRC_REG,GX_LIGHT7,GX_DF_NONE,GX_AF_NONE);
		...
		// text coord 2 renders the bump offset since it uses coord 1 as origin
		GX_SetTexCoordGen(GX_TEXCOORD3, GX_TG_BUMP7, GX_TG_TEXCOORD2, GX_IDENTITY);
...
//			{  0.0f,   0.0f, -0.25f}  // G -> vertical coordinate (t)
			{  0.0f, -0.5f,   0.0f}, // B -> horizontal coordinate (s)
			{  0.0f,   0.0f, -0.5f}  // G -> vertical coordinate (t)
		};
		...
		GX_SetTevIndirect(
			GX_TEVSTAGE0, // tevstage 0 is used to combine normal map and lightmap
		...
		// setup tev stage 0 to lookup the lightmap
		...
		GX_SetTevColorIn(GX_TEVSTAGE0, GX_CC_RASC, GX_CC_ZERO, GX_CC_TEXC, GX_CC_ZERO);
		GX_SetTevColorOp(GX_TEVSTAGE0, GX_TEV_COMP_A8_GT, GX_TB_ZERO, GX_CS_DIVIDE_2, GX_TC_LINEAR, GX_TEVPREV);
		GX_SetTevOrder(GX_TEVSTAGE0, GX_TEXCOORD3, GX_TEXMAP2, GX_COLOR0A0);
//		GX_SetTevOrder(GX_TEVSTAGE0, GX_TEXCOORD2, GX_TEXMAP1, GX_COLOR0A0);

		// setup tev stage 1 to add normal mapped diffuse light
		GX_SetTevIndirect(
		...
		GX_SetTevColorIn(GX_TEVSTAGE1, GX_CC_ZERO, GX_CC_HALF, GX_CC_TEXC, GX_CC_CPREV);
		GX_SetTevColorOp(GX_TEVSTAGE1, GX_TEV_ADD, GX_TB_ZERO, GX_CS_DIVIDE_2, GX_TC_LINEAR, GX_TEVPREV);
