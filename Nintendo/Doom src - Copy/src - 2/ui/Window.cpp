...
#ifdef TRACE_CONSTRUCTS
    Sys_DebugPrintf("idWindow::idWindow(idDeviceContext *d, idUserInterfaceLocal *ui) size %d\r\n", sizeof(*this));
#endif
...
Sys_Printf(">>> New idGuiScript\r\n");
		idGuiScript *gs = new idGuiScript();
Sys_Printf(">>> New idGuiScript done\r\n");
		if (token.Icmp("if") == 0) {
Sys_Printf(">>> 1\r\n");
...
Sys_Printf(">>> TRACE 2....\r\n");
	...
	while( token != "}" ) {
Sys_Printf(">>> TRACE LOOP\r\n");
//Sys_Printf(">>> TRACE LOOP token = %c\r\n", token);
		// track what was parsed so we can maintain it for the guieditor
		src->SetMarker ( );

Sys_Printf(">>> FIRST TOKEN CHECK\r\n");
...
Sys_Printf(">>> TRACE END LOOP \r\n");

	if (ret) {
		EvalRegs(-1, true);
	}

Sys_Printf(">>> TRACE LOOP 4\r\n", token);
	SetupFromState();
	PostParse();
Sys_Printf(">>> TRACE LOOP 5\r\n", token);
