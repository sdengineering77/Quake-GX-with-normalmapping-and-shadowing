...
//    Sys_DebugPrintf("idInternalCVar::idInternalCVar( void ) size %d\r\n", sizeof(*this));
#endif
...
#ifdef TRACE_CONSTRUCTS
//    Sys_DebugPrintf("idInternalCVar::idInternalCVar( const char *newName, const char *newValue, int newFlags ) size %d\r\n", sizeof(*this));
#endif
...
#ifdef TRACE_CONSTRUCTS
//    Sys_DebugPrintf("idInternalCVar::idInternalCVar( const idCVar *cvar ) size %d\r\n", sizeof(*this));
#endif
...
// DRS TODO CONSTRUCTOR idCVarSystemLocal			localCVarSystem;
idCVarSystem *				cvarSystem = NULL;//&localCVarSystem;
...
#ifdef TRACE_CONSTRUCTS
    Sys_DebugPrintf("idCVarSystemLocal::idCVarSystemLocal( void ) size %d\r\n", sizeof(*this));
#endif
...
void idCVarSystemLocal::Toggle_f( const idCmdArgs &args ) {
	...
	idInternalCVar *cvar = ((idCVarSystemLocal *) cvarSystem)->FindInternal( (const char*) args.Argv( 1 ) );
...
void idCVarSystemLocal::Set_f( const idCmdArgs &args ) {
	...
	((idCVarSystemLocal *) cvarSystem)->SetCVarString( args.Argv(1), str );
}
...
void idCVarSystemLocal::SetS_f( const idCmdArgs &args ) {
	...
	Set_f( args );
	cvar = ((idCVarSystemLocal *) cvarSystem)->FindInternal( args.Argv( 1 ) );
	if ( !cvar ) {
		return;
	}
...
void idCVarSystemLocal::ListByFlags( const idCmdArgs &args, cvarFlags_t flags ) {
	...
	for ( i = 0; i < ((idCVarSystemLocal *) cvarSystem)->cvars.Num(); i++ ) {
		cvar = ((idCVarSystemLocal *) cvarSystem)->cvars[i];
...
void idCVarSystemLocal::Restart_f( const idCmdArgs &args ) {
	...
	for ( i = 0; i < ((idCVarSystemLocal *) cvarSystem)->cvars.Num(); i++ ) {
		...
		if ( !( cvar->flags & CVAR_STATIC ) ) {
			hash = ((idCVarSystemLocal *) cvarSystem)->cvarHash.GenerateKey( cvar->nameString, false );
			delete cvar;
			((idCVarSystemLocal *) cvarSystem)->cvars.RemoveIndex( i );
			((idCVarSystemLocal *) cvarSystem)->cvarHash.RemoveIndex( hash, i );
			i--;
			continue;
		}
