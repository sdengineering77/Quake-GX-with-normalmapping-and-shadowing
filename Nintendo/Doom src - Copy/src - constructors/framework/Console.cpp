...
// DRS TODO CONSTRUCTOR static idConsoleLocal localConsole;
idConsole	*console = NULL;//&localConsole;
...
void idConsoleLocal::DrawInput() {
	...
	renderSystem->DrawSmallChar( 1 * SMALLCHAR_WIDTH, y, ']', ((idConsoleLocal *) console)->charSetShader );
...
void idConsoleLocal::DrawNotify() {
	...
	for ( i = current-NUM_CON_TIMES+1; i <= current; i++ ) {
		...
		for ( x = 0; x < LINE_WIDTH; x++ ) {
			...
			renderSystem->DrawSmallChar( (x+1)*SMALLCHAR_WIDTH, v, text_p[x] & 0xff, ((idConsoleLocal *) console)->charSetShader );
		}
...
void idConsoleLocal::DrawSolidConsole( float frac ) {
	...
	for ( x = 0; x < i; x++ ) {
		renderSystem->DrawSmallChar( SCREEN_WIDTH - ( i - x ) * SMALLCHAR_WIDTH,
			(lines-(SMALLCHAR_HEIGHT+SMALLCHAR_HEIGHT/2)), version[x], ((idConsoleLocal *) console)->charSetShader );
	...
	if ( display != current ) {
		...
		for ( x = 0; x < LINE_WIDTH; x += 4 ) {
			renderSystem->DrawSmallChar( (x+1)*SMALLCHAR_WIDTH, idMath::FtoiFast( y ), '^', ((idConsoleLocal *) console)->charSetShader );
		}
		y -= SMALLCHAR_HEIGHT;
		rows--;
	}
	...
	for ( i = 0; i < rows; i++, y -= SMALLCHAR_HEIGHT, row-- ) {
		...
		for ( x = 0; x < LINE_WIDTH; x++ ) {
			...
			renderSystem->DrawSmallChar( (x+1)*SMALLCHAR_WIDTH, idMath::FtoiFast( y ), text_p[x] & 0xff, ((idConsoleLocal *) console)->charSetShader );
		}
	}
