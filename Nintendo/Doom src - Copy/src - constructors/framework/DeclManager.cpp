...
// DRS TODO CONSTRUCTOR idDeclManagerLocal	declManagerLocal;
idDeclManager *		declManager = NULL;//&declManagerLocal;
...
#undef DOOMGX
#ifdef DOOMGX
Sys_DebugPrintf(">>> 0 Allocate...\r\n");
// DRS TODO the original code throws exception when the 6th huffman node is allocated. Going this way works, but
// the reason for failing must still be investigated...
huffmanNode_t *list1 = (huffmanNode_t *) new huffmanNode_t[MAX_HUFFMAN_SYMBOLS];//malloc(MAX_HUFFMAN_SYMBOLS * sizeof(huffmanNode_t));
huffmanNode_t *list2 = (huffmanNode_t *) new huffmanNode_t[MAX_HUFFMAN_SYMBOLS];//malloc(MAX_HUFFMAN_SYMBOLS * sizeof(huffmanNode_t));
Sys_DebugPrintf(">>> 0 END Allocate... %d %d\r\n", sizeof(list1), MAX_HUFFMAN_SYMBOLS * sizeof(huffmanNode_t));
#endif
...
Sys_Printf(">>> int idDeclFile::LoadAndParse()\r\n");
	...
	while( 1 ) {
		...
		numTypes = ((idDeclManagerLocal *) declManager)->GetNumDeclTypes();
		for ( i = 0; i < numTypes; i++ ) {
			idDeclType *typeInfo = ((idDeclManagerLocal *) declManager)->GetDeclType( i );
		...
		reparse = false;
		newDecl = ((idDeclManagerLocal *) declManager)->FindTypeWithoutParsing( identifiedType, name, false );
		if ( newDecl ) {
			// update the existing copy
			if ( newDecl->sourceFile != this || newDecl->redefinedInReload ) {
				src.Warning( "%s '%s' previously defined at %s:%i", ((idDeclManagerLocal *) declManager)->GetDeclNameFromType( identifiedType ),
			...
			newDecl = ((idDeclManagerLocal *) declManager)->FindTypeWithoutParsing( identifiedType, name, true );
#ifdef DOOMGX
...
void idDeclManagerLocal::ListDecls_f( const idCmdArgs &args ) {
	...
	for ( i = 0; i < ((idDeclManagerLocal *) declManager)->declTypes.Num(); i++ ) {
		int size, num;

		if ( ((idDeclManagerLocal *) declManager)->declTypes[i] == NULL ) {
			continue;
		}

		num = ((idDeclManagerLocal *) declManager)->linearLists[i].Num();
		totalDecls += num;

		size = 0;
		for ( j = 0; j < num; j++ ) {
			size += ((idDeclManagerLocal *) declManager)->linearLists[i][j]->Size();
			if ( ((idDeclManagerLocal *) declManager)->linearLists[i][j]->self != NULL ) {
				size += ((idDeclManagerLocal *) declManager)->linearLists[i][j]->self->Size();
			}
		}
		totalStructs += size;

		common->Printf( "%4ik %4i %s\n", size >> 10, num, ((idDeclManagerLocal *) declManager)->declTypes[i]->typeName.c_str() );
	}

	for ( i = 0 ; i < ((idDeclManagerLocal *) declManager)->loadedFiles.Num() ; i++ ) {
		idDeclFile	*df = ((idDeclManagerLocal *) declManager)->loadedFiles[i];
		totalText += df->fileSize;
	}

	common->Printf( "%i total decls is %i decl files\n", totalDecls, ((idDeclManagerLocal *) declManager)->loadedFiles.Num() );
...
//	soundSystem->SetMute( true );
	...
	((idDeclManagerLocal *) declManager)->Reload( force );
...
void idDeclManagerLocal::TouchDecl_f( const idCmdArgs &args ) {
	...
	if ( args.Argc() != 3 ) {
		...
		for ( int i = 0 ; i < ((idDeclManagerLocal *) declManager)->declTypes.Num() ; i++ ) {
			if ( ((idDeclManagerLocal *) declManager)->declTypes[i] ) {
				common->Printf( "%s ", ((idDeclManagerLocal *) declManager)->declTypes[i]->typeName.c_str() );
			}
		}
		common->Printf( "\n" );
		return;
	...
	for ( i = 0; i < ((idDeclManagerLocal *) declManager)->declTypes.Num(); i++ ) {
		if ( ((idDeclManagerLocal *) declManager)->declTypes[i] && ((idDeclManagerLocal *) declManager)->declTypes[i]->typeName.Icmp( args.Argv( 1 ) ) == 0 ) {
			break;
		}
	}
	if ( i >= ((idDeclManagerLocal *) declManager)->declTypes.Num() ) {
		common->Printf( "unknown decl type '%s'\n", args.Argv( 1 ) );
		return;
	}

	const idDecl *decl = ((idDeclManagerLocal *) declManager)->FindType( (declType_t)i, args.Argv( 2 ), false );
	if ( !decl ) {
		common->Printf( "%s '%s' not found\n", ((idDeclManagerLocal *) declManager)->declTypes[i]->typeName.c_str(), args.Argv( 2 ) );
	}
}
...
bool idDeclLocal::IsImplicit( void ) const {
	return ( sourceFile == ((idDeclManagerLocal *) declManager)->GetImplicitDeclFile() );
}
...
bool idDeclLocal::ReplaceSourceFileText( void ) {
	...
	if ( sourceFile == &((idDeclManagerLocal *) declManager)->implicitDecls ) {
...
void idDeclLocal::MakeDefault() {
	...
	((idDeclManagerLocal *) declManager)->MediaPrint( "DEFAULTED\n" );
...
void idDeclLocal::AllocateSelf( void ) {
	if ( self == NULL ) {
		self = ((idDeclManagerLocal *) declManager)->GetDeclType( (int)type )->allocator();
		self->base = this;
	}
}
...
void idDeclLocal::ParseLocal( void ) {
	...
	((idDeclManagerLocal *) declManager)->MediaPrint( "parsing %s %s\n", ((idDeclManagerLocal *) declManager)->declTypes[type]->typeName.c_str(), name.c_str() );
	...
	((idDeclManagerLocal *) declManager)->indent++;
