...
idMsgQueue::idMsgQueue( void ) {
    ...
    Sys_DebugPrintf("idMsgQueue::idMsgQueue( void ) size %d\r\n", sizeof(*this));
#endif
...
idMsgChannel::idMsgChannel() {
#ifdef TRACE_CONSTRUCTS
    Sys_DebugPrintf("idMsgChannel::idMsgChannel() size %d\r\n", sizeof(*this));
#endif
...
void idMsgChannel::Shutdown( void ) {
    Sys_Printf("void idMsgChannel::Shutdown( void ) TODO: delete null?\r\n");
    if (compressor) {
		delete compressor;
		compressor = NULL;
    }
