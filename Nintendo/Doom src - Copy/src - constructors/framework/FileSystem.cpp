...
//DRS TODO CONSTRUCTOR idFileSystemLocal	fileSystemLocal;
idFileSystem *		fileSystem = NULL;//&fileSystemLocal;
...
idFileSystemLocal::idFileSystemLocal( void ) {
    ...
    Sys_DebugPrintf("idFileSystemLocal::idFileSystemLocal( void ) size %d\r\n", sizeof(*this));
#endif
...
void idFileSystemLocal::Dir_f( const idCmdArgs &args ) {
	...
	fileList = ((idFileSystemLocal *) fileSystem)->ListFiles( relativePath, extension );
	...
	((idFileSystemLocal *) fileSystem)->FreeFileList( fileList );
}
...
void idFileSystemLocal::DirTree_f( const idCmdArgs &args ) {
	...
	fileList = ((idFileSystemLocal *) fileSystem)->ListFilesTree( relativePath, extension );
...
void idFileSystemLocal::Path_f( const idCmdArgs &args ) {
	...
	for ( sp = ((idFileSystemLocal *) fileSystem)->searchPaths; sp; sp = sp->next ) {
		if ( sp->pack ) {
			...
			if ( ((idFileSystemLocal *) fileSystem)->serverPaks.Num() ) {
				if ( ((idFileSystemLocal *) fileSystem)->serverPaks.Find( sp->pack ) ) {
	...
	common->Printf( "game DLL: 0x%x in pak: 0x%x\n", ((idFileSystemLocal *) fileSystem)->gameDLLChecksum, ((idFileSystemLocal *) fileSystem)->gamePakChecksum );
#if ID_FAKE_PURE
	common->Printf( "Note: ID_FAKE_PURE is enabled\n" );
#endif
	for( i = 0; i < MAX_GAME_OS; i++ ) {
		if ( ((idFileSystemLocal *) fileSystem)->gamePakForOS[ i ] ) {
			common->Printf( "OS %d - pak 0x%x\n", i, ((idFileSystemLocal *) fileSystem)->gamePakForOS[ i ] );
		}
	}
	// show addon packs that are *not* in the search lists
	common->Printf( "Addon pk4s:\n" );
	for ( sp = ((idFileSystemLocal *) fileSystem)->addonPaks; sp; sp = sp->next ) {
		if ( com_developer.GetBool() ) {
...
void idFileSystemLocal::TouchFile_f( const idCmdArgs &args ) {
	...
	f = ((idFileSystemLocal *) fileSystem)->OpenFileRead( args.Argv( 1 ) );
	if ( f ) {
		((idFileSystemLocal *) fileSystem)->CloseFile( f );
	}
}
	...
	if ( fileSystem->ReadFile( args.Argv( 1 ), ( void** )&buffer, NULL ) && buffer ) {
		...
		if ( src.IsLoaded() ) {
			...
			while( src.ReadToken( &token ) ) {
				...
				idFile *f = ((idFileSystemLocal *) fileSystem)->OpenFileRead( token );
				if ( f ) {
					((idFileSystemLocal *) fileSystem)->CloseFile( f );
				}
			}
...
Sys_Printf(">>> idFileSystemLocal::OpenFileRead %s\r\n", relativePath);
...
Sys_Printf(">>> open file for write: %s\r\n", OSpath.c_str());
...
dword BackgroundDownloadThread( void *parms ) {
	while( 1 ) {
		...
		backgroundDownload_t	*bgl = ((idFileSystemLocal *) fileSystem)->backgroundDownloads;
		if ( !bgl ) {
		...
		((idFileSystemLocal *) fileSystem)->backgroundDownloads = bgl->next;
		Sys_LeaveCriticalSection();
...
*/
 findFile_t idFileSystemLocal::FindFile( const char *path, bool scheduleAddons ) {
	...
	if ( !pak ) {
		...
		delete f; // DRS TODO: added, is it a leak without delete?
		return FIND_YES;
	}
