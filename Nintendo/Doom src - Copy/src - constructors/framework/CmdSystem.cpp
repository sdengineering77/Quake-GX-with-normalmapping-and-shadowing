...
// DRS TODO CONSTRUCTOR idCmdSystemLocal			cmdSystemLocal;
idCmdSystem *				cmdSystem = NULL;//&cmdSystemLocal;
// DRS TODO replace cmdSystemLocal by ((idCmdSystemLocal *) cmdSystem)->
...
void idCmdSystemLocal::ListByFlags( const idCmdArgs &args, cmdFlags_t flags ) {
	...
	for ( cmd = ((idCmdSystemLocal *) cmdSystem)->GetCommands(); cmd; cmd = cmd->next ) {
		if ( !( cmd->flags & flags ) ) {
			continue;
		}
...
void idCmdSystemLocal::Exec_f( const idCmdArgs &args ) {
	...
	((idCmdSystemLocal *) cmdSystem)->BufferCommandText( CMD_EXEC_INSERT, f );
...
void idCmdSystemLocal::Vstr_f( const idCmdArgs &args ) {
	...
	((idCmdSystemLocal *) cmdSystem)->BufferCommandText( CMD_EXEC_APPEND, va( "%s\n", v ) );
}
...
void idCmdSystemLocal::Wait_f( const idCmdArgs &args ) {
	if ( args.Argc() == 2 ) {
		((idCmdSystemLocal *) cmdSystem)->SetWait( atoi( args.Argv( 1 ) ) );
	} else {
		((idCmdSystemLocal *) cmdSystem)->SetWait( 1 );
	}
}
