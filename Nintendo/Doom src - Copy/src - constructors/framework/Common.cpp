...
// DRS TODO CONSTRUCTOR idCommonLocal	commonLocal;
idCommon *		common = NULL;//&commonLocal;
// DRS TODO replace commonLocal
...
idCommonLocal::idCommonLocal( void ) {
    ...
    Sys_DebugPrintf("idCommonLocal::idCommonLocal( void ) size %d\r\n", sizeof(*this));
#endif
...
#ifdef DOOMGX
	char msg[MAX_PRINT_MSG_SIZE];
	vsnprintf (msg, MAX_PRINT_MSG_SIZE-1, fmt, argptr);
	msg[sizeof(msg)-1] = '\0';
	va_end( argptr );
	Sys_Printf( msg ); // DRS TODO remove
#else
	VPrintf( fmt, argptr );
	va_end( argptr );
#endif
...
// DRS TODO	soundSystem->PrintMemInfo( &mi );			// sounds
...
static void Com_Error_f( const idCmdArgs &args ) {
	if ( !com_developer.GetBool() ) {
		((idCommonLocal *) common)->Printf( "error may only be used in developer mode\n" );
		return;
	}

	if ( args.Argc() > 1 ) {
		((idCommonLocal *) common)->FatalError( "Testing fatal error" );
	} else {
		((idCommonLocal *) common)->Error( "Testing drop error" );
	}
}
...
static void Com_Freeze_f( const idCmdArgs &args ) {
	...
	if ( args.Argc() != 2 ) {
		((idCommonLocal *) common)->Printf( "freeze <seconds>\n" );
		return;
	}

	if ( !com_developer.GetBool() ) {
		((idCommonLocal *) common)->Printf( "freeze may only be used in developer mode\n" );
		return;
	}
...
static void Com_Crash_f( const idCmdArgs &args ) {
	if ( !com_developer.GetBool() ) {
		((idCommonLocal *) common)->Printf( "crash may only be used in developer mode\n" );
		return;
	}
...
void Com_WriteConfig_f( const idCmdArgs &args ) {
	...
	if ( args.Argc() != 2 ) {
		((idCommonLocal *) common)->Printf( "Usage: writeconfig <filename>\n" );
		return;
	}

	filename = args.Argv(1);
	filename.DefaultFileExtension( ".cfg" );
	((idCommonLocal *) common)->Printf( "Writing %s.\n", filename.c_str() );
	((idCommonLocal *) common)->WriteConfigToFile( filename );
}
...
void Com_SetMachineSpec_f( const idCmdArgs &args ) {
	((idCommonLocal *) common)->SetMachineSpec();
}
...
void Com_ReloadEngine_f( const idCmdArgs &args ) {
	...
	if ( !((idCommonLocal *) common)->IsInitialized() ) {
		return;
	}
	...
	((idCommonLocal *) common)->ShutdownGame( true );
	((idCommonLocal *) common)->InitGame();
...
void Com_ReloadLanguage_f( const idCmdArgs &args ) {
	((idCommonLocal *) common)->InitLanguageDict();
}
...
void Com_LocalizeGuis_f( const idCmdArgs &args ) {
	...
	if ( idStr::Icmp( args.Argv(1), "all" ) == 0 ) {
		...
		for ( int i = 0; i < files->GetNumFiles(); i++ ) {
			((idCommonLocal *) common)->LocalizeGui( files->GetFile( i ), strTable );
		}
		...
		((idCommonLocal *) common)->LocalizeGui( args.Argv(1), strTable );
	}
	strTable.Save( filename );
}
...
void idCommonLocal::InitRenderSystem( void ) {
	if ( com_skipRenderer.GetBool() ) {
		Sys_Printf("void idCommonLocal::InitRenderSystem( void ) com_skipRenderer\r\n");
		return;
	}
...
void idCommonLocal::Frame( void ) {
	try {
		...
		/* DRS TODO if ( !Sys_FPU_StackIsEmpty() ) {
...
Sys_Printf("call console->ClearNotifyLines()\r\n");
		console->ClearNotifyLines();
		
Sys_Printf("call ClearCommandLine()\r\n");
		ClearCommandLine();
	...
	Sys_Printf("END idCommonLocal::Init()\r\n");
...
void idCommonLocal::InitGame( void ) {
	Sys_Printf("common.cpp idCommonLocal::InitGame TODO\r\n");
...
/* DRS TODO	uiManager->Shutdown(); */

	// shut down the sound system
/* DRS	soundSystem->Shutdown(); */
