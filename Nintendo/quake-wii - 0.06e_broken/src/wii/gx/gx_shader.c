...
=============
*/
/*void GL_DrawStripCommand(int count, trivertx_t *verts, int *order, mplanenbt_t *nbts) {
	int 			i, j;
	Vector 		fverts[3];
	Vector 		n, b, t;
	float			texst[3][2];
...
=============
*/
/*void GL_DrawFanCommand(int count, trivertx_t *verts, int *order, mplanenbt_t *nbts) {
	int 			i, j;
	Vector 		fverts[3];
	Vector 		n, b, t;
	float			texst[3][2];
...
//Con_DPrintf("R_DrawAliasFrameBumpShadow - ");
	...
	for(i=0; i<num; i++) {
		...
		while (1) 	{
			...
			do {
				...
				GX_Position3f32(verts->v[0], verts->v[1], verts->v[2]);
				// normals from nbt list
				GL_VecS8ToF32(&verts->normal[0], &n);
				GL_VecS8ToF32(&verts->bitangent[0], &b);
				GL_VecS8ToF32(&verts->tangent[0], &t);
				guVecScale(&b, &b, 0.125f);
				guVecScale(&t, &t, 0.125f);
				GX_Normal3f32  (n.x,	n.y,	n.z);
				GX_Normal3f32  (b.x,	b.y,	b.z);
	...
	// obvious for now
	R_SetDefaultQuakeShader();
//Con_DPrintf("END R_DrawAliasFrameBumpShadow\r\n");
