...
#define MAX_VOLUMENEIGHBOURS				4*MAX_STATIC_SHADOWVOLUMES	
...
volumeneighbour_t		r_volumeneighbours[MAX_VOLUMENEIGHBOURS];
...
=================
*/
qboolean UT_CreateVolumeFromVertices(const dlight_t *light, Vector *vertices, const int vismask, const int numvertices, shadowvol_t *shadowvol) {
	int 			i;
	int 			numverts;
	Vector 		t, b;
...
//	shadowvol->capsvisible = true;
	...
	for(i=0; i<numverts; i++) {
		...
		if (scale < 1.0f) scale = plusonepixscale;
...
vismask bit 0-32 correspond to visibility of volume frag no. 0-32
=================
*/
qboolean UT_CreateVolumeFromVerticesTranslate(dlight_t *light, Vector *verts, int vismask, int num, Mtx mtx) {
...
vismask = 0xFFFFFFFF;		
	...
	// TODO: vismask stuff not processed in this loop
	int idx = 0;
...
//Con_DPrintf("o: %5.1f %5.1f %5.1f t: %5.1f %5.1f %5.1f - ", vcurr.x, vcurr.y, vcurr.z, vertices[idx].x, vertices[idx].y, vertices[idx].z);			
	...
	if (UT_CreateVolumeFromVertices(light, vertices, vismask, idx, vol)) {
...
//	Con_DPrintf("==================\r\n");
	return UT_CreateVolumeFromVertices(light, vertices, 0xFFFFFFFF, idx, shadowvol);
}
	...
	for (face=0; face<2; face++) {
		...
		for(mode=0; mode<2; mode++) {
			...
			while(vol) {
				...
				if ((1 || vol->visframe == r_framecount)) {
					numvol++;
