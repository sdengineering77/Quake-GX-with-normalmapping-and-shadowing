...
Con_DPrintf("GL_DrawAliasFrame - END ");
...
=============
*/
void GL_VecU8ToF32(vecu8_t in, Vector *out) {
	out->x = (float) in[0];
	out->y = (float) in[1];
	out->z = (float) in[2];
}
...
=============
*/
/*void GL_CreateStripVolumes(dlight_t *light, Vector *mlightpos, int count, mtrivertx_t *verts, maliastri_t *tris) {
	int i, j;
	float dp;
	Vector fverts[3];
	Vector normal;
...
//Con_DPrintf("GL_AliasFrameVolume - ");
	...
	for(i=0; i<numtris; i++, ptris++) {
		if (trivisible[i]) {
			int vismask = 0;
			GL_VecU8ToF32(ptris->v[0], &fverts[0]);
			GL_VecU8ToF32(ptris->v[1], &fverts[1]);
			GL_VecU8ToF32(ptris->v[2], &fverts[2]);

/*int k;
for(k=0; k<3; k++) {
Con_DPrintf("%i %i %i - ", ptris->v[k][0], ptris->v[k][1], ptris->v[k][2]);
}
for(k=0; k<3; k++) {
Con_DPrintf("%5.1f %5.1f %5.1f - ", fverts[k].x, fverts[k].y, fverts[k].z);
}
Con_DPrintf("\r\n");
*/
			// neighbour check
			for(j=0; j<3; j++) {
				int neighbour = ptris->neighbours[j];
				if (neighbour > -1 && trivisible[neighbour]) {
					vismask |= 1<<j;
				}
			}
			// add it (TODO: add vismask)
			UT_CreateVolumeFromVerticesTranslate(light, fverts, vismask, 3, model);
			
		}
	}
//Con_DPrintf("END GL_AliasFrameVolume\r\n");
...
void R_PreprocessWorld(void) {
	...
	R_PreprocessViewModel();
	// draw flat
Con_DPrintf("PreprocessEntities\r\n");
	R_PreprocessEntitiesOnList();
	
Con_DPrintf("Drawdone\r\n");
	GX_DrawDone();
	
	// draw shadow volumes
Con_DPrintf("Draw volumes\r\n");
	GX_LoadPosMtxImm(view, GX_PNMTX0);
	GX_SetPixelFmt(GX_PF_RGB8_Z24, GX_ZC_FAR);
	R_DrawShadowVolumes();
	R_SetDefaultQuakeShader();
	R_RenderMode_Fullscreen();
Con_DPrintf("End preprocess\r\n");
