...
/* Get functions */
void _glGetMatrixd(GLenum pname, GLdouble *params) { // DRS
	PrjMtx	*pm;
	MtxP model;
	int 		i, j;
...
//			for (i=0; i<16; i++) {
//				*params++ = (double) pm->mat[i&0x03][i>>2];
//			}
			for(j = 0; j < 4; ++j) {
				for(i = 0; i < 4; ++i) {
...
//				*params++ = 0.0f;
	...
	case GL_PROJECTION_MATRIX:
		pm = TopPrjMtx(&projection_stack);
//			for (i=0; i<16; i++) { DRS TODO: check if this works correctly:)
//				*params++ = (double) pm->mat[i&0x03][i>>2];
//			}
		for(j = 0; j < 4; ++j) {
			for(i = 0; i < 4; ++i) {
