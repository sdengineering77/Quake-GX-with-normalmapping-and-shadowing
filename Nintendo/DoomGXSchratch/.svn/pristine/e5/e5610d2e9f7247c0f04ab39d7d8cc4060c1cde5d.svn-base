...
void initGlutData(void) {	
	...
	_alphatest_func = GL_ALWAYS;
	_alphatest_ref = 0.0f;
	_alphatest_dirty = true; // setup correctly first time render in setup_render() (gl_render.c)
...
void glutInitDisplayMode(unsigned int mode) {
	// init the vi.
	VIDEO_Init();
	// get a mode and tweak it a little
...
//	_console_inited = !CON_InitEx(rmode, conX, conY, conWidth, conHeight, 0);
	...
	void *gp_fifo = NULL;
	gp_fifo = memalign(32,DEFAULT_FIFO_SIZE*2);
	memset(gp_fifo,0,DEFAULT_FIFO_SIZE*2);
 
	GX_Init(gp_fifo,DEFAULT_FIFO_SIZE*2);
	...
	GX_CopyDisp(frameBuffer[fb],GX_TRUE);
	GX_CopyDisp(frameBuffer[fb],GX_TRUE);
	GX_SetDispCopyGamma(GX_GM_1_0); // 1_7 1_0
...
void glutSwapBuffers(void) {
	...
	GX_CopyDisp(frameBuffer[fb], GX_TRUE);			// DRS TODO
...
//		CON_CopyToFB(frameBuffer[fb]);
													...
													//
	VIDEO_SetNextFramebuffer(frameBuffer[fb]);		//
	//fb ^= 1;		// flip framebuffer				//swapbuffers(DC); DRS moved here from above (DRS TODO: was it fog or this that caused the blacks)
													//
	VIDEO_Flush();									//
													// 
//	VIDEO_WaitVSync();								//	
