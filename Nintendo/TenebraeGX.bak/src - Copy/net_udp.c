...
static __inline__ int gethostname(char *buf, int maxchar) {
	struct in_addr addr;
	addr.s_addr = net_gethostip();
	strncpy(buf, inet_ntoa(addr), maxchar);
	
	return 0;
	
}
static __inline__ struct hostent *gethostbyaddr(struct in_addr *addr, int size, int type) {
	return net_gethostbyname(inet_ntoa(*addr));
}
...
int UDP_GetSocketAddr (int socket, struct qsockaddr *addr)
{
	...
	// DRS TODO: this gives IP as integer, but we need it as string...
	((struct sockaddr_in *)addr)->sin_addr.s_addr = net_gethostip();
#else
