...
void R_DrawAliasFrameARBDiffuseSpecular (aliashdr_t *paliashdr, aliasframeinstant_t *instant)
{
    ...
    /*gxgl_glVertexPointer(3, GL_FLOAT, 0, instant->vertices);
    gxgl_glEnableClientState(GL_VERTEX_ARRAY);
    gxgl_glNormalPointer(GL_FLOAT, 0, instant->normals);
    gxgl_glEnableClientState(GL_NORMAL_ARRAY);

    qglClientActiveTextureARB(GL_TEXTURE0_ARB);
    gxgl_glTexCoordPointer(2, GL_FLOAT, 0, texcoords);
    gxgl_glEnableClientState(GL_TEXTURE_COORD_ARRAY);

    qglClientActiveTextureARB(GL_TEXTURE1_ARB);
    gxgl_glTexCoordPointer(3, GL_FLOAT, 0, linstant->tslights);
    gxgl_glEnableClientState(GL_TEXTURE_COORD_ARRAY);

    qglClientActiveTextureARB(GL_TEXTURE2_ARB);
    gxgl_glTexCoordPointer(3, GL_FLOAT, 0, linstant->tshalfangles);
    gxgl_glEnableClientState(GL_TEXTURE_COORD_ARRAY);*/
