...
//	float numinv = 1.0f/((float)surf->polys->numverts);
//	x *= numinv;
//	y *= numinv;
//	z *= numinv;
...
//	float attn = 0.00015f * light->radius * light->radius/(1.0f + dist*dist/1024.0f);
	float attn = 1 - (dist  * dist) / (light->radius * light->radius);
//	float attn = 0.020f * light->radius/(1.0f + dist*dist/1024.0f); // TODO r^2/dist^2
//	if (lnum == 139) 
//		attn = 100;
...
u64		current_time	= gettime();
	...
	l = cl_staticlights;
	for (lnum=0 ; lnum<cl_numstaticlights; lnum++, l++) {
if (lnum == -1) {debugLnum = true; } else {debugLnum = false; }
		dlight_t *light = l;
...
surfacemetric += num_surfaces;
			Con_DPrintf("cl static lnum: %d radius %f %5.1f %5.1f %5.1f\r\n", lnum, light->radius, light->origin[0], light->origin[1], light->origin[2]);

		} else {
			Con_DPrintf("skipped cl static lnum: %d radius %f %5.1f %5.1f %5.1f\r\n", lnum, light->radius, light->origin[0], light->origin[1], light->origin[2]);
			light->num_staticshadowvolumes = 0; // TODO: DRS do this at proper place???
		}
	}	
current_time = gettime() - current_time;
Con_DPrintf("======= TOTAL num surfaces: %i time: %i =========\r\n", surfacemetric, ((u32) current_time));
