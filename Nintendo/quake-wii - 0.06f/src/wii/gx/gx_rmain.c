...
//	guMtxTranspose(modelview, nmodelview);
...
void GL_AliasFrameVolume (dlight_t *light, aliashdr_t *paliashdr, int posenum) {
	...
	maliastri_t		*ptris;
	maliastri_t 		*tris;
	trianglenbs_t	*pneighbours;
	int					numtris;

	numtris = paliashdr->numtris;
	tris 					= (maliastri_t 	*) ((byte *)paliashdr + paliashdr->tridata);
	tris  					+= posenum * numtris;
	pneighbours	= (trianglenbs_t *) ((byte *)paliashdr + paliashdr->neighbourdata);
	
	// transform light to modelspace
	Mtx 		modelinv, modeltrans;
	Vector	mlightpos;
//	guMtxTranspose(model, modeltrans);
...
//	return;
	...
	for(i=0; i<numtris; i++, ptris++) {
		// check if triangle faces the model lightpos
		GL_VecS8ToF32(&ptris->normal, &normal);
		
		if (normal.x < 0.1f && normal.y < 0.1f && normal.z < 0.1f) {
			dp = 0;
		} else {
			dp = guVecDotProduct(&normal, &mlightpos) - ptris->dist;
		}
		
//		if (CheckTriVisible(ptris, light, &mlightpos, paliashdr, i)) {
//		if (CheckTriVisible(ptris, light, &mlightpos, i) && i > 40) {
//		if (CheckTriVisible(ptris, light, &mlightpos)) {
//		if (dp >= BACKFACE_EPSILON) {
		if (dp < -BACKFACE_EPSILON) {
	...
	for(i=0; i<numtris; i++, ptris++) {
		if (trivisible[i]) {
			...
			int vismask = 0;
			for(k=0; k<3; k++) {
				fverts[2-k].x = ptris->v[k][0];
				fverts[2-k].y = ptris->v[k][1];
				fverts[2-k].z = ptris->v[k][2];
			}

			// neighbour check
			for(j=0; j<3; j++) {
				int neighbour = pneighbours[i][2-j];
