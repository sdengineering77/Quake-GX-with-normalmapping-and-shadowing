...
//===========================================================================
void SZ_Alloc (sizebuf_t *buf, int startsize) // (R853)
{
	if (startsize < 256)
		startsize = 256;
...
==================
*/
void COM_DefaultExtension (char *path, char *extension) // R1031
{
	char    *src;
//
...
==============
*/
char *COM_Parse (char *data) // (R1063)
{
	int             c;
	int             len;
...
/*
================
COM_CheckParm (func start op R1147)
...
================
*/
void COM_Init (char *basedir) // R1318
{
	byte    swaptest[2] = {1,0};
...
void COM_FindAllExt (char *filedir, char *fileext, void (*callback)(const char *)) // R1744
{
	searchpath_t    *search;
	char            netpath[MAX_OSPATH];
	char			sysfileext[32];
...
===========
*/
int COM_OpenFile (const char *filename, int *handle) // R1810
{
...
byte    *loadbuf;
int             loadsize;
byte *COM_LoadFile (const char *path, int usehunk) // R1864
{
	int             h;
	byte    *buf;
	char    base[32];
...
byte *COM_LoadHunkFile (const char *path) // R1918
{
