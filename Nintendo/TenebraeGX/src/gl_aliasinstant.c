...
// DRS TODO: tweak
#define NUM_ALIAS_INSTANTS 24
//#define NUM_ALIAS_INSTANTS 64

#define NUM_ALIAS_LIGHT_INSTANTS 24
...
/*
 * AINST_InitMem
 assumes to be running in isolated mode
 */
void	AINST_InitMem(void) {
	...
	InstantCache = low;
	low += sizeof(aliasframeinstant_t) * NUM_ALIAS_INSTANTS;
	Sys_Printf("End at %p\n", low);

	low = align32(low);
	Sys_Printf("Allocated LightInstantCache at %p\n", low);
	LightInstantCache = low;
	low += sizeof(aliaslightinstant_t) * NUM_ALIAS_LIGHT_INSTANTS;
	Sys_Printf("End at %p\n", low);
