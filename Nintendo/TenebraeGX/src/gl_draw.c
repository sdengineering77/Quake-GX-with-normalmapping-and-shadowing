...
#else
cvar_t		gl_max_size = {"gl_max_size", "512"}; // DRS: a bit lessss, we only get to have 52MB DRAM + 24MB SRAM
#endif
...
int		gl_alpha_format = GL_RGB5_A1; // DRS gx2gl makes GX_RGB4A3 of that
#endif
...
//#else
	glBindTexture(GL_TEXTURE_2D, texnum); // DRS TODO does this bind upload32[texnum]?
//#endif
...
//	int		fstep;		// <AWE> unused because of "#if 0".
	...
	#ifdef GXGL
	// since Wii doesn't resize shadow tex mem can be instantiated once... but still not happy with this...
...
void GL_Upload32 (unsigned *data, int width, int height,  qboolean mipmap, qboolean alpha)
{
	int			texturemode;
	//XYZ
	// DRS Make em a bit smaller
#ifndef GXGL
	const int maxsize = 1024*1024;
static	unsigned	scaled[maxsize];	// [512*256];
#else
	const int maxsize = 512 * 256;
	unsigned	*scaled = malloc(maxsize*sizeof(unsigned)); // [512*256];
	if (!scaled) return;
#endif
...
#else // for GX the default maximum size may be 512x256 // DRS TODO: conback can have 512x256 I guesss, others must be 128x128 or so or look into CMPR soon!
	if (scaled_width > gl_max_size.value)
		scaled_width = gl_max_size.value;
...
#else
		// DRS:  from gl_textures.c, RGB5_A1 seems to map to best wii format until S3TC is implemented
		// case GL_RGB16: return GX_TF_RGB565;
		// case GL_RGB5_A1: return GX_TF_RGB5A3;
		texturemode = GL_RGB5_A1;
...
// DRS TODO: GX should be able to support this, for now, this route is effectively avoided, see gl_rmisc.c R_TranslatePlayerSkin()
...
#ifndef GXGL
	const int maxsize = 1024 * 1024;
static	unsigned	char scaled[maxsize];	// [512*256];
#else
	const int maxsize = 512 * 256;
	unsigned	char *scaled = malloc(maxsize);	// [512*256];
	if (!scaled) return;
#endif
	int			scaled_width, scaled_height;
	...
	if (scaled_height > gl_max_size.value)
		scaled_height = gl_max_size.value;
#else // for GX the default maximum size may be 512x256
	if (scaled_height > gl_max_size.value/2)
		scaled_height = gl_max_size.value/2;
#endif
...
void GL_UploadBump(byte *data, int width, int height, qboolean mipmap, byte* gloss)
{
    ...
    static unsigned char	scaled[maxsize];	// [512*256];
    static unsigned char	scaledgloss[maxsize];	// [512*256];
 #else
 	const int maxsize = 512 * 256;
	...
	if (scaled_height > gl_max_size.value)
		scaled_height = gl_max_size.value;
#else // for GX the default maximum size may be 256x256
...
void GL_UploadNormal(unsigned int *data, int width, int height, qboolean mipmap)
{
#ifndef GXGL
	const int maxsize = 	1024*1024;
    static unsigned int	scaled[maxsize];	// [512*256];
 #else
	const int maxsize = 512*256;
    ...
    {
        texturemode = GL_RGB5_A1; // DRS TODO: IA4/8
    }
...
#else
static	unsigned	trans[512*512];		// DRS TODO: alloc on hunk instead?
#endif
...
static	unsigned char	glosspix[1024*1024];	// PENTA: bumped texture (it seems the previous one never got fixed anyway)
#else
	unsigned char	*bumppix = malloc(512*512);	// DRS: don't use textures bigger than this, same size as trans...
	unsigned char	*glosspix = malloc(512*512); // engine supports 512x256 textures for now...
	if (!glosspix) {
		free(bumppix);
		return;
	...
	if (!strcmp("scrap",identifier)) {
		...
		// nooooooo, we must free our huge willies! return;
	} else {
...
int GL_Load3DAttenTexture(void)
{
// DRS TODO: this can be done in GL, resize tex to (x, y, z) (32x32)x32 texture strip, use indirect tex lookup to get offsets based based on z comp. returns 0 now
#ifndef GXGL
...
{
// DRS TODO: this *could* be done in GL on GX, but since light vectors seem to be in tangent space, f(s, t) = lightdirtex((Lx-Px)/(Lz-Pz), (Ly-Py)/(Lz-Pz)) should be sufficient.
// DRS TODO: used for normalmapping, will use uv mapping in this port though
// DRS TODO: dont use this code for now! it is used in gl_bumpxxx.c
#ifndef GXGL
	vec3_t vec;
	int i, x, y;
