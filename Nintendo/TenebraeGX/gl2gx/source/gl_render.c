...
// DRS update alpha compare when setup_render is first called
static u8 prevGXCompareFunc = GX_NEVER; // will do
...
void gxSetupAlphaCompare() {
	...
	if (_alphatest_enabled) {
		if (_alphatest_dirty) {
			...
			update = false;
			gxCompareFunc = GX_ALWAYS; // redundant but avoids warning
			gxZCompLoc = GX_TRUE; // redundant but avoids warning
		}
	} else {
		if (prevGXCompareFunc != GX_ALWAYS) {
			gxCompareFunc = GX_ALWAYS;
			gxZCompLoc = GX_TRUE;
			update = true;
		} else {
	...
	if (update) {
//	dprintf("comploc %d gxCompareFunc %d ref %f\n", gxZCompLoc, gxCompareFunc, _alphatest_ref);
		GX_SetZCompLoc(gxZCompLoc);
		GX_SetAlphaCompare(gxCompareFunc, (float) _alphatest_ref, GX_AOP_AND, GX_ALWAYS, 0);
		prevGXCompareFunc = gxCompareFunc;
	}
}	

/*void GX_ProjectSquare(float *point, float rad, float *rect) {
	float winv;
	Vector	camspace, eyespace;
	PrjMtx	*p = TopPrjMtx(&projection_stack);
	MtxP	modelview = *TopMtx(&model_stack);
	
	// to eye space
	guVecMultiply(modelview, point, guVector &camspace);
	winv = -1.0f / camspace.z;
	// to cam space
	guVecMultiply(p->mat, &camspace, &eyespace);
	rect[0] = (eyespace.x - rad) * winv;
	rect[1] = (eyespace.y * winv;
	rect[2] = eyespace.x * winv;
...
static void setup_render()
{
	...
	// DRS setup alpha compare (if not enabled default to GL_ALWAYS)
	gxSetupAlphaCompare();
...
void glDrawBuffer(GLenum type) {
	// DRS: no front or backbuffers available, do nothing
}
