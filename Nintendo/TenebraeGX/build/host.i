# 1 "d:/Data/Nintendo/TenebraeGX/src/host.c"
# 1 "d:\\Data\\Nintendo\\TenebraeGX\\build//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "d:/Data/Nintendo/TenebraeGX/src/host.c"
# 22 "d:/Data/Nintendo/TenebraeGX/src/host.c"
...
# 23 "d:/Data/Nintendo/TenebraeGX/src/host.c" 2
# 35 "d:/Data/Nintendo/TenebraeGX/src/host.c"
...
cvar_t pausedemo = {"pausedemo","0"};

cvar_t maxfps = {"maxfps", "120", 1};

cvar_t timescalecvar = {"timescale","1"};
...
static int textureBaseLine = -1;
static int glTextureBaseLine = -1;
extern int texture_extension_number;
...
void Host_EndGame (char *message, ...)
{
 ...
 __builtin_va_start(argptr,message);
...
# 137 "d:/Data/Nintendo/TenebraeGX/src/host.c"
void Host_Error (char *error, ...)
{
 va_list argptr;
 char string[1024];
...
void Host_InitLocal (void)
{
 ...
 Cvar_RegisterVariable (&timescalecvar);
...
# 295 "d:/Data/Nintendo/TenebraeGX/src/host.c"
void SV_ClientPrintf (char *fmt, ...)
{
 va_list argptr;
 char string[1024];
 ...
 MSG_WriteByte (&host_client->message, 8);
 MSG_WriteString (&host_client->message, string);
}
# 315 "d:/Data/Nintendo/TenebraeGX/src/host.c"
void SV_BroadcastPrintf (char *fmt, ...)
{
 va_list argptr;
 char string[1024];
 ...
 for (i=0 ; i<svs.maxclients ; i++)
  ...
  {
   MSG_WriteByte (&svs.clients[i].message, 8);
   MSG_WriteString (&svs.clients[i].message, string);
  }
}
# 340 "d:/Data/Nintendo/TenebraeGX/src/host.c"
void Host_ClientCommands (char *fmt, ...)
{
 va_list argptr;
 char string[1024];
 ...
 MSG_WriteByte (&host_client->message, 9);
 MSG_WriteString (&host_client->message, string);
}
# 362 "d:/Data/Nintendo/TenebraeGX/src/host.c"
void SV_DropClient (qboolean crash)
{
 int saveSelf;
 int i;
 ...
 host_client->netconnection = ((void *)0);
 ...
 for (i=0, client = svs.clients ; i<svs.maxclients ; i++, client++)
 {
  if (!client->active)
   continue;
  MSG_WriteByte (&client->message, 13);
  MSG_WriteByte (&client->message, host_client - svs.clients);
  MSG_WriteString (&client->message, "");
  MSG_WriteByte (&client->message, 14);
  MSG_WriteByte (&client->message, host_client - svs.clients);
  MSG_WriteShort (&client->message, 0);
  MSG_WriteByte (&client->message, 17);
  MSG_WriteByte (&client->message, host_client - svs.clients);
  MSG_WriteByte (&client->message, 0);
 }
}
# 424 "d:/Data/Nintendo/TenebraeGX/src/host.c"
void Host_ShutdownServer(qboolean crash)
{
 int i;
 int count;
...
# 496 "d:/Data/Nintendo/TenebraeGX/src/host.c"
void Host_ClearMemory (void)
{
 ...
 lightmap_textures = 0;
 if (textureBaseLine >= 0) {
  int i, j;
  int numDynTex = texture_extension_number - textureBaseLine;
  GLuint *texPtrArr = malloc(sizeof(GLuint) * numDynTex);
  for (i=textureBaseLine, j=0; i<texture_extension_number; i++, j++) {
   texPtrArr[j] = i;
  }
  Con_Printf("Deleting GL Texture range : %d - %d\n", textureBaseLine, texture_extension_number);
  glDeleteTextures(numDynTex, texPtrArr);
  texture_extension_number = textureBaseLine;
  numgltextures = glTextureBaseLine;
  free(texPtrArr);
 } else {
  textureBaseLine = texture_extension_number;
  glTextureBaseLine = numgltextures;
  Con_DPrintf(">> Setting basetexture to ID: %d\n", textureBaseLine);
 }

}
# 541 "d:/Data/Nintendo/TenebraeGX/src/host.c"
qboolean Host_FilterTime (float time)
{
 realtime += time;

 if (maxfps.value < 1) Cvar_SetValue("maxfps", 120);
 if (!cls.timedemo && realtime - oldrealtime < 1.0/maxfps.value)

  return 0;

 if (timescalecvar.value == 1)
...
# 578 "d:/Data/Nintendo/TenebraeGX/src/host.c"
...
# 646 "d:/Data/Nintendo/TenebraeGX/src/host.c"
void Host_ServerFrame (void)
{
...
# 679 "d:/Data/Nintendo/TenebraeGX/src/host.c"
void _Host_Frame (float time)
{
 ...
 if (sv.active)
  CL_SendCmd ();
# 718 "d:/Data/Nintendo/TenebraeGX/src/host.c"
 Host_GetConsoleCommands ();

 if (sv.active)
  Host_ServerFrame ();
# 731 "d:/Data/Nintendo/TenebraeGX/src/host.c"
 if (!sv.active)
  CL_SendCmd ();
...
{
 if (standard_quake)
  minimum_memory = 0x550000;
 else
  minimum_memory = (0x550000 + 0x100000);
 ...
 Sys_Printf("host init membase %p %d\n", parms->membase, parms->memsize);
 ...
 Host_InitLocal ();
 Sys_Printf(" cls.state %d ca_dedicated %d\n", cls.state, ca_dedicated);
 ...
 Con_Printf ("Exe: ""16:14:01"" ""Sep 23 2012""\n");
 ...
 if (cls.state != ca_dedicated)
 {
  ...
  S_Init ();
# 958 "d:/Data/Nintendo/TenebraeGX/src/host.c"
  CDAudio_Init ();
  Sbar_Init ();
  CL_Init ();
...
# 989 "d:/Data/Nintendo/TenebraeGX/src/host.c"
void Host_Shutdown(void)
{
 ...
 isdown = 1;
# 1015 "d:/Data/Nintendo/TenebraeGX/src/host.c"
}
