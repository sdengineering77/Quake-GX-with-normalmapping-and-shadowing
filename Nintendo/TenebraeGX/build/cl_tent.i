# 1 "d:/Data/Nintendo/TenebraeGX/src/cl_tent.c"
# 1 "d:\\Data\\Nintendo\\TenebraeGX\\build//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "d:/Data/Nintendo/TenebraeGX/src/cl_tent.c"
# 22 "d:/Data/Nintendo/TenebraeGX/src/cl_tent.c"
...
# 23 "d:/Data/Nintendo/TenebraeGX/src/cl_tent.c" 2

int num_temp_entities;
entity_t cl_temp_entities[64];
beam_t cl_beams[24];
...
sfx_t *cl_sfx_ric3;
sfx_t *cl_sfx_r_exp3;
# 45 "d:/Data/Nintendo/TenebraeGX/src/cl_tent.c"
void CL_InitTEnts (void)
{
...
void CL_ParseBeam (model_t *m)
{
 ...
 R_ParticleGunHits(end,5);


 for (i=0, b=cl_beams ; i< 24 ; i++, b++)
  if (b->entity == ent)
  {
   b->entity = ent;
   b->model = m;
   b->endtime = cl.time + 0.2;
   {b->start[0]=start[0];b->start[1]=start[1];b->start[2]=start[2];};
   {b->end[0]=end[0];b->end[1]=end[1];b->end[2]=end[2];};
   return;
  }
...
void CL_ParseTEnt (void)
{
 ...
 case 0:
  ...
  pos[1] = MSG_ReadCoord ();
  pos[2] = MSG_ReadCoord ();
# 160 "d:/Data/Nintendo/TenebraeGX/src/cl_tent.c"
  R_ParticleGunHits(pos, 0);
  if ( rand() % 5 )
 ...
 case 3:
  ...
  dl = CL_AllocDlight (0);
  {dl->origin[0]=pos[0];dl->origin[1]=pos[1];dl->origin[2]=pos[2];};
  dl->radius = 350;
  dl->die = cl.time + 0.5;
  dl->decay = 300;
 ...
 case 12:
  ...
  S_StartSound (-1, 0, cl_sfx_r_exp3, pos, 1, 1);
  break;
# 301 "d:/Data/Nintendo/TenebraeGX/src/cl_tent.c"
 default:
...
void CL_UpdateTEnts (void)
{
 ...
 for (i=0, b=cl_beams ; i< 24 ; i++, b++)
 {
  ...
  {
   {b->start[0]=cl_entities[cl.viewentity].origin[0];b->start[1]=cl_entities[cl.viewentity].origin[1];b->start[2]=cl_entities[cl.viewentity].origin[2];};
  }


  {dist[0]=b->end[0]-b->start[0];dist[1]=b->end[1]-b->start[1];dist[2]=b->end[2]-b->start[2];};
  ...
  {
   yaw = (int) (atan2(dist[1], dist[0]) * 180 / 3.14159265358979323846);
   if (yaw < 0)
    yaw += 360;

   forward = sqrt (dist[0]*dist[0] + dist[1]*dist[1]);
   pitch = (int) (atan2(dist[2], forward) * 180 / 3.14159265358979323846);
   if (pitch < 0)
    pitch += 360;
  }


  {org[0]=b->start[0];org[1]=b->start[1];org[2]=b->start[2];};
