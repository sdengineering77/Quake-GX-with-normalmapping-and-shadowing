# 1 "d:/Data/Nintendo/TenebraeGX/src/sv_main.c"
# 1 "d:\\Data\\Nintendo\\TenebraeGX\\build//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "d:/Data/Nintendo/TenebraeGX/src/sv_main.c"
# 22 "d:/Data/Nintendo/TenebraeGX/src/sv_main.c"
...
# 23 "d:/Data/Nintendo/TenebraeGX/src/sv_main.c" 2

server_t sv;
server_static_t svs;

char localmodels[256][5];
# 36 "d:/Data/Nintendo/TenebraeGX/src/sv_main.c"
void SV_Init (void)
{
 int i;
...
# 80 "d:/Data/Nintendo/TenebraeGX/src/sv_main.c"
void SV_StartParticle (vec3_t org, vec3_t dir, int color, int count)
{
 int i, v;

 if (sv.datagram.cursize > 16384 -16)
  return;
 MSG_WriteByte (&sv.datagram, 18);
...
# 118 "d:/Data/Nintendo/TenebraeGX/src/sv_main.c"
void SV_StartSound (edict_t *entity, int channel, char *sample, int volume,
    float attenuation)
{
    int sound_num;
 ...
 if (sv.datagram.cursize > 16384 -16)
  return;
    ...
    for (sound_num=1 ; sound_num<256
        && sv.sound_precache[sound_num] ; sound_num++)
        if (!strcmp(sample, sv.sound_precache[sound_num]))
            break;

    if ( sound_num == 256 || !sv.sound_precache[sound_num] )
    {
...
# 189 "d:/Data/Nintendo/TenebraeGX/src/sv_main.c"
void SV_SendServerinfo (client_t *client)
{
 char **s;
 char message[2048];

 MSG_WriteByte (&client->message, 8);
 sprintf (message, "%c\nVERSION %4.2f SERVER (%i CRC)", 2, 1.09, pr_crc);
 MSG_WriteString (&client->message,message);

 MSG_WriteByte (&client->message, 11);
 MSG_WriteLong (&client->message, 667);
 ...
 MSG_WriteByte (&client->message, 32);
 MSG_WriteByte (&client->message, sv.edicts->v.sounds);
 MSG_WriteByte (&client->message, sv.edicts->v.sounds);


 MSG_WriteByte (&client->message, 5);
 MSG_WriteShort (&client->message, NUM_FOR_EDICT(client->edict));

 MSG_WriteByte (&client->message, 25);
 MSG_WriteByte (&client->message, 1);

 client->sendsignon = 1;
 client->spawned = 0;
}
# 243 "d:/Data/Nintendo/TenebraeGX/src/sv_main.c"
...
# 302 "d:/Data/Nintendo/TenebraeGX/src/sv_main.c"
void SV_CheckForNewClients (void)
{
...
# 348 "d:/Data/Nintendo/TenebraeGX/src/sv_main.c"
void SV_ClearDatagram (void)
{
 SZ_Clear (&sv.datagram);
}
# 364 "d:/Data/Nintendo/TenebraeGX/src/sv_main.c"
int fatbytes;
byte fatpvs[8192/8];
...
# 410 "d:/Data/Nintendo/TenebraeGX/src/sv_main.c"
...
# 427 "d:/Data/Nintendo/TenebraeGX/src/sv_main.c"
void SV_WriteEntitiesToClient (edict_t *clent, sizebuf_t *msg)
{
 ...
 {org[0]=clent->v.origin[0]+clent->v.view_ofs[0];org[1]=clent->v.origin[1]+clent->v.view_ofs[1];org[2]=clent->v.origin[2]+clent->v.view_ofs[2];};
 pvs = SV_FatPVS (org);


 ent = ((edict_t *)( (byte *)sv.edicts + pr_edict_size));
 for (e=1 ; e<sv.num_edicts ; e++, ent = ((edict_t *)( (byte *)ent + pr_edict_size)))
 {
  ...
  if (ent->baseline.alpha != (eval_alpha ? (*((float*)((char*)&ent->v + eval_alpha))) : 0)) {
   bits |= (1<<17);
  }

  if (ent->baseline.style != (eval_style ? (*((float*)((char*)&ent->v + eval_style))) : 0)) {
   bits |= (1<<19);
  }

  if (ent->baseline.light_lev != (eval_light_lev ? (*((float*)((char*)&ent->v + eval_light_lev))) : 300)) {
   bits |= (1<<18);
  }

  if ((ent->baseline.color[0] != (eval_color ? (*((vec3_t*)((char*)&ent->v + eval_color))) : vec3_origin)[0]) ||
   (ent->baseline.color[1] != (eval_color ? (*((vec3_t*)((char*)&ent->v + eval_color))) : vec3_origin)[1]) ||
   (ent->baseline.color[2] != (eval_color ? (*((vec3_t*)((char*)&ent->v + eval_color))) : vec3_origin)[2]))
  {
   bits |= (1<<16);
  }

  if (ent->baseline.pflags != (eval_pflags ? (*((float*)((char*)&ent->v + eval_pflags))) : 0)) {
   bits |= (1<<20);
  }
  ...
  if (bits & (1<<16)) {
   for (i=0 ; i<3 ; i++)
   {
    MSG_WriteByte(msg, (int)((eval_color ? (*((vec3_t*)((char*)&ent->v + eval_color))) : vec3_origin)[i]*255));
   }
  }

  if (bits & (1<<17))
   MSG_WriteByte (msg, (int)((eval_alpha ? (*((float*)((char*)&ent->v + eval_alpha))) : 0)*255));

  if (bits & (1<<18))
   MSG_WriteShort (msg, (eval_light_lev ? (*((float*)((char*)&ent->v + eval_light_lev))) : 300));

  if (bits & (1<<19))
   MSG_WriteByte (msg, (eval_style ? (*((float*)((char*)&ent->v + eval_style))) : 0));

  if (bits & (1<<20))
   MSG_WriteByte (msg, (eval_pflags ? (*((float*)((char*)&ent->v + eval_pflags))) : 0));
...
void SV_CleanupEnts (void)
{
 ...
 for (e=1 ; e<sv.num_edicts ; e++, ent = ((edict_t *)( (byte *)ent + pr_edict_size)))
 {
  ent->v.effects = (int)ent->v.effects & ~2;
 }
...
void SV_WriteClientdataToMessage (edict_t *ent, sizebuf_t *msg)
{
 ...
 {
  other = ((edict_t *)((byte *)sv.edicts + ent->v.dmg_inflictor));
 ...
 for (i=0, host_client = svs.clients ; i<svs.maxclients ; i++, host_client++)
 {
  if (host_client->old_frags != host_client->edict->v.frags)
  {
   for (j=0, client = svs.clients ; j<svs.maxclients ; j++, client++)
   {
    if (!client->active)
     continue;
    MSG_WriteByte (&client->message, 14);
...
# 852 "d:/Data/Nintendo/TenebraeGX/src/sv_main.c"
...
# 958 "d:/Data/Nintendo/TenebraeGX/src/sv_main.c"
int SV_ModelIndex (char *name)
{
 int i;
 ...
 for (i=0 ; i<256 && sv.model_precache[i] ; i++)
  if (!strcmp(sv.model_precache[i], name))
   return i;
 if (i==256 || !sv.model_precache[i])
...
void SV_CreateBaseline (void)
{
 ...
 for (entnum = 0; entnum < sv.num_edicts ; entnum++)
 {
  ...
  if (entnum > svs.maxclients && !svent->v.modelindex)
   continue;
# 1007 "d:/Data/Nintendo/TenebraeGX/src/sv_main.c"
  {svent->baseline.origin[0]=svent->v.origin[0];svent->baseline.origin[1]=svent->v.origin[1];svent->baseline.origin[2]=svent->v.origin[2];};
  {svent->baseline.angles[0]=svent->v.angles[0];svent->baseline.angles[1]=svent->v.angles[1];svent->baseline.angles[2]=svent->v.angles[2];};
  svent->baseline.frame = svent->v.frame;
  svent->baseline.skin = svent->v.skin;


  svent->baseline.alpha = (eval_alpha ? (*((float*)((char*)&svent->v + eval_alpha))) : 0);
  svent->baseline.light_lev = (eval_light_lev ? (*((float*)((char*)&svent->v + eval_light_lev))) : 300);
  svent->baseline.style = (eval_style ? (*((float*)((char*)&svent->v + eval_style))) : 0);
  svent->baseline.pflags = (eval_pflags ? (*((float*)((char*)&svent->v + eval_pflags))) : 0);

  {svent->baseline.color[0]=(eval_color ? (*((vec3_t*)((char*)&svent->v + eval_color))) : vec3_origin)[0];svent->baseline.color[1]=(eval_color ? (*((vec3_t*)((char*)&svent->v + eval_color))) : vec3_origin)[1];svent->baseline.color[2]=(eval_color ? (*((vec3_t*)((char*)&svent->v + eval_color))) : vec3_origin)[2];};
...
# 1065 "d:/Data/Nintendo/TenebraeGX/src/sv_main.c"
...
# 1095 "d:/Data/Nintendo/TenebraeGX/src/sv_main.c"
void SV_SaveSpawnparms (void)
{
 ...
 for (i=0, host_client = svs.clients ; i<svs.maxclients ; i++, host_client++)
 {
  ...
  pr_global_struct->self = ((byte *)host_client->edict - (byte *)sv.edicts);
  PR_ExecuteProgram (pr_global_struct->SetChangeParms);
  for (j=0 ; j<16 ; j++)
   host_client->spawn_parms[j] = (&pr_global_struct->parm1)[j];
 }
}
# 1122 "d:/Data/Nintendo/TenebraeGX/src/sv_main.c"
