# 1 "d:/Data/Nintendo/TenebraeGX/src/gl_warp.c"
# 1 "d:\\Data\\Nintendo\\TenebraeGX\\build//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "d:/Data/Nintendo/TenebraeGX/src/gl_warp.c"
# 27 "d:/Data/Nintendo/TenebraeGX/src/gl_warp.c"
...
# 28 "d:/Data/Nintendo/TenebraeGX/src/gl_warp.c" 2
...
int newwatertex,newslimetex,newlavatex,newteletex,newenvmap;
...
void SubdividePolygon (int numverts, float *verts)
{
 ...
 for (j=0 ; j<numverts ; j++, v+= 3)
 {
     if (dist[j] >= 0)
     {
  {front[f][0]=v[0];front[f][1]=v[1];front[f][2]=v[2];};
  f++;
     }
     if (dist[j] <= 0)
     {
  {back[b][0]=v[0];back[b][1]=v[1];back[b][2]=v[2];};
  b++;
     }
 ...
 return;
    ...
    {
  tex[0] = (verts[0]*warpface->texinfo->vecs[0][0]+verts[1]*warpface->texinfo->vecs[0][1]+verts[2]*warpface->texinfo->vecs[0][2]);
  tex[1] = (verts[0]*warpface->texinfo->vecs[1][0]+verts[1]*warpface->texinfo->vecs[1][1]+verts[2]*warpface->texinfo->vecs[1][2]);

  R_AllocateVertexInTemp(verts,tex,tex);
    }
}
# 155 "d:/Data/Nintendo/TenebraeGX/src/gl_warp.c"
void GL_SubdivideSurface (msurface_t *fa)
{
    vec3_t verts[64];
    int numverts;
 ...
 {verts[numverts][0]=vec[0];verts[numverts][1]=vec[1];verts[numverts][2]=vec[2];};
 numverts++;
    }
...
float turbsin[] =
{
# 1 "d:/Data/Nintendo/TenebraeGX/src/gl_warp_sin.h" 1
# 20 "d:/Data/Nintendo/TenebraeGX/src/gl_warp_sin.h"
 ...
 -3.06147f, -2.87916f, -2.69512f, -2.50945f, -2.32228f, -2.1337f, -1.94384f, -1.75281f,
 -1.56072f, -1.3677f, -1.17384f, -0.979285f, -0.784137f, -0.588517f, -0.392541f, -0.19633f,
# 192 "d:/Data/Nintendo/TenebraeGX/src/gl_warp.c" 2
};
# 202 "d:/Data/Nintendo/TenebraeGX/src/gl_warp.c"
void EmitWaterPolys (msurface_t *fa)
{
    glpoly_t *p;
    float *v;
    ...
    {
 glBegin (0x0006);
 for (i=0,v=(float *)(&globalVertexTable[p->firstvertex]) ; i<p->numverts ; i++, v+=7)
 {
     os = v[3];
     ot = v[4];


     s = os + (turbsin[(int)((ot*0.125+realtime) * (256.0 / (2 * 3.14159265358979323846))) & 255])*0.25;
     s *= (1.0/64);

     t = ot + (turbsin[(int)((os*0.125+realtime) * (256.0 / (2 * 3.14159265358979323846))) & 255])*0.25;
     t *= (1.0/64);

     glTexCoord2f (s, t);

     if (gl_mtexable)
  qglMultiTexCoord2fARB (0x84C1, s, t);
...
void EmitMirrorWaterPolys (msurface_t *fa)
{
 ...
 for (i=0,v=(float *)(&globalVertexTable[p->firstvertex]) ; i<p->numverts ; i++, v+=7)
 {
     ...
     s = RandomXY(os,ot,57)*turbsin[(int)((ot*0.125+realtime) * (256.0 / (2 * 3.14159265358979323846))) & 255];
     s *=0.25;
     t = RandomXY(os,ot,63)*turbsin[(int)((ot*0.125+realtime) * (256.0 / (2 * 3.14159265358979323846))) & 255];
     t *=0.25;
...
void EmitMirrorPolys (msurface_t *fa)
{
 ...
 for (i=0,v=(float *)(&globalVertexTable[p->firstvertex]); i<p->numverts ; i++, v+=7)
 {
     qglMultiTexCoord2fARB(0x84C1, v[3]/64, v[4]/64);
     glTexCoord3fv(v);
     glVertex3fv(v);
 }
 glEnd ();
...
# 301 "d:/Data/Nintendo/TenebraeGX/src/gl_warp.c"
qboolean OverrideFluidTex(char *name)
{
    if (gl_watershader.value < 1) return 0;
    if (r_wateralpha.value == 1) return 0;
 ...
 GL_Bind(newwatertex);
 glTexEnvf(0x2300, 0x2200, 0x0104);
...
void EmitSkyPolys (msurface_t *fa)
{
 ...
 for (i=0,v=(float *)(&globalVertexTable[p->firstvertex]) ; i<p->numverts ; i++, v+=7)
 {
     {dir[0]=v[0]-r_origin[0];dir[1]=v[1]-r_origin[1];dir[2]=v[2]-r_origin[2];};
     dir[2] *= 3;
...
# 447 "d:/Data/Nintendo/TenebraeGX/src/gl_warp.c"
void EmitBothSkyLayers (msurface_t *fa)
{
...
# 626 "d:/Data/Nintendo/TenebraeGX/src/gl_warp.c"
...
# 761 "d:/Data/Nintendo/TenebraeGX/src/gl_warp.c"
    tmp[0] = 't';
    tmp[1] = 'g';
    tmp[2] = 'a';
...
int LoadTextureInPlace(char* filename, int size, byte* mem, int* width, int* height)
{
    ...
    strncpy(argh, filename,sizeof(argh));
    tmp = argh + strlen(filename) - 3;
# 867 "d:/Data/Nintendo/TenebraeGX/src/gl_warp.c"
    tmp[0] = 't';
    tmp[1] = 'g';
    tmp[2] = 'a';
    COM_FOpenFile (argh, &f);
...
void LoadTGA (FILE *fin)
{
 ...
 fseek(fin, targa_header.id_length, 1);
 ...
 for(row=rows-1; row>=0; row--, pixbuf += row_inc) {
  ...
  case 24:
      ...
      blue = (--(fin)->_r < 0 ? __srget_r(_impure_ptr, fin) : (int)(*(fin)->_p++));
      green = (--(fin)->_r < 0 ? __srget_r(_impure_ptr, fin) : (int)(*(fin)->_p++));
      red = (--(fin)->_r < 0 ? __srget_r(_impure_ptr, fin) : (int)(*(fin)->_p++));
      *pixbuf++ = red;
      *pixbuf++ = green;
      *pixbuf++ = blue;
      *pixbuf++ = 255;
  ...
  case 32:
      ...
      green = (--(fin)->_r < 0 ? __srget_r(_impure_ptr, fin) : (int)(*(fin)->_p++));
      red = (--(fin)->_r < 0 ? __srget_r(_impure_ptr, fin) : (int)(*(fin)->_p++));
      alphabyte = (--(fin)->_r < 0 ? __srget_r(_impure_ptr, fin) : (int)(*(fin)->_p++));
      *pixbuf++ = red;
      *pixbuf++ = green;
      *pixbuf++ = blue;
      *pixbuf++ = alphabyte;
 ...
 for(row=rows-1; row>=0; row--, pixbuf += row_inc) {
  ...
  packetHeader=(--(fin)->_r < 0 ? __srget_r(_impure_ptr, fin) : (int)(*(fin)->_p++));
...
# 1057 "d:/Data/Nintendo/TenebraeGX/src/gl_warp.c"
void LoadColorTGA (FILE *fin, byte *pixels, int *width, int *height, char* fname)
{
    int columns, rows, numPixels;
    byte *pixbuf;
 ...
 for(row=rows-1; row>=0; row--, pixbuf += row_inc) {
     for(column=0; column<columns; column++) {
  unsigned char index;
  index = (--(fin)->_r < 0 ? __srget_r(_impure_ptr, fin) : (int)(*(fin)->_p++));
  *(int *)pixbuf = d_8to24table[index];
  pixbuf+=4;
     }
 }
    } else if (targa_header.image_type==2) {
 for(row=rows-1; row>=0; row--, pixbuf += row_inc) {
     for(column=0; column<columns; column++) {
  unsigned char red,green,blue,alphabyte;
  switch (targa_header.pixel_size) {
...
# 1248 "d:/Data/Nintendo/TenebraeGX/src/gl_warp.c"
    fclose(fin);
    *width = targa_header.width;
    *height = targa_header.height;
}
# 1260 "d:/Data/Nintendo/TenebraeGX/src/gl_warp.c"
void LoadGrayTGA (FILE *fin,byte *pixels,int *width, int *height, char *fname)
{
    int columns, rows, numPixels;
    int row, column;
 ...
 fseek(fin, 768, 1);
    ...
    for(row=rows-1; row>=0; row--, pixbuf += row_inc) {
 for(column=0; column<columns; column++) {
     *pixbuf++= (--(fin)->_r < 0 ? __srget_r(_impure_ptr, fin) : (int)(*(fin)->_p++));
 }
    }
...
int EasyTgaLoad(char *filename)
{
 ...
 texturemode = 0x8058;
    ...
    strncpy(argh, filename,sizeof(argh));
    tmp = argh + strlen(filename) - 3;
# 1416 "d:/Data/Nintendo/TenebraeGX/src/gl_warp.c"
    {
        tmp[0] = 't';
        tmp[1] = 'g';
        tmp[2] = 'a';
 ...
 glTexParameteri(0x0DE1, 0x2803, 0x2901);
    glTexImage2D (0x0DE1, 0, texturemode, width, height, 0, 0x1908, 0x1401, mem);
    free (mem);
...
# 1452 "d:/Data/Nintendo/TenebraeGX/src/gl_warp.c"
char skybox_name[64];
...
void R_LoadSkys (void)
{
 ...
 else
 {
  ...
  glTexImage2D (0x0DE1, 0, 0x8057, targa_header.width, targa_header.height, 0, 0x1908, 0x1401, targa_rgba);
...
void DrawSkyPolygon (int nump, vec3_t vecs)
{
    ...
    c_sky++;
# 1569 "d:/Data/Nintendo/TenebraeGX/src/gl_warp.c"
    {v[0]=vec3_origin[0];v[1]=vec3_origin[1];v[2]=vec3_origin[2];};
    for (i=0, vp=vecs ; i<nump ; i++, vp+=3)
    {
 {v[0]=vp[0]+v[0];v[1]=vp[1]+v[1];v[2]=vp[2]+v[2];};
    }
    av[0] = fabs(v[0]);
    av[1] = fabs(v[1]);
    av[2] = fabs(v[2]);
...
void ClipSkyPolygon (int nump, vec3_t vecs, int stage)
{
 ...
 return;
    ...
    {
 d = (v[0]*norm[0]+v[1]*norm[1]+v[2]*norm[2]);
 if (d > 0.1)
 {
     front = 1;
     sides[i] = 0;
 ...
 return;
    ...
    sides[i] = sides[0];
    dists[i] = dists[0];
    {(vecs+(i*3))[0]=vecs[0];(vecs+(i*3))[1]=vecs[1];(vecs+(i*3))[2]=vecs[2];};
    newc[0] = newc[1] = 0;
 ...
 {
 case 0:
     {newv[0][newc[0]][0]=v[0];newv[0][newc[0]][1]=v[1];newv[0][newc[0]][2]=v[2];};
     newc[0]++;
     break;
 case 1:
     {newv[1][newc[1]][0]=v[0];newv[1][newc[1]][1]=v[1];newv[1][newc[1]][2]=v[2];};
     newc[1]++;
     break;
 case 2:
     {newv[0][newc[0]][0]=v[0];newv[0][newc[0]][1]=v[1];newv[0][newc[0]][2]=v[2];};
...
void R_DrawSkyChain (msurface_t *s)
{
 ...
 for (p=fa->polys ; p ; p=p->next)
 {
     ...
     {
  {verts[i][0]=v[0]-r_origin[0];verts[i][1]=v[1]-r_origin[1];verts[i][2]=v[2]-r_origin[2];};
     }
...
void R_DrawSkyBox (void)
{
 ...
 cl.worldmodel->textures[skytexturenum]->texturechain = ((void *)0);
    }
...
# 1931 "d:/Data/Nintendo/TenebraeGX/src/gl_warp.c"
void R_InitSky (texture_t *mt)
{
 ...
 glTexParameteri(0x0DE1, 0x2802, 0x2901);
 glTexParameteri(0x0DE1, 0x2803, 0x2901);
    glTexImage2D (0x0DE1, 0, gl_solid_format, 128, 128, 0, 0x1908, 0x1401, trans);
    glTexParameterf(0x0DE1, 0x2801, 0x2601);
    glTexParameterf(0x0DE1, 0x2800, 0x2601);
 ...
 alphaskytexture = texture_extension_number++;
    GL_Bind(alphaskytexture);
    glTexImage2D (0x0DE1, 0, gl_alpha_format, 128, 128, 0, 0x1908, 0x1401, trans);
    glTexParameterf(0x0DE1, 0x2801, 0x2601);
    glTexParameterf(0x0DE1, 0x2800, 0x2601);
}
