# 1 "d:/Data/Nintendo/TenebraeGX/src/gl_decals.c"
# 1 "d:\\Data\\Nintendo\\TenebraeGX\\build//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "d:/Data/Nintendo/TenebraeGX/src/gl_decals.c"
# 26 "d:/Data/Nintendo/TenebraeGX/src/gl_decals.c"
...
# 27 "d:/Data/Nintendo/TenebraeGX/src/gl_decals.c" 2
...
void R_ClearDecals (void)
{
 ...
 active_decals = ((void *)0);

 for (i=0 ;i<r_numdecals ; i++)
  decals[i].next = &decals[i+1];
 decals[r_numdecals-1].next = ((void *)0);
}
...
void R_SpawnDecal(vec3_t center, vec3_t normal, vec3_t tangent, ParticleEffect_t *effect)
{
 ...
 {dec->origin[0]=center[0];dec->origin[1]=center[1];dec->origin[2]=center[2];};
 {dec->tangent[0]=tangent[0];dec->tangent[1]=tangent[1];dec->tangent[2]=tangent[2];};
 {dec->normal[0]=normal[0];dec->normal[1]=normal[1];dec->normal[2]=normal[2];};
 ...
 height = width;
 depth = width*0.5;
 dec->radius = ((width>height?width:height)>depth?(width>height?width:height):depth);
...
# 166 "d:/Data/Nintendo/TenebraeGX/src/gl_decals.c"
 d = (center[0]*tangent[0]+center[1]*tangent[1]+center[2]*tangent[2]);
 {leftPlane.normal[0]=tangent[0];leftPlane.normal[1]=tangent[1];leftPlane.normal[2]=tangent[2];};
 leftPlane.dist = -(width * 0.5 - d);
 VectorInverse(tangent);
 {rightPlane.normal[0]=tangent[0];rightPlane.normal[1]=tangent[1];rightPlane.normal[2]=tangent[2];};
 VectorInverse(tangent);
 rightPlane.dist = -(width * 0.5 + d);

 d = (center[0]*binormal[0]+center[1]*binormal[1]+center[2]*binormal[2]);
 {bottomPlane.normal[0]=binormal[0];bottomPlane.normal[1]=binormal[1];bottomPlane.normal[2]=binormal[2];};
 bottomPlane.dist = -(height * 0.5 - d);
 VectorInverse(binormal);
 {topPlane.normal[0]=binormal[0];topPlane.normal[1]=binormal[1];topPlane.normal[2]=binormal[2];};
 VectorInverse(binormal);
 topPlane.dist = -(height * 0.5 + d);

 d = (center[0]*normal[0]+center[1]*normal[1]+center[2]*normal[2]);
 {backPlane.normal[0]=normal[0];backPlane.normal[1]=normal[1];backPlane.normal[2]=normal[2];};
 backPlane.dist = -(depth - d);
 VectorInverse(normal);
 {frontPlane.normal[0]=normal[0];frontPlane.normal[1]=normal[1];frontPlane.normal[2]=normal[2];};
 VectorInverse(normal);
 ...
 for (a = 0; a < dec->vertexCount; a++)
 {
  float s, t;
  vec3_t v;
  {v[0]=dec->vertexArray[a][0]-center[0];v[1]=dec->vertexArray[a][1]-center[1];v[2]=dec->vertexArray[a][2]-center[2];};
  s = (v[0]*tangent[0]+v[1]*tangent[1]+v[2]*tangent[2]) * one_over_w + 0.5F;
  t = (v[0]*binormal[0]+v[1]*binormal[1]+v[2]*binormal[2]) * one_over_h + 0.5F;
...
void DecalWalkBsp_R(decal_t *dec, mnode_t *node)
{
 ...
 plane = node->plane;
 dist = (dec->origin[0]*plane->normal[0]+dec->origin[1]*plane->normal[1]+dec->origin[2]*plane->normal[2]) - plane->dist;
...
qboolean DecalAddPolygon(decal_t *dec, int vertcount, vec3_t *vertices)
{
 ...
 if (count + vertcount >= 128)
  return 0;

 if (dec->triangleCount + vertcount-2 >= 64)
  return 0;
 ...
 {
  {dec->vertexArray[count][0]=vertices[b][0];dec->vertexArray[count][1]=vertices[b][1];dec->vertexArray[count][2]=vertices[b][2];};
  count++;
 }
...
void DecalClipLeaf(decal_t *dec, mleaf_t *leaf)
{
 ...
 for (c=0; c<leaf->nummarksurfaces; c++, surf++) {
  ...
  {t3[0]=(*surf)->plane->normal[0];t3[1]=(*surf)->plane->normal[1];t3[2]=(*surf)->plane->normal[2];};
  ...
  if ((dec->normal[0]*t3[0]+dec->normal[1]*t3[1]+dec->normal[2]*t3[2]) > decalEpsilon)
  {
...
int DecalClipPolygonAgainstPlane(plane_t *plane, int vertexCount, vec3_t *vertex, vec3_t *newVertex)
{
 ...
 {
  qboolean neg = (((plane->normal[0]*vertex[a][0]+plane->normal[1]*vertex[a][1]+plane->normal[2]*vertex[a][2]) - plane->dist) < 0.0);
  negative[a] = neg;
  negativeCount += neg;
 }
 ...
 for (b = 0; b < vertexCount; b++)
 {
  ...
  if (negative[b])
  {
   if (!negative[c])
   {
    ...
    {v1[0]=vertex[c][0];v1[1]=vertex[c][1];v1[2]=vertex[c][2];};
    {v2[0]=vertex[b][0];v2[1]=vertex[b][1];v2[2]=vertex[b][2];};

    t = ((plane->normal[0]*v1[0]+plane->normal[1]*v1[1]+plane->normal[2]*v1[2]) - plane->dist) / (
  ...
  else
  {
   if (negative[c])
   {
    ...
    {v1[0]=vertex[b][0];v1[1]=vertex[b][1];v1[2]=vertex[b][2];};
    {v2[0]=vertex[c][0];v2[1]=vertex[c][1];v2[2]=vertex[c][2];};
   ...
   {newVertex[count][0]=vertex[b][0];newVertex[count][1]=vertex[b][1];newVertex[count][2]=vertex[b][2];};
   count++;
  }
 }
