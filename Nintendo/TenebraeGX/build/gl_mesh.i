# 1 "d:/Data/Nintendo/TenebraeGX/src/gl_mesh.c"
# 1 "d:\\Data\\Nintendo\\TenebraeGX\\build//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "d:/Data/Nintendo/TenebraeGX/src/gl_mesh.c"
# 22 "d:/Data/Nintendo/TenebraeGX/src/gl_mesh.c"
...
# 23 "d:/Data/Nintendo/TenebraeGX/src/gl_mesh.c" 2
...
int NormalToLatLong( const vec3_t normal) {
 ...
 if ( normal[0] == 0 && normal[1] == 0 ) {
  ...
  a = ( ( (atan2( normal[1], normal[0] )) * 180.0f ) / 3.14159265358979323846 ) * (255.0f / 360.0f );
  a &= 0xff;

  b = ( ( (acos( normal[2] )) * 180.0f ) / 3.14159265358979323846 ) * ( 255.0f / 360.0f );
  b &= 0xff;
...
# 166 "d:/Data/Nintendo/TenebraeGX/src/gl_mesh.c"
void GL_MakeAliasModelDisplayLists (model_t *m, aliashdr_t *hdr)
{
 int i, j, k, l;
 ...
 aliasmodel = m;
 paliashdr = hdr;
# 227 "d:/Data/Nintendo/TenebraeGX/src/gl_mesh.c"
 paliashdr->poseverts = paliashdr->numverts;
# 237 "d:/Data/Nintendo/TenebraeGX/src/gl_mesh.c"
  ...
  for (j=0; j<3; j++) {
   ...
   if (!triangles[i].facesfront && stverts[triangles[i].vertindex[j]].onseam) {
    ...
    for (k=0; k<paliashdr->numposes; k++) {
     ...
     verts[k*newcount+newindex].lightnormalindex = NormalToLatLong(r_avertexnormals[poseverts[k][triangles[i].vertindex[j]].lightnormalindex]);
    }
...
# 350 "d:/Data/Nintendo/TenebraeGX/src/gl_mesh.c"
 ...
 for (i=0; i<paliashdr->numposes; i++) {
  for (j=0; j<paliashdr->numtris ; j++) {
   ...
   norms[i*paliashdr->numtris+j].dist = (triangle[0][0]*norms[i*paliashdr->numtris+j].normal[0]+triangle[0][1]*norms[i*paliashdr->numtris+j].normal[1]+triangle[0][2]*norms[i*paliashdr->numtris+j].normal[2]);
  }
 }
 ...
 for (i=0; i<paliashdr->numposes; i++) {
  ...
  for (j=0; j<paliashdr->numtris; j++) {
   ...
   TangentForTri(&indecies[j*3],&verts[i*pheader->poseverts],texcoords,tangent,binormal);
   for (k=0; k<3; k++) {
    l = tris[j].vertindex[k];
    {tangents[i*paliashdr->poseverts+l][0]=tangents[i*paliashdr->poseverts+l][0]+tangent[0];tangents[i*paliashdr->poseverts+l][1]=tangents[i*paliashdr->poseverts+l][1]+tangent[1];tangents[i*paliashdr->poseverts+l][2]=tangents[i*paliashdr->poseverts+l][2]+tangent[2];}
                                          ;
