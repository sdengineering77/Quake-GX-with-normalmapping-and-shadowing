# 1 "d:/Data/Nintendo/TenebraeGX/src/gl_draw.c"
# 1 "d:\\Data\\Nintendo\\TenebraeGX\\build//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "d:/Data/Nintendo/TenebraeGX/src/gl_draw.c"
# 24 "d:/Data/Nintendo/TenebraeGX/src/gl_draw.c"
...
# 25 "d:/Data/Nintendo/TenebraeGX/src/gl_draw.c" 2
...
cvar_t con_clock = {"con_clock", "1", 1};
...
int char_texture;
int glow_texture_object;
int normcube_texture_object;
...
int gl_lightmap_format = 0x1907;
int gl_solid_format = 0x1907;
int gl_alpha_format = 0x8057;

int gl_filter_min = 0x2701;
int gl_filter_max = 0x2601;
...
void GL_Bind (int texnum)
{
 ...
 glBindTexture(0x0DE1, texnum);
# 123 "d:/Data/Nintendo/TenebraeGX/src/gl_draw.c"
}
# 141 "d:/Data/Nintendo/TenebraeGX/src/gl_draw.c"
int scrap_allocated[2][256];
byte scrap_texels[2][256*256*4];
...
void Scrap_Upload (void)
{
 ...
 for (texnum=0 ; texnum<2 ; texnum++) {
  ...
  GL_Upload8 (scrap_texels[texnum], "scrap", 256, 256, 0, 1, 0);
 }
 scrap_dirty = 0;
}
 ...
 if (p->width < 64 && p->height < 64)
 {
  ...
  for (i=0 ; i<p->height ; i++)
   for (j=0 ; j<p->width ; j++, k++)
    scrap_texels[texnum][(y+i)*256 + x + j] = p->data[k];
  texnum += scrap_texnum;
  gl->texnum = texnum;
  gl->sl = (x+0.01)/(float)256;
  gl->sh = (x+p->width-0.01)/(float)256;
  gl->tl = (y+0.01)/(float)256;
  gl->th = (y+p->height-0.01)/(float)256;
...
void Draw_TextureMode_f (void)
{
 ...
 for (i=0, glt=gltextures ; i<numgltextures ; i++, glt++)
 {
  ...
  {
   GL_Bind (glt->texnum);
   glTexParameterf(0x0DE1, 0x2801, gl_filter_min);
   glTexParameterf(0x0DE1, 0x2800, gl_filter_max);
...
void Draw_Init (void)
{
 ...
 char_texture = GL_LoadTexture ("charset", 128, 128, draw_chars, 0, 1, 0);
 ...
 sprintf (ver, "(st %4.2f) %4.2f", (float)1.04, (float)1.09);

 dest = cb->data + 320*186 + 320 - 11 - 8*strlen(ver);
 y = strlen(ver);
 for (x=0 ; x<y ; x++)
  Draw_CharToConback (ver[x], dest+(x<<3));
# 500 "d:/Data/Nintendo/TenebraeGX/src/gl_draw.c"
 conback->width = cb->width;
 conback->height = cb->height;
 ncdata = cb->data;


 glTexParameterf(0x0DE1, 0x2801, 0x2600);
 glTexParameterf(0x0DE1, 0x2800, 0x2600);

 gl = (glpic_t *)conback->data;
 gl->texnum = GL_LoadTexture ("conback", conback->width, conback->height, ncdata, 0, 0, 0);
 gl->sl = 0;
 gl->sh = 1;
 gl->tl = 0;
 gl->th = 1;
 ...
 SHADOW_InitTex(vid.width, vid.height);
...
# 577 "d:/Data/Nintendo/TenebraeGX/src/gl_draw.c"
void Draw_Character (int x, int y, int num)
{
 int row, col;
...
# 637 "d:/Data/Nintendo/TenebraeGX/src/gl_draw.c"
void Draw_DebugChar (char num)
{
    (void)num;
}
...
# 727 "d:/Data/Nintendo/TenebraeGX/src/gl_draw.c"
void Draw_TransPicTranslate (int x, int y, qpic_t *pic, byte *translation)
{
 int v, u, c;
 ...
 glTexImage2D (0x0DE1, 0, gl_alpha_format, 64, 64, 0, 0x1908, 0x1401, trans);
...
# 779 "d:/Data/Nintendo/TenebraeGX/src/gl_draw.c"
void Draw_ConsoleBackground (int lines)
{
...
# 852 "d:/Data/Nintendo/TenebraeGX/src/gl_draw.c"
void Draw_TileClear (int x, int y, int w, int h)
{
 glColor3f (1,1,1);
...
# 876 "d:/Data/Nintendo/TenebraeGX/src/gl_draw.c"
void Draw_Fill (int x, int y, int w, int h, int c)
{
 glDisable (0x0DE1);
...
# 902 "d:/Data/Nintendo/TenebraeGX/src/gl_draw.c"
void Draw_FadeScreen (void)
{
...
# 941 "d:/Data/Nintendo/TenebraeGX/src/gl_draw.c"
void Draw_BeginDisc (void)
{
 if (!draw_disc)
  return;
...
# 959 "d:/Data/Nintendo/TenebraeGX/src/gl_draw.c"
void Draw_EndDisc (void)
{
}
# 970 "d:/Data/Nintendo/TenebraeGX/src/gl_draw.c"
void GL_Set2D (void)
{
...
# 1000 "d:/Data/Nintendo/TenebraeGX/src/gl_draw.c"
int GL_FindTexture (char *identifier)
{
 int i;
 gltexture_t *glt;
...
# 1021 "d:/Data/Nintendo/TenebraeGX/src/gl_draw.c"
...
void GL_ResampleTexture (void *indata, int inwidth, int inheight, void *outdata, int outwidth, int outheight)
{
 ...
 free(row1);
 free(row2);
# 1177 "d:/Data/Nintendo/TenebraeGX/src/gl_draw.c"
}
...
# 1219 "d:/Data/Nintendo/TenebraeGX/src/gl_draw.c"
...
# 1247 "d:/Data/Nintendo/TenebraeGX/src/gl_draw.c"
...
# 1272 "d:/Data/Nintendo/TenebraeGX/src/gl_draw.c"
...
# 1375 "d:/Data/Nintendo/TenebraeGX/src/gl_draw.c"
...
# 1409 "d:/Data/Nintendo/TenebraeGX/src/gl_draw.c"
void GL_PackGloss(byte *gloss,unsigned *dest,int length)
{
    ...
    {
        *dest = *dest & LittleLong (0x00FFFFFF);
        *dest = *dest | LittleLong (*gloss << 24);
    }
}
...
void GL_Upload32 (unsigned *data, int width, int height, qboolean mipmap, qboolean alpha)
{
 ...
 const int maxsize = 512 * 256;
 unsigned *scaled = malloc(maxsize*sizeof(unsigned));
 if (!scaled) return;
 ...
 if (scaled_height > gl_max_size.value/2)
  scaled_height = gl_max_size.value/2;

 if (scaled_width * scaled_height > maxsize)
...
# 1505 "d:/Data/Nintendo/TenebraeGX/src/gl_draw.c"
texels += scaled_width * scaled_height;
 ...
 if (scaled_width == width && scaled_height == height)
 {
  if (!mipmap)
  {
   glTexImage2D (0x0DE1, 0, texturemode, scaled_width, scaled_height, 0, 0x1908, 0x1401, data);
   goto done;
  }
 ...
 glTexImage2D (0x0DE1, 0, texturemode, scaled_width, scaled_height, 0, 0x1908, 0x1401, scaled);
 if (mipmap)
 {
  int miplevel;
  ...
  while (scaled_width > 1 || scaled_height > 1)
  {
   ...
   if (scaled_height < 1)
    scaled_height = 1;
   miplevel++;
   glTexImage2D (0x0DE1, miplevel, texturemode, scaled_width, scaled_height, 0, 0x1908, 0x1401, scaled);
                }
 }
done: ;
 ...
 {
  glTexParameterf(0x0DE1, 0x2801, gl_filter_max);
  glTexParameterf(0x0DE1, 0x2800, gl_filter_max);
 }
...
void GL_Upload8_EXT_ (byte *data, int width, int height, qboolean mipmap, qboolean alpha)
{
 int i, s;
 qboolean noalpha;
 int samples;
 ...
 const int maxsize = 512 * 256;
 unsigned char *scaled = malloc(maxsize);
 if (!scaled) return;
 ...
 if (scaled_width == width && scaled_height == height)
 {
  if (!mipmap)
  {
   glTexImage2D (0x0DE1, 0, 0x80E5, scaled_width, scaled_height, 0, 0x1900 , 0x1401, data);
   goto done;
  }
 ...
 glTexImage2D (0x0DE1, 0, 0x80E5, scaled_width, scaled_height, 0, 0x1900, 0x1401, scaled);
 if (mipmap)
 {
  int miplevel;
  ...
  while (scaled_width > 1 || scaled_height > 1)
  {
   ...
   if (scaled_height < 1)
    scaled_height = 1;
   miplevel++;
   glTexImage2D (0x0DE1, miplevel, 0x80E5, scaled_width, scaled_height, 0, 0x1900, 0x1401, scaled);
  }
 }
done: ;
...
unsigned int * genNormalMap(byte *pixels, int w, int h, float scale)
{
  ...
  for (i=0; i<h; i++) {
    for (j=0; j<w; j++) {
      ...
      nmap[i*w+j] = LittleLong ((255 << 24)|(b << 16)|(g << 8)|(r));
    }
  }
...
void GL_UploadBump(byte *data, int width, int height, qboolean mipmap, byte* gloss)
{
  ...
  const int maxsize = 512 * 256;
 unsigned char *scaled = malloc(maxsize);
    unsigned char *scaledgloss = malloc(maxsize);
 if (!scaledgloss) {
  free(scaled);
  return;
 } else if (!scaled) return;
 ...
 ;
    ...
    if (scaled_width > gl_max_size.value/2)
  scaled_width = gl_max_size.value/2;
 if (scaled_height > gl_max_size.value/2)
  scaled_height = gl_max_size.value/2;
 ...
 glTexParameteri(0x0DE1, 0x2802, 0x2901);
 glTexParameteri(0x0DE1, 0x2803, 0x2901);
    glTexImage2D (0x0DE1, 0, texturemode, scaled_width, scaled_height, 0,
    0x1908, 0x1401, nmap);
 ...
 while (scaled_width > 1 || scaled_height > 1)
 {
  ...
  scaled_height = 1;
     ...
     glTexImage2D (0x0DE1, miplevel, texturemode, scaled_width, scaled_height, 0, 0x1908,
     0x1401, nmap);
 }
    }
...
void GL_UploadNormal(unsigned int *data, int width, int height, qboolean mipmap)
{
 ...
 const int maxsize = 512*256;
 unsigned int *scaled = malloc(maxsize*sizeof(unsigned int));
 if (!scaled) return;
...
# 1972 "d:/Data/Nintendo/TenebraeGX/src/gl_draw.c"
void GL_GetOverrideName(char *identifier,char *tail, char *dest) {
 unsigned i;
...
# 1994 "d:/Data/Nintendo/TenebraeGX/src/gl_draw.c"
static unsigned trans[512*512];
...
void GL_Upload8 (byte *data,char *identifier, int width, int height, qboolean mipmap, qboolean alpha, qboolean bump)
{
 ...
 unsigned char *bumppix = malloc(512*512);
 unsigned char *glosspix = malloc(512*512);
 if (!glosspix) {
  free(bumppix);
  return;
 } else if (!bumppix) return;
 ...
 if ( LoadTextureInPlace(filename, 4, (unsigned char*)&trans[0], &width, &height) )
 {
  ...
  for (i=0; i<width*height; i++) {
   r = (trans[i] & 0x00FF0000) >> 16;
   g = (trans[i] & 0x0000FF00) >> 8;
   b = (trans[i] & 0x000000FF);
   bumppix[i] = (r+g+b)/3;
  }
 }
 ...
 if (!strcmp("scrap",identifier)) {
  ...
  if (!COM_CheckParm("-nobumpmaps") && ( sh_bumpmaps.value ) && bump )
  {
   ...
   else
   {
    ...
    if ( LoadTextureInPlace(filename, 1, (byte *) &bumppix[0], &width, &height) )
    {
     char filename[128];
     int gloss_width, gloss_height;
     Con_DPrintf("Overriding bumpmap for %s\n",identifier);
     GL_GetOverrideName(identifier,"_gloss",filename);
     if ( LoadTextureInPlace(filename, 1, (byte *) &glosspix[0], &gloss_width, &gloss_height) )
     {
      Con_DPrintf("Using gloss map for %s\n",identifier);
     }
     else
...
if (glGetError() == 0x0505) {
 Sys_Printf("Out of memory loading %s\n", identifier);
}
...
int GL_LoadCubeMap_ (int identifier)
{
# 2346 "d:/Data/Nintendo/TenebraeGX/src/gl_draw.c"
 return 0;
...
static GLenum oldtarget = 0x84C0;
...
void GL_SelectTexture (GLenum target)
{
 ...
 if (target == oldtarget)
  return;
 cnttextures[oldtarget-0x84C0] = currenttexture;
 currenttexture = cnttextures[target-0x84C0];
 oldtarget = target;
}
...
# 2390 "d:/Data/Nintendo/TenebraeGX/src/gl_draw.c"
int GL_Load2DAttenTexture(void)
{
 ...
 glTexImage2D(0x0DE1, 0, 0x1906, 64, 64,
     0, 0x1906, 0x1401, data);
...
# 2450 "d:/Data/Nintendo/TenebraeGX/src/gl_draw.c"
int GL_Load3DAttenTexture(void)
{
# 2516 "d:/Data/Nintendo/TenebraeGX/src/gl_draw.c"
 return 0;

}
# 2528 "d:/Data/Nintendo/TenebraeGX/src/gl_draw.c"
...
{
# 2612 "d:/Data/Nintendo/TenebraeGX/src/gl_draw.c"
 return 0;
