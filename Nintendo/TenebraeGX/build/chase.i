# 1 "d:/Data/Nintendo/TenebraeGX/src/chase.c"
# 1 "d:\\Data\\Nintendo\\TenebraeGX\\build//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "d:/Data/Nintendo/TenebraeGX/src/chase.c"
# 22 "d:/Data/Nintendo/TenebraeGX/src/chase.c"
...
# 23 "d:/Data/Nintendo/TenebraeGX/src/chase.c" 2
...
void TraceLine (vec3_t start, vec3_t end, vec3_t impact)
{
 ...
 {impact[0]=trace.endpos[0];impact[1]=trace.endpos[1];impact[2]=trace.endpos[2];};
}
...
void Chase_Update (void)
{
 ...
 {stop[0]=stop[0]-r_refdef.vieworg[0];stop[1]=stop[1]-r_refdef.vieworg[1];stop[2]=stop[2]-r_refdef.vieworg[2];};
 dist = (stop[0]*forward[0]+stop[1]*forward[1]+stop[2]*forward[2]);
 if (dist < 1)
  dist = 1;
 r_refdef.viewangles[0] = -atan(stop[2] / dist) / 3.14159265358979323846 * 180;

 TraceLine(r_refdef.vieworg, chase_dest, stop);
 if (Length(stop) != 0)
  {chase_dest[0]=stop[0];chase_dest[1]=stop[1];chase_dest[2]=stop[2];};


 {r_refdef.vieworg[0]=chase_dest[0];r_refdef.vieworg[1]=chase_dest[1];r_refdef.vieworg[2]=chase_dest[2];};
}
