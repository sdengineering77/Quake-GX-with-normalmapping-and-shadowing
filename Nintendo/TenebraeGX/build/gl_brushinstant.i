# 1 "d:/Data/Nintendo/TenebraeGX/src/gl_brushinstant.c"
# 1 "d:\\Data\\Nintendo\\TenebraeGX\\build//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "d:/Data/Nintendo/TenebraeGX/src/gl_brushinstant.c"
# 25 "d:/Data/Nintendo/TenebraeGX/src/gl_brushinstant.c"
...
# 26 "d:/Data/Nintendo/TenebraeGX/src/gl_brushinstant.c" 2
# 34 "d:/Data/Nintendo/TenebraeGX/src/gl_brushinstant.c"
brushlightinstant_t *BLightInstantCache;
...
void BINST_InitMem(void) {
 ...
 Sys_Printf("Allocated BLightInstantCache at %p\n", low);
 BLightInstantCache = low;
 low += sizeof(brushlightinstant_t) * 24;
 Sys_Printf("End at %p\n", low);
...
void R_ClearBrushInstantCaches() {
 ...
 for (i=0; i<24; i++) {
  ...
  BLightInstantCache[i].lastent = ((void *)0);
 }
}
...
# 163 "d:/Data/Nintendo/TenebraeGX/src/gl_brushinstant.c"
 ...
 if ((ins->lastent == e) &&
  ...
  (bdist(ins->lastlorg,currentshadowlight->origin) < 0.1) &&
  (bdist(ins->lasteorg,e->origin) < 0.1) &&
  (bdist(ins->lasteangles,e->angles) < 0.5) &&
  (fabs(ins->lastlradius - currentshadowlight->radius) <= 0.1) &&
...
qboolean R_IsVisibleSurf(msurface_t *surf, brushlightinstant_t *ins)
{
 ...
 case 2:
  ...
  break;
 default:
  dist = (ins->lightpos[0]*plane->normal[0]+ins->lightpos[1]*plane->normal[1]+ins->lightpos[2]*plane->normal[2]) - plane->dist;
  break;
 }
...
void R_CalcBrushVolumeVerts(entity_t *e, brushlightinstant_t *ins) {
 ...
 for (i=0; i<model->nummodelsurfaces; i++, surf++)
 {
  ...
  for (j=0 ; j<surf->numedges ; j++)
  {
   ...
   {v1[0]=(*v2)[0]-ins->lightpos[0];v1[1]=(*v2)[1]-ins->lightpos[1];v1[2]=(*v2)[2]-ins->lightpos[2];};
   scale = Length (v1);
   ...
   {ins->extvertices[extrvertcount+j][0]=v1[0]+(*v2)[0];ins->extvertices[extrvertcount+j][1]=v1[1]+(*v2)[1];ins->extvertices[extrvertcount+j][2]=v1[2]+(*v2)[2];};
  }
...
void R_CalcBrushAttenCoords(entity_t *e, brushlightinstant_t *ins) {
 ...
 for (i=0 ; i<e->model->nummodelsurfaces ; i++, psurf++)
 {
  ...
  {s[0]=psurf->texinfo->vecs[0][0];s[1]=psurf->texinfo->vecs[0][1];s[2]=psurf->texinfo->vecs[0][2];};
  {t[0]=psurf->texinfo->vecs[1][0];t[1]=psurf->texinfo->vecs[1][1];t[2]=psurf->texinfo->vecs[1][2];};
  ...
  case 2:
   ...
   break;
  default:
   dist = (ins->lightpos[0]*splitplane->normal[0]+ins->lightpos[1]*splitplane->normal[1]+ins->lightpos[2]*splitplane->normal[2]) - splitplane->dist;
   break;
  }
  ...
  for (j=0 ; j<poly->numverts ; j++, v+= 7)
  {
   ...
   ins->atencoords[usedattencoords][0] = (nearToVert[0]*s[0]+nearToVert[1]*s[1]+nearToVert[2]*s[2]) * scale + 0.5;
   ins->atencoords[usedattencoords][1] = (nearToVert[0]*t[0]+nearToVert[1]*t[1]+nearToVert[2]*t[2]) * scale + 0.5;
   usedattencoords++;
  }
 }
}
...
void R_SetupBrushLightHAV(entity_t *ent, brushlightinstant_t *ins)
{
 ...
 {lightOr[0]=ins->lightpos[0];lightOr[1]=ins->lightpos[1];lightOr[2]=ins->lightpos[2];};
 ...
 for (i=0 ; i<ent->model->nummodelsurfaces ; i++, psurf++)
 {
  ...
  for (j=0 ; j<poly->numverts ; j++, v+= 7)
  {
   ...
   {lightDir[0]=lightOr[0]-v[0];lightDir[1]=lightOr[1]-v[1];lightDir[2]=lightOr[2]-v[2];};


   if (psurf->flags & 2) {
    ins->tslights[usedts][2] = -(lightDir[0]*psurf->plane->normal[0]+lightDir[1]*psurf->plane->normal[1]+lightDir[2]*psurf->plane->normal[2]);
   } else {
    ins->tslights[usedts][2] = (lightDir[0]*psurf->plane->normal[0]+lightDir[1]*psurf->plane->normal[1]+lightDir[2]*psurf->plane->normal[2]);
   }
   ins->tslights[usedts][1] = -(lightDir[0]*psurf->texinfo->vecs[1][0]+lightDir[1]*psurf->texinfo->vecs[1][1]+lightDir[2]*psurf->texinfo->vecs[1][2]);
   ins->tslights[usedts][0] = (lightDir[0]*psurf->texinfo->vecs[0][0]+lightDir[1]*psurf->texinfo->vecs[0][1]+lightDir[2]*psurf->texinfo->vecs[0][2]);


   VectorNormalize(lightDir);

   {H[0]=ins->vieworg[0]-v[0];H[1]=ins->vieworg[1]-v[1];H[2]=ins->vieworg[2]-v[2];};
   VectorNormalize(H);
   {H[0]=lightDir[0]+H[0];H[1]=lightDir[1]+H[1];H[2]=lightDir[2]+H[2];};


   if (psurf->flags & 2) {
    ins->tshalfangles[usedts][2] = -(H[0]*psurf->plane->normal[0]+H[1]*psurf->plane->normal[1]+H[2]*psurf->plane->normal[2]);
   } else {
    ins->tshalfangles[usedts][2] = (H[0]*psurf->plane->normal[0]+H[1]*psurf->plane->normal[1]+H[2]*psurf->plane->normal[2]);
   }

   ins->tshalfangles[usedts][1] = -(H[0]*psurf->texinfo->vecs[1][0]+H[1]*psurf->texinfo->vecs[1][1]+H[2]*psurf->texinfo->vecs[1][2]);
   ins->tshalfangles[usedts][0] = (H[0]*psurf->texinfo->vecs[0][0]+H[1]*psurf->texinfo->vecs[0][1]+H[2]*psurf->texinfo->vecs[0][2]);
...
void R_SetupBrushInstantForLight(entity_t *e)
{
 ...
 if (R_CullBox (mins, maxs))
  brushlightinstant->shadowonly = 1;
 else
  brushlightinstant->shadowonly = 0;
 ...
 if (update)
 {
  ...
  {brushlightinstant->lasteorg[0]=e->origin[0];brushlightinstant->lasteorg[1]=e->origin[1];brushlightinstant->lasteorg[2]=e->origin[2];};
  {brushlightinstant->lastlorg[0]=currentshadowlight->origin[0];brushlightinstant->lastlorg[1]=currentshadowlight->origin[1];brushlightinstant->lastlorg[2]=currentshadowlight->origin[2];};
  {brushlightinstant->lasteangles[0]=e->angles[0];brushlightinstant->lasteangles[1]=e->angles[1];brushlightinstant->lasteangles[2]=e->angles[2];};
 ...
 if ((update) || CheckBrushHalfAngle(brushlightinstant)) {
  ...
  {brushlightinstant->lastvorg[0]=r_refdef.vieworg[0];brushlightinstant->lastvorg[1]=r_refdef.vieworg[1];brushlightinstant->lastvorg[2]=r_refdef.vieworg[2];};
