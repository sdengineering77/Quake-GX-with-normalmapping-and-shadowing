# 1 "d:/Data/Nintendo/TenebraeGX/src/gl_aliasinstant.c"
# 1 "d:\\Data\\Nintendo\\TenebraeGX\\build//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "d:/Data/Nintendo/TenebraeGX/src/gl_aliasinstant.c"
# 37 "d:/Data/Nintendo/TenebraeGX/src/gl_aliasinstant.c"
...
# 38 "d:/Data/Nintendo/TenebraeGX/src/gl_aliasinstant.c" 2
# 50 "d:/Data/Nintendo/TenebraeGX/src/gl_aliasinstant.c"
aliasframeinstant_t *InstantCache;
aliaslightinstant_t *LightInstantCache;


void R_SetupInstantForFrame(entity_t *e, qboolean forcevis);
# 64 "d:/Data/Nintendo/TenebraeGX/src/gl_aliasinstant.c"
...
void AINST_InitMem(void) {
 ...
 void *low = align32((void *) SYS_GetArena2Lo());
 Sys_Printf("Allocated InstantCache at %p\n", low);
 InstantCache = low;
 low += sizeof(aliasframeinstant_t) * 24;
 Sys_Printf("End at %p\n", low);

 low = align32(low);
 Sys_Printf("Allocated LightInstantCache at %p\n", low);
 LightInstantCache = low;
 low += sizeof(aliaslightinstant_t) * 24;
 Sys_Printf("End at %p\n", low);
...
aliaslightinstant_t *R_AllocateLightInstant(entity_t *e, aliashdr_t * paliashdr) {
 ...
 int i, oldest, oindex;
# 137 "d:/Data/Nintendo/TenebraeGX/src/gl_aliasinstant.c"
 for (i=0; i<24; i++) {
 ...
 LightInstantCache[oindex].lastent = ((void *)0);
 LightInstantCache[oindex].lastframeinstant = ((void *)0);
 LightInstantCache[oindex].lastlight = ((void *)0);
...
void R_ClearInstantCaches() {
 ...
 for (i=0; i<24; i++) {
  ...
  InstantCache[i].lastent = ((void *)0);
  InstantCache[i].paliashdr = ((void *)0);
 }

 for (i=0; i<24; i++) {
  LightInstantCache[i].lockframe = 0;
  LightInstantCache[i].lastent = ((void *)0);
  LightInstantCache[i].lastlight = ((void *)0);
  LightInstantCache[i].lastframeinstant = ((void *)0);
 }
}
...
void R_SetupInstants ()
{
 ...
 for (i=0 ; i<cl_numvisedicts ; i++)
 {
  ...
  if (currententity->model->type == mod_alias) {
   R_SetupInstantForFrame(currententity,0);
  }
 }
 ...
 if (cl.viewent.model && cl.viewent.model->type == mod_alias)
  R_SetupInstantForFrame(&cl.viewent,1);


 if (cl_entities[cl.viewentity].model && cl_entities[cl.viewentity].model->type == mod_alias)
  R_SetupInstantForFrame(&cl_entities[cl.viewentity],1);

}
# 233 "d:/Data/Nintendo/TenebraeGX/src/gl_aliasinstant.c"
...
void R_InterpolateNormals(aliashdr_t *paliashdr, aliasframeinstant_t *instant, int pose1, int pose2, float blend) {
 ...
 for (i=0; i<paliashdr->poseverts; i++) {
  ...
  lat *= (float)3.14159265358979323846/128;
  lng *= (float)3.14159265358979323846/128;
...
# 395 "d:/Data/Nintendo/TenebraeGX/src/gl_aliasinstant.c"
...
# 421 "d:/Data/Nintendo/TenebraeGX/src/gl_aliasinstant.c"
...
qboolean CheckUpdate(entity_t *e, aliasframeinstant_t *ins) {
 ...
 if (sh_nocache.value) return 1;

 if ((ins->lastent == e) &&
  (ins->lastpose1 == e->pose1) &&
  (ins->lastpose2 == e->pose2) &&
  ( (fabs(ins->lastblend - e->blend) <= 0.0000001) || (e->pose1 == e->pose2) ) &&
  (ins->lastshadowonly == ins->shadowonly) &&
  (ins->lastpaliashdr == ins->paliashdr) &&
  (ins->lockframe >= r_framecount-10))
 {
  return 0;
 } else {
  return 1;
...
void R_SetupAliasInstantForFrame(entity_t *e, qboolean forcevis, aliashdr_t *paliashdr, aliasframeinstant_t *aliasframeinstant)
{
 ...
 if (!forcevis) {
  ...
  if (e->angles[0] || e->angles[1] || e->angles[2])
  {
   ...
   {mins[0]=e->origin[0]+e->model->mins[0];mins[1]=e->origin[1]+e->model->mins[1];mins[2]=e->origin[2]+e->model->mins[2];};
   {maxs[0]=e->origin[0]+e->model->maxs[0];maxs[1]=e->origin[1]+e->model->maxs[1];maxs[2]=e->origin[2]+e->model->maxs[2];};
  }

  if (R_CullBox (mins, maxs))
   aliasframeinstant->shadowonly = 1;
  else
   aliasframeinstant->shadowonly = 0;
 } else aliasframeinstant->shadowonly = 0;
...
# 591 "d:/Data/Nintendo/TenebraeGX/src/gl_aliasinstant.c"
...
void R_SetupLightHAV(aliasframeinstant_t *instant, aliaslightinstant_t *linstant) {
 ...
 for (i=0; i<instant->paliashdr->poseverts; i++) {
  ...
  {tz[0]=instant->normals[i][0];tz[1]=instant->normals[i][1];tz[2]=instant->normals[i][2];};
  {ty[0]=instant->tangents[i][0];ty[1]=instant->tangents[i][1];ty[2]=instant->tangents[i][2];};
  {tx[0]=instant->binomials[i][0];tx[1]=instant->binomials[i][1];tx[2]=instant->binomials[i][2];};


  {lightDir[0]=linstant->lightpos[0]-instant->vertices[i][0];lightDir[1]=linstant->lightpos[1]-instant->vertices[i][1];lightDir[2]=linstant->lightpos[2]-instant->vertices[i][2];};

  linstant->tslights[i][0] = (lightDir[0]*tx[0]+lightDir[1]*tx[1]+lightDir[2]*tx[2]);
  linstant->tslights[i][1] = -(lightDir[0]*ty[0]+lightDir[1]*ty[1]+lightDir[2]*ty[2]);
  linstant->tslights[i][2] = (lightDir[0]*tz[0]+lightDir[1]*tz[1]+lightDir[2]*tz[2]);


  VectorNormalize(lightDir);
  {H[0]=linstant->vieworg[0]-instant->vertices[i][0];H[1]=linstant->vieworg[1]-instant->vertices[i][1];H[2]=linstant->vieworg[2]-instant->vertices[i][2];};
  VectorNormalize(H);
  {H[0]=lightDir[0]+H[0];H[1]=lightDir[1]+H[1];H[2]=lightDir[2]+H[2];};

  linstant->tshalfangles[i][0] = (H[0]*tx[0]+H[1]*tx[1]+H[2]*tx[2]);
  linstant->tshalfangles[i][1] = -(H[0]*ty[0]+H[1]*ty[1]+H[2]*ty[2]);
  linstant->tshalfangles[i][2] = (H[0]*tz[0]+H[1]*tz[1]+H[2]*tz[2]);
 }
}

extern int extrudeTimeStamp;
extern int extrudedTimestamp[2048];
...
void R_CalcVolumeVerts(aliasframeinstant_t *instant, aliaslightinstant_t *linstant) {
 ...
 for (i=0; i<paliashdr->numtris; i++) {
  d = (instant->triplanes[i].normal[0]*linstant->lightpos[0]+instant->triplanes[i].normal[1]*linstant->lightpos[1]+instant->triplanes[i].normal[2]*linstant->lightpos[2]) - instant->triplanes[i].dist;
  if (d > 0)
   linstant->triangleVis[i] = 1;
  else
   linstant->triangleVis[i] = 0;
 }
 ...
 for (i=0; i<paliashdr->numtris; i++, triangle++) {
  ...
  if (linstant->triangleVis[i]) {
   ...
   for (j=0; j<3; j++) {
    ...
    {v2[0]=instant->vertices[index][0];v2[1]=instant->vertices[index][1];v2[2]=instant->vertices[index][2];};

    {(*v1)[0]=v2[0]-linstant->lightpos[0];(*v1)[1]=v2[1]-linstant->lightpos[1];(*v1)[2]=v2[2]-linstant->lightpos[2];};
    scale = Length ((*v1));
...
void R_CalcAttenColors(aliasframeinstant_t *instant, aliaslightinstant_t *linstant)
{
 ...
 for (i=0; i<instant->paliashdr->poseverts; i++) {
  ...
  {diff[0]=*v[0]-linstant->lightpos[0];diff[1]=*v[1]-linstant->lightpos[1];diff[2]=*v[2]-linstant->lightpos[2];};
...
void R_CalcIndeciesForLight(aliasframeinstant_t *instant, aliaslightinstant_t *linstant) {
 ...
 tris = (mtriangle_t *)((byte *)paliashdr + paliashdr->triangles);
# 751 "d:/Data/Nintendo/TenebraeGX/src/gl_aliasinstant.c"
 j = 0;
 linstant->numtris = 0;
 for (i=0; i<paliashdr->numtris; i++) {
  if (!linstant->triangleVis[i]) {
...
float dist(vec3_t v1, vec3_t v2) {
 vec3_t diff;
 {diff[0]=v1[0]-v2[0];diff[1]=v1[1]-v2[1];diff[2]=v1[2]-v2[2];};
 return Length(diff);
}
...
qboolean CheckLightUpdate(entity_t *e, aliashdr_t *paliashdr, aliaslightinstant_t *ins, aliasframeinstant_t *aliasframeinstant) {
 ...
 if ((ins->lastent == e) &&
  ...
  (dist(ins->lastlorg,currentshadowlight->origin) < 0.1) &&
  (dist(ins->lasteorg,e->origin) < 0.1) &&
  (dist(ins->lasteangles,e->angles) < 0.5) &&
  (abs(ins->lastlradius - currentshadowlight->radius) <= 0.1) &&
...
void R_SetupSurfaceInstantForLight(entity_t *e,aliashdr_t *paliashdr, aliasframeinstant_t *aliasframeinstant)
{
 ...
 if (update)
 {
  R_CalcVolumeVerts(aliasframeinstant, aliaslightinstant);
   ...
   {aliaslightinstant->lasteorg[0]=e->origin[0];aliaslightinstant->lasteorg[1]=e->origin[1];aliaslightinstant->lasteorg[2]=e->origin[2];};
   {aliaslightinstant->lastlorg[0]=currentshadowlight->origin[0];aliaslightinstant->lastlorg[1]=currentshadowlight->origin[1];aliaslightinstant->lastlorg[2]=currentshadowlight->origin[2];};
   {aliaslightinstant->lasteangles[0]=e->angles[0];aliaslightinstant->lasteangles[1]=e->angles[1];aliaslightinstant->lasteangles[2]=e->angles[2];};
 ...
 if ((update) || CheckHalfAngle(aliaslightinstant)) {
  ...
  {aliaslightinstant->lastvorg[0]=r_refdef.vieworg[0];aliaslightinstant->lastvorg[1]=r_refdef.vieworg[1];aliaslightinstant->lastvorg[2]=r_refdef.vieworg[2];};
