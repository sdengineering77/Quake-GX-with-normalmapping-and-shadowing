# 1 "d:/Data/Nintendo/TenebraeGX/src/sv_move.c"
# 1 "d:\\Data\\Nintendo\\TenebraeGX\\build//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "d:/Data/Nintendo/TenebraeGX/src/sv_move.c"
# 22 "d:/Data/Nintendo/TenebraeGX/src/sv_move.c"
...
# 23 "d:/Data/Nintendo/TenebraeGX/src/sv_move.c" 2
# 35 "d:/Data/Nintendo/TenebraeGX/src/sv_move.c"
int c_yes, c_no;
...
qboolean SV_CheckBottom (edict_t *ent)
{
 ...
 {mins[0]=ent->v.origin[0]+ent->v.mins[0];mins[1]=ent->v.origin[1]+ent->v.mins[1];mins[2]=ent->v.origin[2]+ent->v.mins[2];};
 {maxs[0]=ent->v.origin[0]+ent->v.maxs[0];maxs[1]=ent->v.origin[1]+ent->v.maxs[1];maxs[2]=ent->v.origin[2]+ent->v.maxs[2];};
...
# 110 "d:/Data/Nintendo/TenebraeGX/src/sv_move.c"
qboolean SV_movestep (edict_t *ent, vec3_t move, qboolean relink)
{
 float dz;
 ...
 {neworg[0]=ent->v.origin[0]+move[0];neworg[1]=ent->v.origin[1]+move[1];neworg[2]=ent->v.origin[2]+move[2];};


 if ( (int)ent->v.flags & (2 | 1) )
 {

  for (i=0 ; i<2 ; i++)
  {
   {neworg[0]=ent->v.origin[0]+move[0];neworg[1]=ent->v.origin[1]+move[1];neworg[2]=ent->v.origin[2]+move[2];};
   enemy = ((edict_t *)((byte *)sv.edicts + ent->v.enemy));
   if (i == 0 && enemy != sv.edicts)
   {
    dz = ent->v.origin[2] - ((edict_t *)((byte *)sv.edicts + ent->v.enemy))->v.origin[2];
    if (dz > 40)
     neworg[2] -= 8;
    if (dz < 30)
     neworg[2] += 8;
   ...
   if (trace.fraction == 1)
   {
    if ( ((int)ent->v.flags & 2) && SV_PointContents(trace.endpos) == -1 )
     return 0;
 ...
 neworg[2] += 18;
 {end[0]=neworg[0];end[1]=neworg[1];end[2]=neworg[2];};
 end[2] -= 18*2;
 ...
 if (trace.fraction == 1)
 {
  ...
  if ( (int)ent->v.flags & 1024 )
  {
   {ent->v.origin[0]=ent->v.origin[0]+move[0];ent->v.origin[1]=ent->v.origin[1]+move[1];ent->v.origin[2]=ent->v.origin[2]+move[2];};
   if (relink)
    SV_LinkEdict (ent, 1);
 ...
 if ( (int)ent->v.flags & 1024 )
 {
  ...
  ent->v.flags = (int)ent->v.flags & ~1024;
 }
...
# 232 "d:/Data/Nintendo/TenebraeGX/src/sv_move.c"
...
qboolean SV_StepDirection (edict_t *ent, float yaw, float dist)
{
 ...
 {oldorigin[0]=ent->v.origin[0];oldorigin[1]=ent->v.origin[1];oldorigin[2]=ent->v.origin[2];};
 if (SV_movestep (ent, move, 0))
 {
  delta = ent->v.angles[1] - ent->v.ideal_yaw;
  if (delta > 45 && delta < 315)
  {
   {ent->v.origin[0]=oldorigin[0];ent->v.origin[1]=oldorigin[1];ent->v.origin[2]=oldorigin[2];};
  }
  SV_LinkEdict (ent, 1);
  return 1;
 }
...
# 284 "d:/Data/Nintendo/TenebraeGX/src/sv_move.c"
...
void SV_MoveToGoal (void)
{
 ...
 goal = ((edict_t *)((byte *)sv.edicts + ent->v.goalentity));
 dist = (pr_globals[4]);
 ...
 if ( ((edict_t *)((byte *)sv.edicts + ent->v.enemy)) != sv.edicts && SV_CloseEnough (ent, goal, dist) )
