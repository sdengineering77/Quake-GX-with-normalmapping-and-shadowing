# 1 "d:/Data/Nintendo/TenebraeGX/src/gl_svbsp.c"
# 1 "d:\\Data\\Nintendo\\TenebraeGX\\build//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "d:/Data/Nintendo/TenebraeGX/src/gl_svbsp.c"
# 24 "d:/Data/Nintendo/TenebraeGX/src/gl_svbsp.c"
...
# 25 "d:/Data/Nintendo/TenebraeGX/src/gl_svbsp.c" 2
...
# 39 "d:/Data/Nintendo/TenebraeGX/src/gl_svbsp.c"
plane_t PlanePool[65536/2];
int planesusedpool;

svnode_t NodePool[65536];
int nodesusedpool;
...
# 103 "d:/Data/Nintendo/TenebraeGX/src/gl_svbsp.c"
svnode_t *R_CreateEmptyTree(void) {
 ...
 plane1 = AllocPlane (((void *)0));
 plane2 = AllocPlane (((void *)0));
 node1 = AllocNode ();
 node2 = AllocNode ();
 ...
 plane1->normal[1] = 0;
 plane1->normal[2] = 0;
 plane1->dist = (currentshadowlight->origin[0]*plane1->normal[0]+currentshadowlight->origin[1]*plane1->normal[1]+currentshadowlight->origin[2]*plane1->normal[2]);

 plane2->normal[0] = -1;
 plane2->normal[1] = 0;
 plane2->normal[2] = 0;
 plane2->dist = (currentshadowlight->origin[0]*plane2->normal[0]+currentshadowlight->origin[1]*plane2->normal[1]+currentshadowlight->origin[2]*plane2->normal[2]);


 node1->splitplane = plane1;
 node1->children[0] = node2;
 node1->children[1] = ((void *)0);

 node2->splitplane = plane2;
 node2->children[0] = ((void *)0);
 node2->children[1] = ((void *)0);
 ...
 for (i=0; i<vnum; i++) {
  ...
  if (sideB == 2) {
   ...
   if (sideA == 1) {
    ...
    {v[0]=(*ptB)[0]-(*ptA)[0];v[1]=(*ptB)[1]-(*ptA)[1];v[2]=(*ptB)[2]-(*ptA)[2];};
    sect = - ((plane->normal[0]*(*ptA)[0]+plane->normal[1]*(*ptA)[1]+plane->normal[2]*(*ptA)[2])-plane->dist) /
        (plane->normal[0]*v[0]+plane->normal[1]*v[1]+plane->normal[2]*v[2]);
    VectorScale (v,sect,v);


    {newVert[0]=(*ptA)[0]+v[0];newVert[1]=(*ptA)[1]+v[1];newVert[2]=(*ptA)[2]+v[2];};
    {inpts[in_c][0]=newVert[0];inpts[in_c][1]=newVert[1];inpts[in_c][2]=newVert[2];};
    {outpts[out_c][0]=newVert[0];outpts[out_c][1]=newVert[1];outpts[out_c][2]=newVert[2];};

    out_c++;
    in_c++;
   }
   {outpts[out_c][0]=polygon[i][0];outpts[out_c][1]=polygon[i][1];outpts[out_c][2]=polygon[i][2];};
   out_c++;
  }
   ...
   {inpts[in_c][0]=polygon[i][0];inpts[in_c][1]=polygon[i][1];inpts[in_c][2]=polygon[i][2];};
   in_c++;
  }
   ...
   {inpts[in_c][0]=polygon[i][0];inpts[in_c][1]=polygon[i][1];inpts[in_c][2]=polygon[i][2];};
   {outpts[out_c][0]=inpts[in_c][0];outpts[out_c][1]=inpts[in_c][1];outpts[out_c][2]=inpts[in_c][2];};
   in_c++;
   out_c++;
  }
  ptA = ptB;
...
# 242 "d:/Data/Nintendo/TenebraeGX/src/gl_svbsp.c"
svnode_t *R_AddShadowCaster(svnode_t *node, vec3_t *v, int vnum, msurface_t *surf,int depth) {

 int sign;
 int signs[32],signs2[32];
 vec3_t v1[32],v2[32];
 int vnum1,vnum2;
 int i;
 ...
 if (vnum == 0) return ((void *)0);


 sign = 0;
 for (i=0; i<vnum; i++) {
  sign |= signs[i] = Epsilon_Sign ((v[i][0]*node->splitplane->normal[0]+v[i][1]*node->splitplane->normal[1]+v[i][2]*node->splitplane->normal[2])- node->splitplane->dist);
 }

 if (sign == 1) {

  if (node->children[0] != ((void *)0)) {
...
svnode_t *ExpandVolume(vec3_t *v,int *sign, int vnum, msurface_t *surf) {
 ...
 for (i=1; i<vnum; i++) {
  ...
  if (currnode->children[0] == ((void *)0)) break;
...
svnode_t *NodeFromEdge(vec3_t *v, int vnum, int edgeindex) {
 ...
 {vert1[0]=(*v1)[0]-currentshadowlight->origin[0];vert1[1]=(*v1)[1]-currentshadowlight->origin[1];vert1[2]=(*v1)[2]-currentshadowlight->origin[2];};

 {vert2[0]=(*v1)[0]-(*v2)[0];vert2[1]=(*v1)[1]-(*v2)[1];vert2[2]=(*v1)[2]-(*v2)[2];};

 CrossProduct (vert1, vert2, normal);
 VectorNormalize (normal);


 dist = (normal[0]*currentshadowlight->origin[0]+normal[1]*currentshadowlight->origin[1]+normal[2]*currentshadowlight->origin[2]);


 {tryplane.normal[0]=normal[0];tryplane.normal[1]=normal[1];tryplane.normal[2]=normal[2];};
 tryplane.dist = dist;

 plane = AllocPlane (&tryplane);
 if (!plane) return ((void *)0);
 ...
 res->children[0] = ((void *)0);
 res->children[1] = ((void *)0);
