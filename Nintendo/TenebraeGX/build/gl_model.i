# 1 "d:/Data/Nintendo/TenebraeGX/src/gl_model.c"
# 1 "d:\\Data\\Nintendo\\TenebraeGX\\build//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "d:/Data/Nintendo/TenebraeGX/src/gl_model.c"
# 25 "d:/Data/Nintendo/TenebraeGX/src/gl_model.c"
...
# 26 "d:/Data/Nintendo/TenebraeGX/src/gl_model.c" 2
...
byte mod_novis[8192/8];


model_t mod_known[512];
int mod_numknown = 0;

cvar_t gl_subdivide_size = {"gl_subdivide_size", "64", 1};
...
# 61 "d:/Data/Nintendo/TenebraeGX/src/gl_model.c"
void *Mod_Extradata (model_t *mod)
{
 void *r;
...
{
 static byte decompressed[8192/8];
 int c;
 byte *out;
 int row;
...
void Mod_UncacheAll(void) {
# 203 "d:/Data/Nintendo/TenebraeGX/src/gl_model.c"
 Cache_Flush();
 Mod_ClearAll();
}
...
# 266 "d:/Data/Nintendo/TenebraeGX/src/gl_model.c"
model_t *Mod_LoadModel (model_t *mod, qboolean crash)
{
 void *d;
 unsigned *buf;
...
# 366 "d:/Data/Nintendo/TenebraeGX/src/gl_model.c"
model_t *Mod_ForName (char *name, qboolean crash)
{
 model_t *mod;
     char target[128];
...
# 388 "d:/Data/Nintendo/TenebraeGX/src/gl_model.c"
byte *mod_base;
...
void Mod_LoadTextures (lump_t *l)
{
 ...
 for (i=0 ; i<m->nummiptex ; i++)
 {
  ...
  else
  {
   ...
   texture_mode = 0x2701;
   tx->gl_texturenum = GL_LoadTexture (mt->name, tx->width, tx->height, (byte *)(tx+1), 0, 0, 1);
   tx->gl_lumitex = GL_LoadLuma (mt->name, 1);
   texture_mode = 0x2601;
  }
 }
...
# 786 "d:/Data/Nintendo/TenebraeGX/src/gl_model.c"
void CalcSurfaceExtents (msurface_t *s)
{
 ...
 for (i=0 ; i<2 ; i++)
 {
  ...
  if ( !(tex->flags & 1) && s->extents[i] > 512 )
...
void Mod_LoadNodes (lump_t *l)
{
 ...
 Mod_SetParent (loadmodel->nodes, ((void *)0));
}
 ...
 for ( i=0 ; i<count ; i++, in++, out++)
 {
  ...
  if (out->contents == -3)
  {
   ...
   if (iswater) for (j=0 ; j<out->nummarksurfaces ; j++) {
    out->firstmarksurface[j]->flags |= 0x80;
   }
  }
  ...
  for (j=0; j<out->nummarksurfaces; j++) {
   if (out->firstmarksurface[j]->flags & 0x400) {
    out->firstmarksurface[j] = ((void *)0);
   }
  }

  newnum = 0;
  for (j=0; j<out->nummarksurfaces; j++) {
   if (out->firstmarksurface[j] != ((void *)0)) {
...
# 1153 "d:/Data/Nintendo/TenebraeGX/src/gl_model.c"
...
void Mod_LoadBrushModel (model_t *mod, void *buffer)
{
 ...
 if (i != 29)
  Sys_Error ("Mod_LoadBrushModel: %s has wrong version number (%i should be %i)", mod->name, i, 29);
 ...
 Con_DPrintf("Faces\n");
 Mod_LoadFaces (&header->lumps[7]);
 Con_DPrintf("MarkSurfaces\n");
 Mod_LoadMarksurfaces (&header->lumps[11]);
 Con_DPrintf("Visibility\n");
 Mod_LoadVisibility (&header->lumps[4]);
 Con_DPrintf("Leafs\n");
 Mod_LoadLeafs (&header->lumps[10]);
 Con_DPrintf("Nodes\n");
 Mod_LoadNodes (&header->lumps[5]);
 Con_DPrintf("ClipNodes\n");
 Mod_LoadClipnodes (&header->lumps[9]);
 ...
 for (i=0 ; i<mod->numsubmodels ; i++)
 {
  ...
  {mod->maxs[0]=bm->maxs[0];mod->maxs[1]=bm->maxs[1];mod->maxs[2]=bm->maxs[2];};
  {mod->mins[0]=bm->mins[0];mod->mins[1]=bm->mins[1];mod->mins[2]=bm->mins[2];};
...
# 1402 "d:/Data/Nintendo/TenebraeGX/src/gl_model.c"
aliashdr_t *pheader;

stvert_t stverts[2048];
mtriangle_t triangles[2048];



trivertx_t *poseverts[256];
int posenum;
...
void * Mod_LoadAliasFrame (void * pin, maliasframedesc_t *frame)
{
 ...
 for (i=0 ; i<3 ; i++)
 {
  ...
  aliasbboxmins[i] = (frame->bboxmin.v[i]<aliasbboxmins[i]?frame->bboxmin.v[i]:aliasbboxmins[i]);
  aliasbboxmaxs[i] = (frame->bboxmax.v[i]>aliasbboxmaxs[i]?frame->bboxmax.v[i]:aliasbboxmaxs[i]);
 }
...
# 1515 "d:/Data/Nintendo/TenebraeGX/src/gl_model.c"
typedef struct
{
 short x, y;
} floodfill_t;
...
# 1537 "d:/Data/Nintendo/TenebraeGX/src/gl_model.c"
void Mod_FloodFillSkin( byte *skin, int skinwidth, int skinheight )
{
 ...
 while (outpt != inpt)
 {
  ...
  outpt = (outpt + 1) & (0x1000 - 1);

  if (x > 0) { if (pos[-1] == fillcolor) { pos[-1] = 255; fifo[inpt].x = x + (-1), fifo[inpt].y = y + (0); inpt = (inpt + 1) & (0x1000 - 1); } else if (pos[-1] != 255) fdc = pos[-1]; };
  if (x < skinwidth - 1) { if (pos[1] == fillcolor) { pos[1] = 255; fifo[inpt].x = x + (1), fifo[inpt].y = y + (0); inpt = (inpt + 1) & (0x1000 - 1); } else if (pos[1] != 255) fdc = pos[1]; };
  if (y > 0) { if (pos[-skinwidth] == fillcolor) { pos[-skinwidth] = 255; fifo[inpt].x = x + (0), fifo[inpt].y = y + (-1); inpt = (inpt + 1) & (0x1000 - 1); } else if (pos[-skinwidth] != 255) fdc = pos[-skinwidth]; };
  if (y < skinheight - 1) { if (pos[skinwidth] == fillcolor) { pos[skinwidth] = 255; fifo[inpt].x = x + (0), fifo[inpt].y = y + (1); inpt = (inpt + 1) & (0x1000 - 1); } else if (pos[skinwidth] != 255) fdc = pos[skinwidth]; };
...
void *Mod_LoadAllSkins (int numskins, daliasskintype_t *pskintype)
{
 ...
 if (pskintype->type == ALIAS_SKIN_SINGLE) {
  ...
  GL_LoadTexture (name, pheader->skinwidth,
    pheader->skinheight, (byte *)(pskintype + 1), 1, 0, 1);
  ...
  pheader->gl_texturenum[i][j&3] =
      GL_LoadTexture (name, pheader->skinwidth,
        pheader->skinheight, (byte *)(pskintype), 1, 0, 1);
  pheader->gl_lumatex[i][j&3] = GL_LoadLuma(name, 1);
...
# 1673 "d:/Data/Nintendo/TenebraeGX/src/gl_model.c"
void Mod_LoadAliasModel (model_t *mod, void *buffer)
{
 int i, j;
 mdl_t *pinmodel;
 ...
 pheader->size = LittleFloat (pinmodel->size) * (1.0 / 11.0);
...
# 1885 "d:/Data/Nintendo/TenebraeGX/src/gl_model.c"
 ...
 *ppframe = pspriteframe;
 ...
 pspriteframe->gl_texturenum = GL_LoadTexture (name, width, height, (byte *)(pinframe + 1), 1, 1, 0);
...
void Mod_LoadSpriteModel (model_t *mod, void *buffer)
{
 ...
 if (version != 1)
  Sys_Error ("%s has wrong version number "
     "(%i should be %i)", mod->name, version, 1);
 ...
 COM_FindFile (filename,((void *)0),&f);
 if (f) {
...
# 2070 "d:/Data/Nintendo/TenebraeGX/src/gl_model.c"
void Mod_Print (void)
{
 int i;
 model_t *mod;
