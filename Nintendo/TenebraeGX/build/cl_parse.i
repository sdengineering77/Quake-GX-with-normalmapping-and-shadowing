# 1 "d:/Data/Nintendo/TenebraeGX/src/cl_parse.c"
# 1 "d:\\Data\\Nintendo\\TenebraeGX\\build//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "d:/Data/Nintendo/TenebraeGX/src/cl_parse.c"
# 22 "d:/Data/Nintendo/TenebraeGX/src/cl_parse.c"
...
# 23 "d:/Data/Nintendo/TenebraeGX/src/cl_parse.c" 2
...
# 79 "d:/Data/Nintendo/TenebraeGX/src/cl_parse.c"
...
void CL_ParseStartSoundPacket(void)
{
  ...
  attenuation = MSG_ReadByte () * 0.015625;
 else
  attenuation = 1.0;
...
# 147 "d:/Data/Nintendo/TenebraeGX/src/cl_parse.c"
...
void CL_ParseServerInfo (void)
{
 ...
 if (cl.maxclients < 1 || cl.maxclients > 64)
 {
 ...
 Con_Printf ("%c%s\n", 2, str);
# 261 "d:/Data/Nintendo/TenebraeGX/src/cl_parse.c"
...
# 336 "d:/Data/Nintendo/TenebraeGX/src/cl_parse.c"
int bitcounts[16];
...
void CL_ParseUpdate (int bits)
{
 ...
 if ((bits & (1<<15)) && (cl.server_proto_version == 667))
 {
  i = MSG_ReadShort ();
  bits |= (i<<16);
 }
...
# 457 "d:/Data/Nintendo/TenebraeGX/src/cl_parse.c"
 if (bits & (1<<13))
 ...
 {ent->msg_origins[1][0]=ent->msg_origins[0][0];ent->msg_origins[1][1]=ent->msg_origins[0][1];ent->msg_origins[1][2]=ent->msg_origins[0][2];};
 {ent->msg_angles[1][0]=ent->msg_angles[0][0];ent->msg_angles[1][1]=ent->msg_angles[0][1];ent->msg_angles[1][2]=ent->msg_angles[0][2];};
 ...
 {
  {ent->msg_origins[1][0]=ent->msg_origins[0][0];ent->msg_origins[1][1]=ent->msg_origins[0][1];ent->msg_origins[1][2]=ent->msg_origins[0][2];};
  {ent->origin[0]=ent->msg_origins[0][0];ent->origin[1]=ent->msg_origins[0][1];ent->origin[2]=ent->msg_origins[0][2];};
  {ent->msg_angles[1][0]=ent->msg_angles[0][0];ent->msg_angles[1][1]=ent->msg_angles[0][1];ent->msg_angles[1][2]=ent->msg_angles[0][2];};
  {ent->angles[0]=ent->msg_angles[0][0];ent->angles[1]=ent->msg_angles[0][1];ent->angles[2]=ent->msg_angles[0][2];};
  ent->forcelink = 1;
 }
}
...
void CL_ParseBaseline (entity_t *ent)
{
 ...
 if (cl.server_proto_version == 667) {
 ...
 {ent->origin[0]=ent->baseline.origin[0];ent->origin[1]=ent->baseline.origin[1];ent->origin[2]=ent->baseline.origin[2];};
 ent->model = cl.model_precache[ent->baseline.modelindex];

 R_FillEntityLeafs (ent);
}
# 593 "d:/Data/Nintendo/TenebraeGX/src/cl_parse.c"
void CL_ParseClientdata (int bits)
{
 ...
 {cl.mvelocity[1][0]=cl.mvelocity[0][0];cl.mvelocity[1][1]=cl.mvelocity[0][1];cl.mvelocity[1][2]=cl.mvelocity[0][2];};
 for (i=0 ; i<3 ; i++)
 {
  if (bits & ((1<<2)<<i) )
 ...
 cl.onground = (bits & (1<<10)) != 0;
 cl.inwater = (bits & (1<<11)) != 0;

 if (bits & (1<<12))
  cl.stats[5] = MSG_ReadByte ();
 else
  cl.stats[5] = 0;
...
void CL_NewTranslation (int slot)
{
 ...
 for (i=0 ; i<(1 << 6) ; i++, dest += 256, source+=256)
 {
  if (top < 128)
   memcpy (dest + 16, source + top, 16);
  else
   for (j=0 ; j<16 ; j++)
    dest[16 +j] = source[top+15-j];

  if (bottom < 128)
   memcpy (dest + 96, source + bottom, 16);
  else
   for (j=0 ; j<16 ; j++)
    dest[96 +j] = source[bottom+15-j];
 }
}
...
void CL_ParseStatic (void)
{
 ...
 ent->effects = ent->baseline.effects;
 {ent->color[0]=ent->baseline.color[0];ent->color[1]=ent->baseline.color[1];ent->color[2]=ent->baseline.color[2];};
 ent->alpha = ent->baseline.alpha;
 ent->style = ent->baseline.style;
 ent->light_lev = ent->baseline.light_lev;
 ent->pflags = ent->baseline.pflags;
 ...
 {ent->origin[0]=ent->baseline.origin[0];ent->origin[1]=ent->baseline.origin[1];ent->origin[2]=ent->baseline.origin[2];};
 {ent->angles[0]=ent->baseline.angles[0];ent->angles[1]=ent->baseline.angles[1];ent->angles[2]=ent->baseline.angles[2];};
 R_AddEfrags (ent);
 R_CalcSvBsp(ent);
...
# 815 "d:/Data/Nintendo/TenebraeGX/src/cl_parse.c"
void CL_ParseServerMessage (void)
{
 int cmd;
 int i;
 ...
 while (1)
 {
  ...
  case 32:
   ...
   if ( (cls.demoplayback || cls.demorecording) && (cls.forcetrack != -1) )
    CDAudio_Play ((byte)cls.forcetrack, 1);
   else
    CDAudio_Play ((byte)cl.cdtrack, 1);
   break;
