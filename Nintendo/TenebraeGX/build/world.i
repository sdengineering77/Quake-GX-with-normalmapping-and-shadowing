# 1 "d:/Data/Nintendo/TenebraeGX/src/world.c"
# 1 "d:\\Data\\Nintendo\\TenebraeGX\\build//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "d:/Data/Nintendo/TenebraeGX/src/world.c"
# 22 "d:/Data/Nintendo/TenebraeGX/src/world.c"
...
# 23 "d:/Data/Nintendo/TenebraeGX/src/world.c" 2
# 33 "d:/Data/Nintendo/TenebraeGX/src/world.c"
typedef struct
{
...
int SV_HullPointContents (hull_t *hull, int num, vec3_t p);
# 56 "d:/Data/Nintendo/TenebraeGX/src/world.c"
static hull_t box_hull;
static dclipnode_t box_clipnodes[6];
static mplane_t box_planes[6];
# 68 "d:/Data/Nintendo/TenebraeGX/src/world.c"
void SV_InitBoxHull (void)
{
 ...
 for (i=0 ; i<6 ; i++)
 {
  ...
  if (i != 5)
   ...
   box_clipnodes[i].children[side^1] = -2;
...
# 105 "d:/Data/Nintendo/TenebraeGX/src/world.c"
...
# 129 "d:/Data/Nintendo/TenebraeGX/src/world.c"
hull_t *SV_HullForEntity (edict_t *ent, vec3_t mins, vec3_t maxs, vec3_t offset)
{
 ...
 if (ent->v.solid == 4)
 {
  ...
  {size[0]=maxs[0]-mins[0];size[1]=maxs[1]-mins[1];size[2]=maxs[2]-mins[2];};
  if (size[0] < 3)
   hull = &model->hulls[0];
  else if (size[0] <= 32)
   hull = &model->hulls[1];
  ...
  {offset[0]=hull->clip_mins[0]-mins[0];offset[1]=hull->clip_mins[1]-mins[1];offset[2]=hull->clip_mins[2]-mins[2];};
  {offset[0]=offset[0]+ent->v.origin[0];offset[1]=offset[1]+ent->v.origin[1];offset[2]=offset[2]+ent->v.origin[2];};
 }
 else
 {

  {hullmins[0]=ent->v.mins[0]-maxs[0];hullmins[1]=ent->v.mins[1]-maxs[1];hullmins[2]=ent->v.mins[2]-maxs[2];};
  {hullmaxs[0]=ent->v.maxs[0]-mins[0];hullmaxs[1]=ent->v.maxs[1]-mins[1];hullmaxs[2]=ent->v.maxs[2]-mins[2];};
  hull = SV_HullForBox (hullmins, hullmaxs);

  {offset[0]=ent->v.origin[0];offset[1]=ent->v.origin[1];offset[2]=ent->v.origin[2];};
 }


 return hull;
}
# 181 "d:/Data/Nintendo/TenebraeGX/src/world.c"
...
areanode_t *SV_CreateAreaNode (int depth, vec3_t mins, vec3_t maxs)
{
 ...
 {mins1[0]=mins[0];mins1[1]=mins[1];mins1[2]=mins[2];};
 {mins2[0]=mins[0];mins2[1]=mins[1];mins2[2]=mins[2];};
 {maxs1[0]=maxs[0];maxs1[1]=maxs[1];maxs1[2]=maxs[2];};
 {maxs2[0]=maxs[0];maxs2[1]=maxs[1];maxs2[2]=maxs[2];};
...
# 263 "d:/Data/Nintendo/TenebraeGX/src/world.c"
void SV_UnlinkEdict (edict_t *ent)
{
 if (!ent->area.prev)
  return;
 RemoveLink (&ent->area);
 ent->area.prev = ent->area.next = ((void *)0);
}
...
void SV_TouchLinks ( edict_t *ent, areanode_t *node )
{
 ...
 for (l = node->trigger_edicts.next ; l != &node->trigger_edicts ; l = next)
 {
  ...
  if (touch == ent)
   continue;
  if (!touch->v.touch || touch->v.solid != 1)
   continue;
  ...
  pr_global_struct->self = ((byte *)touch - (byte *)sv.edicts);
  pr_global_struct->other = ((byte *)ent - (byte *)sv.edicts);
...
# 328 "d:/Data/Nintendo/TenebraeGX/src/world.c"
void SV_FindTouchedLeafs (edict_t *ent, mnode_t *node)
{
 ...
 sides = (((splitplane)->type < 3)? ( ((splitplane)->dist <= (ent->v.absmin)[(splitplane)->type])? 1 : ( ((splitplane)->dist >= (ent->v.absmax)[(splitplane)->type])? 2 : 3 ) ) : BoxOnPlaneSide( (ent->v.absmin), (ent->v.absmax), (splitplane)));
...
void SV_LinkEdict (edict_t *ent, qboolean touch_triggers)
{
 ...
 (ent->v.angles[0] || ent->v.angles[1] || ent->v.angles[2]) )
 {
  ...
  max = (ent->v.mins[0]*ent->v.mins[0]+ent->v.mins[1]*ent->v.mins[1]+ent->v.mins[2]*ent->v.mins[2]);
  v = (ent->v.maxs[0]*ent->v.maxs[0]+ent->v.maxs[1]*ent->v.maxs[1]+ent->v.maxs[2]*ent->v.maxs[2]);
  if (max < v)
  max = v;
  max = sqrt(max);
  for (i=0 ; i<3 ; i++)
 ...
 {
  {ent->v.absmin[0]=ent->v.origin[0]+ent->v.mins[0];ent->v.absmin[1]=ent->v.origin[1]+ent->v.mins[1];ent->v.absmin[2]=ent->v.origin[2]+ent->v.mins[2];};
  {ent->v.absmax[0]=ent->v.origin[0]+ent->v.maxs[0];ent->v.absmax[1]=ent->v.origin[1]+ent->v.maxs[1];ent->v.absmax[2]=ent->v.origin[2]+ent->v.maxs[2];};
 }



 if ((int)ent->v.effects & 16)
 {
  float radius = (eval_light_lev ? (*((float*)((char*)&ent->v + eval_light_lev))) : 300);
...
# 501 "d:/Data/Nintendo/TenebraeGX/src/world.c"
int SV_HullPointContents (hull_t *hull, int num, vec3_t p)
{
 float d;
 dclipnode_t *node;
...
# 537 "d:/Data/Nintendo/TenebraeGX/src/world.c"
int SV_PointContents (vec3_t p)
{
 int cont;
...
# 561 "d:/Data/Nintendo/TenebraeGX/src/world.c"
edict_t *SV_TestEntityPosition (edict_t *ent)
{
 trace_t trace;
...
# 591 "d:/Data/Nintendo/TenebraeGX/src/world.c"
...
# 651 "d:/Data/Nintendo/TenebraeGX/src/world.c"
 if (t1 < 0)
  frac = (t1 + (0.03125))/(t1-t2);
 else
  frac = (t1 - (0.03125))/(t1-t2);
 if (frac < 0)
  frac = 0;
 ...
 if (!SV_RecursiveHullCheck (hull, node->children[side], p1f, midf, p1, mid, trace) )
  return 0;
# 679 "d:/Data/Nintendo/TenebraeGX/src/world.c"
 ...
 if (!side)
 {
  {trace->plane.normal[0]=plane->normal[0];trace->plane.normal[1]=plane->normal[1];trace->plane.normal[2]=plane->normal[2];};
  trace->plane.dist = plane->dist;
 }
 else
 {
  {trace->plane.normal[0]=vec3_origin[0]-plane->normal[0];trace->plane.normal[1]=vec3_origin[1]-plane->normal[1];trace->plane.normal[2]=vec3_origin[2]-plane->normal[2];};
  trace->plane.dist = -plane->dist;
 }
...
# 732 "d:/Data/Nintendo/TenebraeGX/src/world.c"
trace_t SV_ClipMoveToEntity (edict_t *ent, vec3_t start, vec3_t mins, vec3_t maxs, vec3_t end)
{
 ...
 {start_l[0]=start[0]-offset[0];start_l[1]=start[1]-offset[1];start_l[2]=start[2]-offset[2];};
 {end_l[0]=end[0]-offset[0];end_l[1]=end[1]-offset[1];end_l[2]=end[2]-offset[2];};
 ...
 (ent->v.angles[0] || ent->v.angles[1] || ent->v.angles[2]) )
 {
  ...
  start_l[0] = (temp[0]*forward[0]+temp[1]*forward[1]+temp[2]*forward[2]);
  start_l[1] = -(temp[0]*right[0]+temp[1]*right[1]+temp[2]*right[2]);
  start_l[2] = (temp[0]*up[0]+temp[1]*up[1]+temp[2]*up[2]);

  {temp[0]=end_l[0];temp[1]=end_l[1];temp[2]=end_l[2];};
  end_l[0] = (temp[0]*forward[0]+temp[1]*forward[1]+temp[2]*forward[2]);
  end_l[1] = -(temp[0]*right[0]+temp[1]*right[1]+temp[2]*right[2]);
  end_l[2] = (temp[0]*up[0]+temp[1]*up[1]+temp[2]*up[2]);
 }
 ...
 (ent->v.angles[0] || ent->v.angles[1] || ent->v.angles[2]) )
 {
  ...
  if (trace.fraction != 1)
  {
   {a[0]=vec3_origin[0]-ent->v.angles[0];a[1]=vec3_origin[1]-ent->v.angles[1];a[2]=vec3_origin[2]-ent->v.angles[2];};
   AngleVectors (a, forward, right, up);

   {temp[0]=trace.endpos[0];temp[1]=trace.endpos[1];temp[2]=trace.endpos[2];};
   trace.endpos[0] = (temp[0]*forward[0]+temp[1]*forward[1]+temp[2]*forward[2]);
   trace.endpos[1] = -(temp[0]*right[0]+temp[1]*right[1]+temp[2]*right[2]);
   trace.endpos[2] = (temp[0]*up[0]+temp[1]*up[1]+temp[2]*up[2]);

   {temp[0]=trace.plane.normal[0];temp[1]=trace.plane.normal[1];temp[2]=trace.plane.normal[2];};
   trace.plane.normal[0] = (temp[0]*forward[0]+temp[1]*forward[1]+temp[2]*forward[2]);
   trace.plane.normal[1] = -(temp[0]*right[0]+temp[1]*right[1]+temp[2]*right[2]);
   trace.plane.normal[2] = (temp[0]*up[0]+temp[1]*up[1]+temp[2]*up[2]);
  }
 }
 ...
 if (trace.fraction != 1)
  {trace.endpos[0]=trace.endpos[0]+offset[0];trace.endpos[1]=trace.endpos[1]+offset[1];trace.endpos[2]=trace.endpos[2]+offset[2];};
...
# 823 "d:/Data/Nintendo/TenebraeGX/src/world.c"
void SV_ClipToLinks ( areanode_t *node, moveclip_t *clip )
{
 ...
 for (l = node->solid_edicts.next ; l != &node->solid_edicts ; l = next)
 {
  ...
  if (clip->type == 1 && touch->v.solid != 4)
   continue;
  ...
  if (clip->passedict)
  {
    if (((edict_t *)((byte *)sv.edicts + touch->v.owner)) == clip->passedict)
    continue;
   if (((edict_t *)((byte *)sv.edicts + clip->passedict->v.owner)) == touch)
    continue;
  }

  if ((int)touch->v.flags & 32)
