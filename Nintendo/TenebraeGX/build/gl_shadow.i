# 1 "d:/Data/Nintendo/TenebraeGX/src/gl_shadow.c"
# 1 "d:\\Data\\Nintendo\\TenebraeGX\\build//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "d:/Data/Nintendo/TenebraeGX/src/gl_shadow.c"
# 25 "d:/Data/Nintendo/TenebraeGX/src/gl_shadow.c"
...
# 26 "d:/Data/Nintendo/TenebraeGX/src/gl_shadow.c" 2

int numShadowLights;
int numStaticShadowLights;
int numUsedShadowLights;

shadowlight_t shadowlights[128];
shadowlight_t *usedshadowlights[64];
shadowlight_t *currentshadowlight;


int volumeCmdsBuff[131072 +128];
float volumeVertsBuff[87381 +128];
lightcmd_t lightCmdsBuff[65536 +128];
int numVolumeCmds;
int numLightCmds;
int numVolumeVerts;

msurface_t *shadowchain;
byte *lightvis;
byte worldvis[8192/8];
...
shadowlight_t* AllocShadowLight(void) {
 ...
 if (numShadowLights >= 128) {
  return ((void *)0);
 }

 shadowlights[numShadowLights].owner = ((void *)0);
 numShadowLights++;
...
void R_ShadowFromDlight(dlight_t *light) {
 ...
 {l->origin[0]=light->origin[0];l->origin[1]=light->origin[1];l->origin[2]=light->origin[2];};
...
int R_CubeMapLookup(int i) {
# 152 "d:/Data/Nintendo/TenebraeGX/src/gl_shadow.c"
 return 0;
...
void R_ShadowFromEntity(entity_t *ent) {
 ...
 {l->origin[0]=ent->origin[0];l->origin[1]=ent->origin[1];l->origin[2]=ent->origin[2];};
 l->radius = 350;
 l->color[0] = 1;
 l->color[1] = 1;
 l->color[2] = 1;
 ...
 {l->baseColor[0]=ent->color[0];l->baseColor[1]=ent->color[1];l->baseColor[2]=ent->color[2];};
...
# 234 "d:/Data/Nintendo/TenebraeGX/src/gl_shadow.c"
...
# 259 "d:/Data/Nintendo/TenebraeGX/src/gl_shadow.c"
...
int cut_ent;
# 285 "d:/Data/Nintendo/TenebraeGX/src/gl_shadow.c"
void R_InitShadowsForFrame(void) {
 ...
 cut_ent = 0;
 Q_memset (&worldvis, 0, 8192/8);

 vis = Mod_LeafPVS (r_viewleaf, cl.worldmodel);
 Q_memcpy(&worldvis, vis, 8192/8);

 for (i=0; i<numShadowLights; i++) {
  currentshadowlight = &shadowlights[i];
  if (R_ContributeFrame(currentshadowlight)) {
   currentshadowlight->visible = 1;
   if (numUsedShadowLights < 64) {
...
qboolean R_MarkShadowSurf(msurface_t *surf, shadowlight_t *light)
{
 ...
 if (( surf->flags & 0x10 ) || ( surf->flags & 4 )) {
  return 0;
 }
 ...
 case 2:
  ...
  break;
 default:
  dist = (light->origin[0]*plane->normal[0]+light->origin[1]*plane->normal[1]+light->origin[2]*plane->normal[2]) - plane->dist;
  break;
 }


 if (((surf->flags & 2) && (dist > 0)) ||
  (!(surf->flags & 2) && (dist < 0)))
 {
  return 0;
 }
...
# 388 "d:/Data/Nintendo/TenebraeGX/src/gl_shadow.c"
qboolean InShadowEntity(entity_t *ent) {
 ...
 {dst[0]=currentshadowlight->origin[0]-ent->origin[0];dst[1]=currentshadowlight->origin[1]-ent->origin[1];dst[2]=currentshadowlight->origin[2]-ent->origin[2];};
 d = Length (dst);

 if (d < (currentshadowlight->radius + radius)) {

  if (sh_noefrags.value) return 1;
...
# 429 "d:/Data/Nintendo/TenebraeGX/src/gl_shadow.c"
void MarkShadowEntities() {
 ...
 for (i=0 ; i<cl_numvisedicts ; i++)
 {
  ...
  if ((currententity->model->flags & 1024) && (currententity->model->flags & 512)) {
   continue;
  }

  if (mirror) {
   {mins[0]=currententity->origin[0]+currententity->model->mins[0];mins[1]=currententity->origin[1]+currententity->model->mins[1];mins[2]=currententity->origin[2]+currententity->model->mins[2];};
   {maxs[0]=currententity->origin[0]+currententity->model->maxs[0];maxs[1]=currententity->origin[1]+currententity->model->maxs[1];maxs[2]=currententity->origin[2]+currententity->model->maxs[2];};
...
# 482 "d:/Data/Nintendo/TenebraeGX/src/gl_shadow.c"
void R_ProjectSphere (shadowlight_t *light, int *rect)
{
 ...
 maxx = -1000000;
 maxy = -1000000;
# 506 "d:/Data/Nintendo/TenebraeGX/src/gl_shadow.c"
 for (i=16 ; i>=0 ; i--)
 {
  a = i/16.0 * 3.14159265358979323846*2;
  for (j=0 ; j<3 ; j++)
 ...
 if (minx < r_Iviewport[0]) minx = r_Iviewport[0];
 if (maxx < r_Iviewport[0]) maxx = r_Iviewport[0];

 if (maxx > r_Iviewport[2]) maxx = r_Iviewport[2];
 if (minx > r_Iviewport[2]) minx = r_Iviewport[2];

 if (miny < r_Iviewport[1]) miny = r_Iviewport[1];
 if (maxy < r_Iviewport[1]) maxy = r_Iviewport[1];

 if (maxy > r_Iviewport[3]) maxy = r_Iviewport[3];
 if (miny > r_Iviewport[3]) miny = r_Iviewport[3];
...
# 562 "d:/Data/Nintendo/TenebraeGX/src/gl_shadow.c"
...
# 596 "d:/Data/Nintendo/TenebraeGX/src/gl_shadow.c"
...
# 669 "d:/Data/Nintendo/TenebraeGX/src/gl_shadow.c"
qboolean R_ContributeFrame (shadowlight_t *light)
{
 mleaf_t *lightleaf;
 float dist;
...
# 704 "d:/Data/Nintendo/TenebraeGX/src/gl_shadow.c"
 if (dist == 0) {
 ...
 if (!sh_noscissor.value) {
  vec3_t dst;
  float d;
  {dst[0]=light->origin[0]-r_refdef.vieworg[0];dst[1]=light->origin[1]-r_refdef.vieworg[1];dst[2]=light->origin[2]-r_refdef.vieworg[2];};
  d = Length (dst);
  ...
  if (d > light->radius) {
   ...
   if (light->scizz.coords[0] >= light->scizz.coords[2] ||
    light->scizz.coords[1] >= light->scizz.coords[3]) {
    return 0;
   }
   light->scizz.coords[0] = light->scizz.coords[0]&0xFFFFFFFE;
   light->scizz.coords[1] = light->scizz.coords[1]&0xFFFFFFFE;
   light->scizz.coords[2] = (light->scizz.coords[2] + 1)&0xFFFFFFFE;
   light->scizz.coords[3] = (light->scizz.coords[3] + 1)&0xFFFFFFFE;
 ...
 if (light->isStatic) {
  ...
  Q_memcpy(&light->vis,lightvis,8192/8);
  Q_memcpy(&light->entvis, lightvis, 8192/8);
 }

 if (HasSharedLeafs(lightvis,&worldvis[0])) {
# 773 "d:/Data/Nintendo/TenebraeGX/src/gl_shadow.c"
  return 1;
 } else {
...
# 790 "d:/Data/Nintendo/TenebraeGX/src/gl_shadow.c"
qboolean R_FillShadowChain (shadowlight_t *light)
{
 ...
 return (shadowchain) ? 1 : 0;
}

void *VolumeVertsPointer;
# 822 "d:/Data/Nintendo/TenebraeGX/src/gl_shadow.c"
void R_ConstructShadowVolume(shadowlight_t *light) {
 ...
 if (gl_var) {
  if (light->numVolumeVerts < 2048*4/4) {
   memcpy(AGP_Buffer,light->volumeVerts,light->numVolumeVerts*4);
   VolumeVertsPointer = AGP_Buffer;
  }
 }
}
# 850 "d:/Data/Nintendo/TenebraeGX/src/gl_shadow.c"
void R_DrawShadowVolume(shadowlight_t *light) {
# 861 "d:/Data/Nintendo/TenebraeGX/src/gl_shadow.c"
...
# 884 "d:/Data/Nintendo/TenebraeGX/src/gl_shadow.c"
void R_MarkBrushModelSurfaces(entity_t *e) {
 ...
 {oldlightorigin[0]=currentshadowlight->origin[0];oldlightorigin[1]=currentshadowlight->origin[1];oldlightorigin[2]=currentshadowlight->origin[2];};
 ...
 {currentshadowlight->origin[0]=currentshadowlight->origin[0]-e->origin[0];currentshadowlight->origin[1]=currentshadowlight->origin[1]-e->origin[1];currentshadowlight->origin[2]=currentshadowlight->origin[2]-e->origin[2];};
 if (rotated)
 {
  vec3_t temp;
  ...
  {temp[0]=currentshadowlight->origin[0];temp[1]=currentshadowlight->origin[1];temp[2]=currentshadowlight->origin[2];};
  currentshadowlight->origin[0] = (temp[0]*forward[0]+temp[1]*forward[1]+temp[2]*forward[2]);
  currentshadowlight->origin[1] = -(temp[0]*right[0]+temp[1]*right[1]+temp[2]*right[2]);
  currentshadowlight->origin[2] = (temp[0]*up[0]+temp[1]*up[1]+temp[2]*up[2]);
...
e->angles[0] = -e->angles[0];
 ...
 {currentshadowlight->origin[0]=oldlightorigin[0];currentshadowlight->origin[1]=oldlightorigin[1];currentshadowlight->origin[2]=oldlightorigin[2];};
 glPopMatrix ();
}
# 1120 "d:/Data/Nintendo/TenebraeGX/src/gl_shadow.c"
void R_DrawBrushModelVolumes(entity_t *e) {
 ...
 for (i=0; i<model->nummodelsurfaces; i++, surf++)
 {
  ...
  for (j=0 ; j<surf->numedges ; j++)
  {
   if (ins->neighbourVis[count+j]) {
    GX_Begin(0x98, 3, 4);
    float *v1, *v2, *v3, *v4;
    v1 = (float *)(&globalVertexTable[surf->polys->firstvertex+j]);
    v2 = &ins->extvertices[count+j][0];
    v3 = (float *)(&globalVertexTable[surf->polys->firstvertex+((j+1)% poly->numverts)]);
    v4 = &ins->extvertices[count+((j+1)% poly->numverts) ][0];
    GX_Position3f32(v1[0], v1[1], v1[2]);
    GX_Position3f32(v2[0], v2[1], v2[2]);
    GX_Position3f32(v3[0], v3[1], v3[2]);
    GX_Position3f32(v4[0], v4[1], v4[2]);
# 1166 "d:/Data/Nintendo/TenebraeGX/src/gl_shadow.c"
    GX_End();
   }
  }
  ...
  GX_Begin(0xA0, 3, numEdges);
  for (j=0; j<numEdges; j++)
  {


   float *v = (float *)(&globalVertexTable[surf->polys->firstvertex+j]);
  ...
  for (j=numEdges-1; j>=0 ; j--)
  {
   ...
   float *v = &ins->extvertices[count+j][0];
   GX_Position3f32(v[0], v[1], v[2]);
  }
...
# 1218 "d:/Data/Nintendo/TenebraeGX/src/gl_shadow.c"
int getVertexIndexFromSurf(msurface_t *surf, int index, model_t *model) {
...
# 1252 "d:/Data/Nintendo/TenebraeGX/src/gl_shadow.c"
void PrecalcVolumesForLight(model_t *model) {
 msurface_t *surf;
 ...
 while (surf)
 {
  ...
  for (i=0 ; i<surf->numedges ; i++)
  {
   ...
   {v1[0]=(*v2)[0]-currentshadowlight->origin[0];v1[1]=(*v2)[1]-currentshadowlight->origin[1];v1[2]=(*v2)[2]-currentshadowlight->origin[2];};
   ...
   {vert1[0]=v1[0]+(*v2)[0];vert1[1]=v1[1]+(*v2)[1];vert1[2]=v1[2]+(*v2)[2];};
   {((vec3_t *)(volumeVerts+vertPos))[0][0]=vert1[0];((vec3_t *)(volumeVerts+vertPos))[0][1]=vert1[1];((vec3_t *)(volumeVerts+vertPos))[0][2]=vert1[2];};
   vertPos+=3;
  ...
  for (i=0 ; i<surf->numedges ; i++)
  {
   ...
   v2 = (vec3_t *)(&globalVertexTable[surf->polys->firstvertex+i]);
              volumeVerts[vertPos++] = (*v2)[0];
              volumeVerts[vertPos++] = (*v2)[1];
              volumeVerts[vertPos++] = (*v2)[2];
  }
  ...
  for (i=0 ; i<surf->numedges ; i++)
  {
   ...
   if (shadow) {
    ...
    if (!lastshadow) {
     ...
     volumeCmds[volumePos++] = 0x0008;
     numPos = volumePos;
     volumeCmds[volumePos++] = 4;
     stripLen = 2;


     volumeCmds[volumePos++] = startNearVerts+i;
     volumeCmds[volumePos++] = startVerts+i;

    }

    volumeCmds[volumePos++] = startNearVerts+((i+1)%poly->numverts);
  ...
  if (volumePos > 131072) {
   ...
   dist = (currentshadowlight->origin[0]*splitplane->normal[0]+currentshadowlight->origin[1]*splitplane->normal[1]+currentshadowlight->origin[2]*splitplane->normal[2]) - splitplane->dist;
   ...
   lightCmds[lightPos++].asInt = 0x0006;
   ...
   for (j=0 ; j<poly->numverts ; j++, v+= 7)
   {

    {nearToVert[0]=v[0]-nearPt[0];nearToVert[1]=v[1]-nearPt[1];nearToVert[2]=v[2]-nearPt[2];};


    lightCmds[lightPos++].asVec = (nearToVert[0]*(*s)[0]+nearToVert[1]*(*s)[1]+nearToVert[2]*(*s)[2]) * scale + 0.5;
    lightCmds[lightPos++].asVec = (nearToVert[0]*(*t)[0]+nearToVert[1]*(*t)[1]+nearToVert[2]*(*t)[2]) * scale + 0.5;


    {
     vec3_t lightDir, tsLightDir;

     {lightDir[0]=currentshadowlight->origin[0]-v[0];lightDir[1]=currentshadowlight->origin[1]-v[1];lightDir[2]=currentshadowlight->origin[2]-v[2];};

     if (surf->flags & 2) {
      tsLightDir[2] = -(lightDir[0]*surf->plane->normal[0]+lightDir[1]*surf->plane->normal[1]+lightDir[2]*surf->plane->normal[2]);
     } else {
      tsLightDir[2] = (lightDir[0]*surf->plane->normal[0]+lightDir[1]*surf->plane->normal[1]+lightDir[2]*surf->plane->normal[2]);
     }

     tsLightDir[1] = -(lightDir[0]*(*t)[0]+lightDir[1]*(*t)[1]+lightDir[2]*(*t)[2]);
     tsLightDir[0] = (lightDir[0]*(*s)[0]+lightDir[1]*(*s)[1]+lightDir[2]*(*s)[2]);
...
void DrawVolumeFromCmds(int *volumeCmds, lightcmd_t *lightCmds, float *volumeVerts) {
 ...
 while (1) {
  ...
  GX_Begin(GX_AsGXPrimitiveType(command), 3, num);

  if ((command == 0x0008) || (command == 0x0007)) {
   for (i=0; i<num; i++) {
    int ind = volumeCmds[volumePos++];
    v = &volumeVerts[ind*3];
    GX_Position3f32(v[0], v[1], v[2]);
   }
  } else {
  ...
  GX_End();
# 1543 "d:/Data/Nintendo/TenebraeGX/src/gl_shadow.c"
 }
...
# 1593 "d:/Data/Nintendo/TenebraeGX/src/gl_shadow.c"
void R_RenderGlow (shadowlight_t *light)
{
 vec3_t hit = {0, 0, 0};
 double x,y,z;
 ...
 if (Length(hit) != 0) {
  light->haloalpha = light->haloalpha - (cl.time - cl.oldtime)*2;
 ...
 gluProject(light->origin[0], light->origin[1], light->origin[2], r_Dworld_matrix,
                   r_Dproject_matrix, (GLint *) r_Iviewport, &x, &y, &z);


 if (!hitWorld) {


  glReadPixels((int)x,(int)y,1,1,0x1902,0x1406,&realz);
  if (realz < z) {
   light->haloalpha = light->haloalpha - (cl.time - cl.oldtime)*2;
   if (light->haloalpha < 0) return;
  } else {
 ...
 glColor4f(light->color[0],light->color[1],light->color[2],light->haloalpha*0.7);
 ...
 glEnd ();
# 1698 "d:/Data/Nintendo/TenebraeGX/src/gl_shadow.c"
 glEnable(0x0BC0);
 glColor3f (1,1,1);
 glDisable (0x0BE2);
...
# 1715 "d:/Data/Nintendo/TenebraeGX/src/gl_shadow.c"
svnode_t *currentlightroot;
vec3_t testvect = {10,10,10};
# 1727 "d:/Data/Nintendo/TenebraeGX/src/gl_shadow.c"
...
# 1776 "d:/Data/Nintendo/TenebraeGX/src/gl_shadow.c"
void AddToShadowBsp(msurface_t *surf) {
 vec3_t surfvects[32];
 int numsurfvects;
 int i;
 float *v;
# 1824 "d:/Data/Nintendo/TenebraeGX/src/gl_shadow.c"
 if (surf->numedges > 32) {
 ...
 for (i=0, v=(float *)(&globalVertexTable[surf->polys->firstvertex]); i<numsurfvects; i++, v+=7) {
  {surfvects[i][0]=v[0];surfvects[i][1]=v[1];surfvects[i][2]=v[2];};
 }
...
# 1854 "d:/Data/Nintendo/TenebraeGX/src/gl_shadow.c"
void R_RecursiveShadowAdd(mnode_t *node)
{
 ...
 case 2:
  ...
  break;
 default:
  dot = (currentshadowlight->origin[0]*plane->normal[0]+currentshadowlight->origin[1]*plane->normal[1]+currentshadowlight->origin[2]*plane->normal[2]) - plane->dist;
  break;
 }
...
# 1930 "d:/Data/Nintendo/TenebraeGX/src/gl_shadow.c"
...
# 1956 "d:/Data/Nintendo/TenebraeGX/src/gl_shadow.c"
...
int done = 0;
# 1974 "d:/Data/Nintendo/TenebraeGX/src/gl_shadow.c"
void R_CalcSvBsp(entity_t *ent) {
  ...
  || !strcmp (ent->model->name, "progs/w_light.spr"))
 {
  ...
  for (i=0 ; i<cl.worldmodel->numtextures ; i++)
  {
   if (!cl.worldmodel->textures[i]) continue;
    cl.worldmodel->textures[i]->texturechain = ((void *)0);
  }
  ...
  Q_memcpy(&currentshadowlight->vis[0], lightvis, 8192/8);
  Q_memcpy(&currentshadowlight->entvis[0], lightvis, 8192/8);
...
void LightFromSurface(msurface_t *surf) {
 ...
 for (i=0 ; i<poly->numverts ; i++, v+= 7)
 {
  {center[0]=center[0]+v[0];center[1]=center[1]+v[1];center[2]=center[2]+v[2];};
 }
 ...
 if (surf->flags & 2) {
  ...
  {normal[0]=surf->plane->normal[0];normal[1]=surf->plane->normal[1];normal[2]=surf->plane->normal[2];};
 }
 ...
 for (i=0; i<numShadowLights; i++) {
  ...
  {dist[0]=orig[0]-light->origin[0];dist[1]=orig[1]-light->origin[1];dist[2]=orig[2]-light->origin[2];};
  length = Length(dist);

  if (length < (350.0*sh_radiusscale.value + light->radius*sh_radiusscale.value)) {
   tooClose = 1;
  }
 }
 ...
 if (!tooClose) {
  ...
  fakeEnt.light_lev = 350.0;
  {fakeEnt.origin[0]=orig[0];fakeEnt.origin[1]=orig[1];fakeEnt.origin[2]=orig[2];};
  fakeEnt.model = Mod_ForName("progs/w_light.spr",1);
  R_CalcSvBsp(&fakeEnt);
...
# 2349 "d:/Data/Nintendo/TenebraeGX/src/gl_shadow.c"
