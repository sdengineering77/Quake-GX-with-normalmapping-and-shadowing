# 1 "d:/Data/Nintendo/TenebraeGX/src/gl_md3.c"
# 1 "d:\\Data\\Nintendo\\TenebraeGX\\build//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "d:/Data/Nintendo/TenebraeGX/src/gl_md3.c"
# 24 "d:/Data/Nintendo/TenebraeGX/src/gl_md3.c"
...
# 25 "d:/Data/Nintendo/TenebraeGX/src/gl_md3.c" 2
...
void ClosestPointOnLine(vec3_t a, vec3_t b, vec3_t p, vec3_t res)
{
 ...
 {c[0]=p[0]-a[0];c[1]=p[1]-a[1];c[2]=p[2]-a[2];};
 {V[0]=b[0]-a[0];V[1]=b[1]-a[1];V[2]=b[2]-a[2];};
 d = Length(V);
 VectorNormalize(V);
 t = (V[0]*c[0]+V[1]*c[1]+V[2]*c[2]);
 ...
 VectorScale(V, t, V);
 {res[0]=a[0]+V[0];res[1]=a[1]+V[1];res[2]=a[2]+V[2];};
}
...
void Orthogonalize(vec3_t v1, vec3_t v2, vec3_t res)
{
 ...
 {res[0]=v2[0]-v2ProjV1[0];res[1]=v2[1]-v2ProjV1[1];res[2]=v2[2]-v2ProjV1[2];};
 VectorNormalize(res);
}
...
# 281 "d:/Data/Nintendo/TenebraeGX/src/gl_md3.c"
void Mod_LoadMd3Model (model_t *mod, void *buffer)
{
 int i, j, k, l;
 ...
 vec3_t md3scale = {(1.0/64), (1.0/64), (1.0/64)};
 ...
 if (pinmodel->version != 15)
  Sys_Error ("%s has wrong version number (%i should be %i)",
     mod->name, pinmodel->version, 15);


 pinmodel->numFrames=LittleLong(pinmodel->numFrames);
 pinmodel->numTags=LittleLong(pinmodel->numTags);
 pinmodel->numSurfaces=LittleLong(pinmodel->numSurfaces);

 pinmodel->numSkins=LittleLong(pinmodel->numSkins);

 pinmodel->ofsFrames=LittleLong(pinmodel->ofsFrames);
 pinmodel->ofsTags=LittleLong(pinmodel->ofsTags);
 pinmodel->ofsSurfaces=LittleLong(pinmodel->ofsSurfaces);
 pinmodel->ofsEnd=LittleLong(pinmodel->ofsEnd);
# 344 "d:/Data/Nintendo/TenebraeGX/src/gl_md3.c"
 if ( pinmodel->numFrames < 1 ) {
  Con_Printf( "LoadMd3Model: %s has no frames\n", mod->name );
  return;
 }

 if ( pinmodel->numFrames > 256) {
...
# 392 "d:/Data/Nintendo/TenebraeGX/src/gl_md3.c"
  if ( surf->numVerts > 2048)
 ...
 surf = (md3Surface_t *) ( (byte *)pinmodel + pinmodel->ofsSurfaces );
# 444 "d:/Data/Nintendo/TenebraeGX/src/gl_md3.c"
...
for (surfcount = 0; surfcount < pinmodel->numSurfaces; ++surfcount) {
 ...
 {pheader->scale[0]=md3scale[0];pheader->scale[1]=md3scale[1];pheader->scale[2]=md3scale[2];};
 {pheader->scale_origin[0]=md3origin[0];pheader->scale_origin[1]=md3origin[1];pheader->scale_origin[2]=md3origin[2];};
 pheader->boundingradius = 100;
 {pheader->eyeposition[0]=md3origin[0];pheader->eyeposition[1]=md3origin[1];pheader->eyeposition[2]=md3origin[2];};
 pheader->numskins = 1;
 pheader->skinwidth = 4;
 pheader->skinheight = 4;
 ...
 for (i=0; i<pheader->numframes; i++, frame++) {
  ...
  for (j=0; j<pheader->poseverts; j++) {
   ...
   verts[k].v[0] = xyz[k].xyz[0]*(1.0/64);
   verts[k].v[1] = xyz[k].xyz[1]*(1.0/64);
   verts[k].v[2] = xyz[k].xyz[2]*(1.0/64);
   verts[k].lightnormalindex = xyz[k].normal;

   for (l=0; l<3; l++) {
    pheader->mins[l] = (pheader->mins[l]<verts[k].v[l]?pheader->mins[l]:verts[k].v[l]);
    pheader->maxs[l] = (pheader->maxs[l]>verts[k].v[l]?pheader->maxs[l]:verts[k].v[l]);
   }
  }

  for (j=0; j<3; j++) {
   mod->mins[j] = (mod->mins[j]<pheader->mins[j]?mod->mins[j]:pheader->mins[j]);
   mod->maxs[j] = (mod->maxs[j]>pheader->maxs[j]?mod->maxs[j]:pheader->maxs[j]);
  }
 ...
 for (i=0; i<pheader->numposes; i++) {
  for (j=0; j<pheader->numtris ; j++) {
   ...
   {v1[0]=triangle[0][0]-triangle[1][0];v1[1]=triangle[0][1]-triangle[1][1];v1[2]=triangle[0][2]-triangle[1][2];};
   {v2[0]=triangle[2][0]-triangle[1][0];v2[1]=triangle[2][1]-triangle[1][1];v2[2]=triangle[2][2]-triangle[1][2];};
   CrossProduct(v2,v1, normal);
   VectorScale(normal, 1/Length(normal), norms[i*pheader->numtris+j].normal);


   norms[i*pheader->numtris+j].dist = (triangle[0][0]*norms[i*pheader->numtris+j].normal[0]+triangle[0][1]*norms[i*pheader->numtris+j].normal[1]+triangle[0][2]*norms[i*pheader->numtris+j].normal[2]);
  }
 }
 ...
 for (i=0; i<pheader->numposes; i++) {
  ...
  for (j=0; j<pheader->numtris; j++) {
   ...
   for (k=0; k<3; k++) {
    ...
    {tangents[i*pheader->poseverts+l][0]=tangents[i*pheader->poseverts+l][0]+tangent[0];tangents[i*pheader->poseverts+l][1]=tangents[i*pheader->poseverts+l][1]+tangent[1];tangents[i*pheader->poseverts+l][2]=tangents[i*pheader->poseverts+l][2]+tangent[2];}
                                        ;
    {binormals[i*pheader->poseverts+l][0]=binormals[i*pheader->poseverts+l][0]+binormal[0];binormals[i*pheader->poseverts+l][1]=binormals[i*pheader->poseverts+l][1]+binormal[1];binormals[i*pheader->poseverts+l][2]=binormals[i*pheader->poseverts+l][2]+binormal[2];}
                                         ;
    numNormals[l]++;
   }
  }
 ...
 pheader->gl_texturenum[0][3] = GL_LoadTexture (shadername, 4, 4, &fake[0], 1, 0, 1);

 pheader->gl_lumatex[0][0] =
 pheader->gl_lumatex[0][1] =
 pheader->gl_lumatex[0][2] =
 pheader->gl_lumatex[0][3] = GL_LoadLuma(shadername, 1);
...
# 709 "d:/Data/Nintendo/TenebraeGX/src/gl_md3.c"
