...
//		GX_SetTevColorIn(tevstage, GX_CC_RASC, GX_CC_ZERO, GX_CC_TEXC, GX_CC_ZERO);
		GX_SetTevColorIn(tevstage, GX_CC_RASC, GX_CC_ZERO, mode ? GX_CC_ZERO : GX_CC_TEXC, GX_CC_ZERO);
		GX_SetTevColorOp(tevstage, GX_TEV_COMP_A8_GT, GX_TB_ZERO, GX_CS_DIVIDE_2, GX_TC_LINEAR, GX_TEVPREV);
//		GX_SetTevOrder(tevstage, GX_TEXCOORD3, GX_TEXMAP2, GX_COLOR0A0);
		GX_SetTevOrder(tevstage, GX_TEXCOORD2, GX_TEXMAP1, GX_COLOR0A0);
		...
		GX_SetTevIndirect(
			...
			GX_ITW_OFF, // do not use texture wrapping
			GX_ITW_OFF, // do not use texture wrapping
			GX_TRUE, // add in previous coordinates (TODO: are these coordinates or lookup results?)
			GX_FALSE, // don't use mipmap stuff
			GX_ITBA_OFF // don't use bump alpha (TODO: can this be used to blend reflection and difuse?)
			); 
		GX_SetTevColorIn(tevstage, GX_CC_RASC, GX_CC_ZERO, GX_CC_TEXC, GX_CC_ZERO);
//		GX_SetTevColorIn(tevstage, GX_CC_RASC, GX_CC_ZERO, mode ? GX_CC_TEXC : GX_CC_ZERO, GX_CC_ZERO);
		...
		GX_SetTevColorIn(tevstage, GX_CC_ZERO, GX_CC_HALF, GX_CC_TEXC, GX_CC_CPREV);
		GX_SetTevColorOp(tevstage, GX_TEV_ADD, GX_TB_ZERO, GX_CS_DIVIDE_2, GX_TC_LINEAR, GX_TEVPREV);
		GX_SetTevOrder(tevstage, GX_TEXCOORD1, GX_TEXMAP1, GX_COLORNULL);
	
		// setup number of tev stages
		GX_SetNumTevStages(tevstage+1);
