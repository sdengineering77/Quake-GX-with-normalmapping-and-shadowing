...
void R_InitTextureHeap (void)
{
	...
	texture_heap_size = 35 * 1024 * 1024;
...
//	TPL_GetTexture(&lightTPL, normal_map, &normalmap);
	TPL_GetTexture(&lightTPL, light_map, &lightmap);
	TPL_GetTexture(&lightTPL, specular_map, &specularmap);
	r_lightdirtexnum = GL_CreateLightDirTexRGB565(&normalmap); // TODO: remove texnum stuff
...
============
*/
int GL_CreateLightDirTexRGB565(GXTexObj *texObj) {
	u16 texbuf[256*256];
	s16 x, y;
	float height = 16;
	memset(&texbuf[0], 0xFF, 256*256*2);
	for (y=-128; y<128; y++) {
		for (x=-128; x<128; x++) {
			float vect_x = -x;
			float vect_y = -y;
			...
			texbuf[(x+128) + (y+128)*256] = (g<<5|b);
		}
	}


	u16 *buf = (u16 *) __lwp_heap_allocate(&texture_heap, 256 * 256 * TEXTURE_SIZE);
	u16 *bufptr = buf; 
	if (!bufptr)
	...
	for(y=0; y<256; y+=4) {
		for(x=0; x<256; x+=4) {
			bufptr[ 0]  = texbuf[x +       y*256];
			bufptr[ 1]  = texbuf[x + 1 + y*256];
			bufptr[ 2]  = texbuf[x + 2 + y*256];
			bufptr[ 3]  = texbuf[x + 3 + y*256];
			bufptr[ 4]  = texbuf[x +       (y+1)*256];
			bufptr[ 5]  = texbuf[x + 1 + (y+1)*256];
			bufptr[ 6]  = texbuf[x + 2 + (y+1)*256];
			bufptr[ 7]  = texbuf[x + 3 + (y+1)*256];
			bufptr[ 8]  = texbuf[x +       (y+2)*256];
			bufptr[ 9]  = texbuf[x + 1 + (y+2)*256];
			bufptr[10] = texbuf[x + 2 + (y+2)*256];
			bufptr[11] = texbuf[x + 3 + (y+2)*256];
			bufptr[12] = texbuf[x +       (y+3)*256];
			bufptr[13] = texbuf[x + 1 + (y+3)*256];
			bufptr[14] = texbuf[x + 2 + (y+3)*256];
			bufptr[15] = texbuf[x + 3 + (y+3)*256];
			
			bufptr +=16; 
		}
	}

	GX_InitTexObj(texObj, buf, 256, 256, GX_TF_RGB565, GX_CLAMP, GX_CLAMP, GX_TRUE);
//	GX_InitTexObjLOD(texObj,GX_LINEAR,GX_LINEAR, 1, 1, 0.0f, GX_TC_GE, GX_TRUE, GX_ANISO_1);
	DCFlushRange(buf, 256*256*2);
	return 0;
} 
