 ...
 * invertOp = true = invert stencil operation, so backface = add, frontface = subtract
 * =================================================
 */
void GX_InitShadowShader(bool add, bool invertOp) {
	...
	if (add) {
		if (invertOp) {
			...
			GX_SetCullMode(GX_CULL_FRONT); // do not invert 
		}
		GX_SetBlendMode(GX_BM_BLEND, GX_BL_ONE, GX_BL_ONE, GX_LO_NOOP); // add
	} else {
		if (invertOp) {
			GX_SetCullMode(GX_CULL_FRONT);
		} else {
			GX_SetCullMode(GX_CULL_BACK); // do not invert 
		}
		GX_SetBlendMode(GX_BM_SUBSTRACT, GX_BL_ONE, GX_BL_ONE, GX_LO_NOOP); // substract
	}
