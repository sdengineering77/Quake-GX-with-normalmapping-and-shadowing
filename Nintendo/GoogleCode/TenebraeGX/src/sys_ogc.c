...
dirdata_t *Sys_Findfirst (char *dirname, char *filter, dirdata_t *dirdata) {
	...
	if (dirdata && filter){
		...
		// DRS TODO: originally closed in uxdata_free, but get a DSI exception when doing so... Perhaps a 
		// small memoryleak in COM_LoadPakFile? 
		closedir(dir); 
...
int main(int argc, char* argv[]) {
    ...
    //printf("Tenebrae GX needs a printf...\n");
	VID_Setup(); // We may need the console, and also need a VSync
