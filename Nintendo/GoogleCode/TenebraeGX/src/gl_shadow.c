...
// DRS TODO: whatever this may be for...
#ifndef GXGL
	if (i > NUM_CUBEMAPS) {
		return 0;
	} else {
...
void R_ProjectSphere (shadowlight_t *light, int *rect)
{
	...
	//for(i=0; i<4; i++) {
	//	Sys_Printf("sphere_prj %04.4f %04.4f %04.4f %04.4f\n", r_Dproject_matrix[i*4+0], r_Dproject_matrix[i*4+1], r_Dproject_matrix[i*4+2], r_Dproject_matrix[i*4+3]);
	//}
	//for(i=0; i<4; i++) {
	//	Sys_Printf("sphere_mv %04.4f %04.4f %04.4f %04.4f\n", r_Dworld_matrix[i*4+0], r_Dworld_matrix[i*4+1], r_Dworld_matrix[i*4+2], r_Dworld_matrix[i*4+3]);
	//}
	//Sys_Printf("r_refdef.vieworg %04.4f %04.4f %04.4f\n", r_refdef.vieworg[0], r_refdef.vieworg[1], r_refdef.vieworg[2]);
	...
	for (i=16 ; i>=0 ; i--)
	{
		...
		// TODO: DRS I dont understand why this originally worked in GL without inverting the y axis.
		// got vague results on GX so I added it... 
		// can't understand why it calculates 16 points either; origin, origin+rad*vup, origin+rad*vright 
		// should be enough?
		py = glheight - py;
...
//Sys_Printf("R_ContributeFrame %p\n", light);	

//if (!light->isStatic) return false;

	//verry soft light, don't bother.
//Sys_Printf("R_ContributeFrame b %f\n", b);	
	...
	int i;
	for (i=0; i<3; i++) {
		mins[i] = light->origin[i] - light->radius * 0.5f;
		maxs[i] = light->origin[i] + light->radius * 0.5f;
	}
...
//Sys_Printf("R_ContributeFrame or %4.1f %4.1f %4.1f ra %f dist %f\n", light->origin[0], light->origin[1], light->origin[2], light->radius, dist);	
	if (dist == 0)  {
...
#ifdef GXGL
			// DRS make coords multiple of 2. Also, this shouldn't happen because of frustum check, but if width && height == 0 return false
			if (light->scizz.coords[0] >= light->scizz.coords[2] || 
				light->scizz.coords[1] >= light->scizz.coords[3]) {
				return false;
			}
			light->scizz.coords[0] = light->scizz.coords[0]&0xFFFFFFFE; // align to prev boundary
			light->scizz.coords[1] = light->scizz.coords[1]&0xFFFFFFFE;
			light->scizz.coords[2] = (light->scizz.coords[2] + 1)&0xFFFFFFFE; // align to next boundary
...
//Sys_Printf("R_ContributeFrame return shared leafs\n");	
		return true;
	} else {
//Sys_Printf("R_ContributeFrame return NO shared leafs\n");	
		return false;
	}
...
*/
// DRS TODO: visible volumes?		glColor3f(1,0,0);
...
void R_DrawBrushModelVolumes(entity_t *e) {
	...
	for (i=0; i<model->nummodelsurfaces; i++, surf++)
	{
		...
		GX_Begin(GX_TRIANGLEFAN, GX_VTXFMT3, numEdges);
...
// DRS TODO: arrays	glVertexPointer(3, GL_FLOAT, 0, volumeVerts);
// DRS TODO: arrays	glEnableClientState(GL_VERTEX_ARRAY);
...
// DRS TODO: arrays		glDrawElements(command,num,GL_UNSIGNED_INT,&volumeCmds[volumePos]);
// DRS TODO: arrays		volumePos+=num;

		GX_Begin(GX_AsGXPrimitiveType(command), GX_VTXFMT3, num);
