...
void R_DrawSequentialPoly (msurface_t *s)
{
	...
	// fluids with watermap
	if (s->flags & SURF_DRAWTURB) {
		// Bind textures
		t = s->texinfo->texture;
		GL_Bind0 (t->gl_texturenum);
		R_DrawFluidPolyBumpShadow(s);
		return;
	}
...
// instanced model
	...
	for (i=0 ; i<clmodel->nummodelsurfaces ; i++, psurf++) {
		...
		for (l=0; l<r_contributinglightscount ; l++) {
			...
			//guVecMultiply(modelinv, (Vector *) &light->origin[0], &delta);
			dot = guVecDotProduct (&delta, (Vector *) &pplane->normal[0]) - pplane->dist;
			if (((psurf->flags & SURF_PLANEBACK) && (dot < -BACKFACE_EPSILON)) ||
				(!(psurf->flags & SURF_PLANEBACK) && (dot > BACKFACE_EPSILON))) {
	...
	psurf = &clmodel->surfaces[clmodel->firstmodelsurface];
	for (i=0 ; i<clmodel->nummodelsurfaces ; i++, psurf++) {
		for(l=0; l<psurf->numstaticlights; l++) {
			// add dynamic shadow volume
			UT_AddShadowVolumeTranslate(psurf->lightdata[l].light, psurf, model);
		}
	}
...
void BuildSurfaceDisplayList (msurface_t *fa)
{
	...
	boolean createfans = false;

// reconstruct the polygon
	pedges = currentmodel->edges;
	lnumverts = fa->numedges + (createfans ? 2 : 0); // DRS +1
	vertpage = 0;
	...
	for (i=0 ; i<lnumverts ; i++) {
		if (i > 0 || !createfans) {
			lindex = currentmodel->surfedges[fa->firstedge + (i - (createfans ? 1 : 0)) % fa->numedges];
//			lindex = currentmodel->surfedges[fa->firstedge + i ];
...
//Con_DPrintf("=======================\r\n");			
	...
	//
	if (!createfans && !gl_keeptjunctions.value && !(fa->flags & SURF_UNDERWATER) )
	{
		for (i = 0 ; i < lnumverts ; ++i)
		{
			vec3_t v1, v2;
