...
cvar_t		scr_fov = {"fov","106"};	// DRS 16:9
...
qboolean scr_takeScreenshot; // DRS temp hack
...
//	r_refdef.fov_y = CalcFov (r_refdef.fov_x, r_refdef.vrect.width, r_refdef.vrect.height);
	r_refdef.fov_y = CalcFov (r_refdef.fov_x, 16, 9); // DRS 16:9
...
================== 
*/  
//void *scr_screenshotbuf;
void SCR_ScreenShot_f (void) 
{
	
	u16 		w = glwidth;//640;
	u16		h = glheight;//480;
	byte		buffer[3 * 640 * 528 + 18];
	char		pcxname[80]; 
...
//	buffer = (byte *) scr_screenshotbuf;// malloc(w*h*3 + 18);
	memset (buffer, 0, 18);
...
//	gxCopyToTextureRGB8(0, 0, w, h, w, h, buffer+18); // DRS TODO revise 
	GL_GrabXFB(buffer+18);
	...
	DCFlushRange(buffer, w*h*3 + 18);
	COM_WriteFile (pcxname, buffer, w*h*3 + 18);
