...
void GL_CreateDivTex(GXTexObj *tex) {
	int res = 64; 
	u8 texbuf[res*res];
	s16 x, y;
...
void GL_CreateWaterTexRGB565(GXTexObj *texObj) {
	u16 *buf = (u16 *) memalign(32, 32*32*2);
	glwatermap = buf;
	if (!buf)
		Sys_Error("createLightDirTexRGB565: Out of memory.");

	GX_InitTexObj(texObj, buf, 32, 32, GX_TF_RGB565, GX_REPEAT, GX_REPEAT, GX_TRUE);
	GX_InitTexObjLOD(texObj,GX_LINEAR,GX_LINEAR, 1, 1, 0.0f, GX_TC_GE, GX_TRUE, GX_ANISO_1);
	DCFlushRange(buf, 32*32*2);
} 


void GL_CreateLightBulbTexI8(GXTexObj *texLightDir, int specularpower) {
	u8 texbuf[128*128];
	s16 x, y;
//	float height = 2;
...
//		for (x=-32; x<32; x++) {
	...
	for (y=0; y<fact; y++) {
		for (x=0; x<fact; x++) {
			...
			float cos = powf(sqrtf(1 - sin*sin), specularpower);
...
//	TPL_GetTexture(&lightTPL, specular_map, &specularmap);
	...
	GL_CreateLightBulbTexI8(&lightmap, 1);	
	GL_CreateLightBulbTexI8(&specularmap, 8);	
	GL_CreateLightRampTexI8(&lightattnmap);
	GL_CreateLightShadesTexI8(&attenuatedshadesmap);
	GL_CreateWaterTexRGB565(&watermap);
	GL_CreateDivTex(&divmap);
	R_InitStencilTextures(); // DRS needed some hook, move when an applicable place has been found
...
void GX_CopyRGBA8_To_CMPR(u8 *dest, u32 *src, int x1, int y1, int x2, int y2, int src_width) {
	u8 raw4x4[16*4];
	int x, y, xx, yy, xxx, yyy;

	// sample tiles of 8x8 pixels
	for (y=y1; y<y2; y+=8) { 
		for (x=x1; x<x2; x+=8) {
			// sample subblocks 4x4
			for(yy=0; yy<8; yy+=4) {
				for(xx=0; xx<8; xx+=4) {
					// sample the subblock pixels
					for(yyy=0; yyy<4; yyy++) {
						for(xxx=0; xxx<4; xxx++) {
							u32 r, g, b, srccolor;
							srccolor = src[xxx+xx+x+(yyy+yy+y)*src_width];
							r = srccolor & 0xFF;
							srccolor >>= 8;
							g = srccolor & 0xFF;
							srccolor >>= 8;
					...
					// calc tile address. 32 bytes/tile * (xtile + ytile * wtile) 
					tiledest += 32 * (xtile + ytile * wtile);
					// calc subblock address: 8 bytes/block * (xblock + yblock * wblock)
					tiledest += 8 * (xblock + yblock * wblock);

					stb_compress_dxt_block(tiledest, raw4x4, 0, 0);
				}
			}
		}
	}
...
//	destination->data = __lwp_heap_allocate(&texture_heap, scaled_width * scaled_height/2);
//	int size = GX_GetTexBufferSize(scaled_width, scaled_height, GX_TF_CMPR, GX_FALSE, 1);
//	if (size != scaled_width * scaled_height/2) {
//Con_DPrintf("required %i used %i - ", size, scaled_width * scaled_height/2);
//	}
	...
	if (destination->type == 4) { //
		GX_CopyRGBA8_To_CMPR((u8 *) destination->data, scaled, 0, 0, scaled_width, scaled_height, scaled_width);
		GX_InitTexObj(&destination->gx_tex, destination->data, scaled_width, scaled_height, GX_TF_CMPR, GX_REPEAT, GX_REPEAT, GX_FALSE);
		GX_InitTexObjLOD(&destination->gx_tex, GX_LIN_MIP_LIN, GX_LIN_MIP_LIN, 1, 1, 0.0f, GX_TC_GE, GX_TRUE, GX_ANISO_1);
	} else {
		...
		GX_InitTexObjLOD(&destination->gx_tex, GX_LINEAR, GX_LINEAR, 0, 1, 0.0f, GX_TRUE, GX_TRUE, GX_ANISO_1);
	}
...
===============
*/
void GL_UploadHL (gltexture_t *destination, byte *data, int width, int height,  qboolean mipmap, qboolean alpha)
{
	int			i, s, p;
	byte 	*pal, *buf;
	...
	for (i = 0; i<s; i++) {
		...
		if (alpha && p == 255) {
			...
			p *= 3;
			buf[i * 4 + 0] = (byte) 255;
			buf[i * 4 + 1] = (byte) pal[p + 2];
			buf[i * 4 + 2] = (byte) pal[p + 1];
			buf[i * 4 + 3] = (byte) pal[p];
		}
	}
		Con_DPrintf("uploaded alpha %i w/h %i/%i\r\n", alpha, width, height);
...
/*
===============
GL_RenderNormalMapHL
===============
*/
void GL_RenderNormalMapHL (gltexture_t *destination, byte *data, int width, int height,  qboolean mipmap, qboolean alpha)
{
	static byte	bumpmap[512*512];
	int			i, s, p, r, g, b;
	unsigned	*normalmap;
...
================
*/
int GL_LoadNormalMapHL (char *baseTextureIdentifier, byte *data)
{
	int			i;
	gltexture_t	*glt;
	gltexture_t *base = 0;
...
reload:
	...
	glt->keep = base->keep;
	glt->used = true;
	GL_RenderNormalMapHL (glt, data, base->width, base->height, base->mipmap,  (baseTextureIdentifier[0] == '{'));
...
================
*/
int GL_LoadTextureHL (char *identifier, int width, int height, byte *data, qboolean mipmap)
{
	int			i;
	gltexture_t	*glt;
...
reload:
	...
	GL_UploadHL(glt, data, width, height, mipmap, (identifier[0] == '{') );
...
//					GX_InitTexObj(&gltextures[i].gx_tex, gltextures[i].data, gltextures[i].scaled_width, gltextures[i].scaled_height, GX_TF_RGB565, GX_REPEAT, GX_REPEAT, /*mipmap ? GX_TRUE :*/ GX_FALSE);
					GX_InitTexObj(&gltextures[i].gx_tex, gltextures[i].data, gltextures[i].scaled_width, gltextures[i].scaled_height, GX_TF_CMPR, GX_REPEAT, GX_REPEAT, GX_FALSE);
					GX_InitTexObjLOD(&gltextures[i].gx_tex, GX_LIN_MIP_LIN, GX_LIN_MIP_LIN, 1, 1, 0.0f, GX_TC_GE, GX_TRUE, GX_ANISO_1);
				...
				if (gltextures[i].type == 2) {
					...
					GX_InitTexObjLOD(&gltextures[i].gx_tex, GX_LINEAR, GX_LINEAR, 1, 1, 0.0f, GX_TC_GE, GX_TRUE, GX_ANISO_1);
					DCFlushRange(gltextures[i].data, gltextures[i].scaled_width * gltextures[i].scaled_height * TEXTURE_SIZE);
				}
