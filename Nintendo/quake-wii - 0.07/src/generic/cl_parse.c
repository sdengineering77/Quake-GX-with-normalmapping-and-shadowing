...
void CL_ParseStaticLights(char *data) {
	...
	// clear all lights
	memset(&cl_staticlights[0], 0x00, MAX_WORLDLIGHTS*sizeof(dlight_t));
	// parse ents
	while (cl_numstaticlights<MAX_WORLDLIGHTS) 	{
		...
		if (ent.v.classname && strncmp("light", (char *)ent.v.classname, 5) == 0) {
			...
			cl_staticlights[cl_numstaticlights].flags = LIGHT_FLAG_STATIC|LIGHT_FLAG_POINTLIGHT;

			float *pRadius = (float *) &ent.v.velocity; // hack light level TODO: just make light parse method, ditch the edict
			float *pStyle  = (float *) &ent.v.effects; // hack style
			
			if (strncmp("light_torch", (char *)ent.v.classname, 11) == 0 || 
				strncmp("light_flame", (char *)ent.v.classname, 11) == 0) {
				cl_staticlights[cl_numstaticlights].radius = 240.0f; // These really should be visible
			} else 			
			if (*pRadius > 0) {
				cl_staticlights[cl_numstaticlights].radius = (*pRadius); 
			} else {
				cl_staticlights[cl_numstaticlights].radius = 256.0f; // For lacking light?
			}
			if (*pStyle > 0 && *pStyle < 255) {
...
// local state
	...
	// DRS links the leafs to surfaces
//	CL_LinkLeafs();
//	TestLinks();
	// DRS load lights...
